...
function v722H2HStrip(h2,h,a){
  ...
  return `<div class="v722-h2h">
    <div class="v722-h2h-top">
      <b>ULTIMI 5 TESTA A TESTA</b>
      <small>${hw} V ${realTeamCode(h)} · ${d} P · ${aw} V ${realTeamCode(a)}</small>
    </div>
    <div class="v722-h2h-results">${results}</div>
  </div>`;
}
...
function v722TeamTech(metrics,team,sample){
  ...
  return `<section class="v19-card v722-team-tech">
    <div class="v19-chead">
      <b>DATI TECNICI</b>
      <span>ULTIME ${sample||0}/5</span>
    </div>
    <div class="v722-team-tech-id">
      ${realCrest(team)}
      <b>${realEsc(team.name||"")}</b>
    </div>
    <div class="v722-team-tech-grid">
      ${rows.map(r=>`<div>
        <span>${r[0]}</span>
        <strong>${r[2]==="pct"?realFmt(r[1],0)+"%":realFmt(r[1],r[2])}</strong>
      </div>`).join("")}
    </div>
  </section>`;
}

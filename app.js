...
function v7726FilterLeagueLogo(f){
  ...
  const selectors=[
    '[data-league="'+name.replace(/"/g,'\\"')+'"] img',
    '[data-league-name="'+name.replace(/"/g,'\\"')+'"] img',
    '.league-filter img[alt="'+name.replace(/"/g,'\\"')+'"]',
    '.v60-league-filter img[alt="'+name.replace(/"/g,'\\"')+'"]'
  ];
  for(const sel of selectors){
    try{
      const el=document.querySelector(sel);
      if(el?.src) return `<img class="v7726-filter-league-logo" src="${realEsc(el.src)}" alt="">`;
    }catch{}
  }

  // Same asset resolver used by the filter row. v60LeagueLogo may return HTML;
  // accept it only when it contains a real <img>, never its initials fallback.
  try{
    const league=(typeof v60LeagueFromFixture==="function") ? v60LeagueFromFixture(f) : null;
    const html=(league && typeof v60LeagueLogo==="function") ? (v60LeagueLogo(league,false)||"") : "";
...
async function v67MatchCenter(){
  ...
  try{
    const [dayFs,livePack]=await Promise.all([realFixtures(date),realApi("/live-fixtures").catch(()=>({data:[]}))]);
...
async function v7711PerformanceData(){
  ...
  let confSum=0,confN=0,fixturesDone=0;
  ...
  for(const [date,rows] of Object.entries(byDate)){
    ...
    for(const e of rows){
      ...
      for(const [name,p] of Object.entries(e.markets||{})){
        ...
        v7711Add(markets[name],hit); v7711Add(marketTotal,hit);
      }
      for(const s of (e.topSignals||[])){
        if(!signals[s.type])continue;
        const hit=v7711PickHit(s.pick,o);
        v7711Add(signals[s.type],hit); v7711Add(signalTotal,hit);
  ...
  return {markets,signals,marketTotal,signalTotal,bands,avgConfidence:confN?Math.round(confSum/confN):null,fixturesDone,tracked:entries.length};
}
...
function v7711PerfRow(label,s,unit){
  const t=v7711StatText(s);
  return `<div class="perf-row v7711-perf-row"><b>${label}</b><span>${t.ratio} ${unit}</span><strong>${t.pct}</strong></div>`;
}
async function performance(){
  ...
  document.querySelector("#main").innerHTML=`<div class="page"><button class="back-btn" data-nav="more">‹ ALTRO</button><div class="eyebrow">MODEL PERFORMANCE</div><h1>PERFORMANCE</h1><p class="subtitle">Solo pronostici pre-match salvati e partite concluse FT.</p>
   ...
   <section class="panel"><div class="section-head"><h2>PER MERCATO</h2></div>
    ${v7711PerfRow("1X2",d.markets["1X2"],"pronostici")}
    ${v7711PerfRow("Over/Under 2.5",d.markets["Over/Under"],"pronostici")}
    ${v7711PerfRow("Gol/No Gol",d.markets["Gol/No Gol"],"pronostici")}
    <div class="perf-row v7711-total"><b>TOTALE MERCATI</b><span>${mt.ratio} pronostici</span><strong>${mt.pct}</strong></div>
   </section>
   <section class="panel"><div class="section-head"><h2>PER TOP SIGNAL</h2></div>
    ${v7711PerfRow("Confidence",d.signals.Confidence,"segnali")}
    ${v7711PerfRow("Value",d.signals.Value,"segnali")}
    ${v7711PerfRow("Forma",d.signals.Forma,"segnali")}
    <div class="perf-row v7711-total"><b>TOTALE TOP SIGNAL</b><span>${st.ratio} segnali</span><strong>${st.pct}</strong></div>
   </section>
   <p class="v7711-note">Programmate e LIVE non entrano nei conteggi. Le vecchie partite vengono recuperate solo quando esiste già uno snapshot pre-match congelato sul dispositivo; i risultati si aggiornano automaticamente a FT.</p>
  </div>`;
  bind();
}

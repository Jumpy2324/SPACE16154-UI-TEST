  ...
  try{
    ...
    const render=(filter="all")=>{
      ...
      list.innerHTML=groups.length?groups.map(([l,fs])=>`<section class="v67-center-league"><h2>${realEsc(l)}</h2>${fs.map(f=>{
        ...
        return `<button class="v67-center-card" data-real-match="${f.id}">
          <div class="v67-center-meta">${v67FixtureStatusHtml(f)}</div>
          <div class="v67-center-teams"><div>${realCrest(h)}<b>${realEsc(h.name||"—")}</b></div><span>VS</span><div>${realCrest(a)}<b>${realEsc(a.name||"—")}</b></div></div>
        </button>`}).join("")}</section>`).join(""):`<div class="empty-note">Nessuna partita per questo filtro.</div>`;
 ...
 try{
  ...
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   ...
   ${fs.slice(1,4).length ? `<section class="v5-upcoming">
    <div class="v2-panel-head"><h2>PROSSIME PARTITE</h2><button data-nav="matches">Vedi tutte ›</button></div>
    <div class="v5-upcoming-list">${fs.slice(1,4).map(x=>`<div class="v23-match-card">
     <div class="v23-time">${v67FixtureStatusHtml(x)}</div>
     <div class="v23-match-row"><div class="v23-team home">${realCrest(x.h)}<b>${realEsc(x.h.name||"")}</b></div><button class="v23-analysis" data-real-match="${x.id}">ANALISI</button><div class="v23-team away"><b>${realEsc(x.a.name||"")}</b>${realCrest(x.a)}</div></div>
    </div>`).join("")}</div>
   </section>` : ""}
  </div>`;
  bind();
    ...
    root.innerHTML=`<section class="m13-page v712-match-center">
      ...
      <div class="v712-fixture-list">
        ${visible.length?visible.map(f=>{
          ...
          return `<article class="v712-fixture-card ${st}" data-real-match="${f.id}">
            <div class="v712-fixture-top">
              <span class="v712-league-mini v714-league-mini">${l?v60LeagueLogo(l,true):""}<b>${realEsc(f.leagueName||"")}</b></span>
              <span class="v712-status ${st}">${st==="live"?"● LIVE":st==="finished"?"FT":realEsc(f.time||"")}</span>
            </div>
            <div class="v712-fixture-main">
              <div class="v712-team">${realCrest(f.h)}<b>${realEsc(f.h.name||"")}</b></div>
              <div class="v712-score">${v68HeroCenter(f,f)}</div>
              <div class="v712-team away">${realCrest(f.a)}<b>${realEsc(f.a.name||"")}</b></div>
            </div>
            <div class="v712-open">ANALISI <span>›</span></div>
          </article>`;

...
function renderSig(){
 ...
 document.querySelector("#sigList").innerHTML=matches.length?matches.map(m=>{
   ...
   return `<button class="v41-signal-card" data-signal-match="${m.id}">
 ...
 document.querySelectorAll("[data-signal-match]").forEach(b=>b.onclick=()=>v43SignalDetail(b.dataset.signalMatch));
}

/* ===== V43 TOP SIGNAL DETAIL ===== */
function v43SignalDetail(matchId){
 const m=MATCHES.find(x=>x.id===matchId); if(!m) return signalsPage();
 const p=topOutcome(m), c=m.signals.find(s=>s.type==="Confidence"), v=m.signals.find(s=>s.type==="Value"), f=m.signals.find(s=>s.type==="Forma");
 const primary=c||v||f||m.signals[0];
 const drivers=[
 ...
 document.querySelector("#main").innerHTML=`<div class="page v43-signal-detail">
  ...
  <div class="eyebrow">SIGNAL DETAIL</div>
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=9.6.0-ui-test-v44",{updateViaCache:"none"}).catch(()=>{}));

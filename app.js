...
const CONFIG={
  version:"12.2.2-v72-2-structural",
...
${v72TeamTech(metrics,team,sample)}
    </div>`;
    bind();
...
function v72H2HStrip(h2,h,a){
  if(!Array.isArray(h2)||!h2.length)return "";
  let hw=0,aw=0,d=0;
  const chips=h2.slice(0,5).map(f=>{
    const home=realLoc(f,"home")||{},away=realLoc(f,"away")||{};
    const hs=realScore(f,home.id),as=realScore(f,away.id);
    if(hs==null||as==null)return "";
    const hGoals=String(home.id)===String(h.id)?hs:(String(away.id)===String(h.id)?as:null);
    const aGoals=String(home.id)===String(a.id)?hs:(String(away.id)===String(a.id)?as:null);
    if(hGoals==null||aGoals==null)return "";
    if(hGoals>aGoals)hw++; else if(aGoals>hGoals)aw++; else d++;
    return `<span>${hGoals}-${aGoals}</span>`;
  }).filter(Boolean).join("");
  return `<div class="v72-h2h-strip"><div class="v72-h2h-head"><b>ULTIMI 5 TESTA A TESTA</b><small>${hw} V ${realTeamCode(h)} · ${d} P · ${aw} V ${realTeamCode(a)}</small></div><div class="v72-h2h-chips">${chips}</div></div>`;
}
function v72TechnicalCompare(tech,h,a){
  const rows=(Array.isArray(tech)?tech:[]).filter(r=>r[1]!=null || r[2]!=null);
  if(!rows.length)return "";
  return `<section class="v72-tech-card"><div class="v72-tech-title"><h2>DATI TECNICI</h2><span>MEDIA ULTIME 5</span></div><div class="v72-tech-teams"><div>${realCrest(h)}<b>${realEsc(h.name)}</b></div><small>CONFRONTO</small><div>${realCrest(a)}<b>${realEsc(a.name)}</b></div></div><div class="v72-tech-list">${rows.map(r=>`<div class="v72-tech-row"><strong>${v58FmtMetric(r[0],r[1])}</strong><span>${realEsc(r[0])}</span><strong>${v58FmtMetric(r[0],r[2])}</strong></div>`).join("")}</div><p class="real-note">Medie sulle ultime 5 gare utili. I dati mancanti non vengono stimati.</p></section>`;
}
function v72TeamTech(metrics,team,sample){
  const rows=[["xG",metrics.xg,2],["xGA",metrics.xga,2],["Gol fatti",metrics.gf,1],["Gol subiti",metrics.ga,1],["Possesso",metrics.pos,"pct"],["Tiri",metrics.shots,1],["Tiri in porta",metrics.sot,1],["Corner",metrics.corners,1]].filter(r=>r[1]!=null);
  if(!rows.length)return "";
  return `<section class="v19-card v72-team-tech"><div class="v19-chead"><b>DATI TECNICI</b><span>ULTIME ${sample||0}/5</span></div><div class="v72-team-tech-head">${realCrest(team)}<b>${realEsc(team.name||"")}</b></div><div class="v72-team-tech-list">${rows.map(r=>`<div><span>${r[0]}</span><strong>${r[2]==="pct"?realFmt(r[1],0)+"%":realFmt(r[1],r[2])}</strong></div>`).join("")}</div></section>`;
}
function v72ExplicitOfficialLineups(ctx,h,a){
  const xs=Array.isArray(ctx?.lineups)?ctx.lineups:[];
  const ok=t=>{
    const team=xs.filter(x=>String(x?.team_id??x?.participant_id)===String(t.id));
    if(team.length<7)return false;
    return team.some(x=>x?.confirmed===true||x?.is_confirmed===true||x?.official===true||x?.is_official===true||/confirm|official/i.test(String(x?.status||x?.state||x?.type?.name||"")));
  };
  return ok(h)&&ok(a);
}
...
function v59HasOfficialLineups(ctx,h,a){return v72ExplicitOfficialLineups(ctx,h,a)}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=12.2.2-v72-2-structural",{updateViaCache:"none"}).catch(()=>{}));

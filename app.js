...
const CONFIG={
  version:"10.3.0-ui-test-v53-prematch-real",
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=10.3.0-ui-test-v53-prematch-real",{updateViaCache:"none"}).catch(()=>{}));
...
function smFormHtml(items){
  if(!items.length)return '<span class="sm53-muted">Nessuna gara recente disponibile</span>';
  return `<div class="sm53-form">${items.map(x=>`<span class="${x.r==='W'?'w':x.r==='L'?'l':x.r==='D'?'d':''}" title="${escapeHtml(x.opp)}">${x.r}</span>`).join('')}</div>`;
}
function smTeamPrematchCard(title,team,history,standing){
  const form=smRecentForm(history,team?.id),avg=smHistoryAverages(history,team?.id),xg=smXgAverage(history,team?.id);
  if(xg!==null&&!avg.some(x=>x.label==='xG'))avg.unshift({label:'xG',value:xg,count:history.length});
  const gf=form.filter(x=>x.gf!==null).map(x=>x.gf),ga=form.filter(x=>x.ga!==null).map(x=>x.ga);
  return `<section class="sm53-teamcard"><div class="sm53-teamhead">${shield(teamCode(team),'',team?.image_path||'')}<div><small>${title}</small><b>${escapeHtml(team?.name||'—')}</b>${standing?`<em>Classifica: ${escapeHtml(standing)}°</em>`:''}</div></div>
    <div class="sm53-sub">ULTIME 5</div>${smFormHtml(form)}
    ${(gf.length||ga.length)?`<div class="sm53-scoreline"><span>Gol fatti <b>${gf.length?smFmt(gf.reduce((a,b)=>a+b,0)/gf.length):'—'}</b></span><span>Gol subiti <b>${ga.length?smFmt(ga.reduce((a,b)=>a+b,0)/ga.length):'—'}</b></span></div>`:''}
    ${avg.length?`<div class="sm53-mini-grid">${avg.slice(0,8).map(x=>`<div><span>${escapeHtml(x.label)}</span><b>${smFmt(x.value,x.label==='Possesso %'?0:1)}</b><small>${x.count} rilev.</small></div>`).join('')}</div>`:`<div class="sm53-emptyline">Statistiche recenti non disponibili nel pacchetto ricevuto.</div>`}
  </section>`;
}
function smH2hHtml(pack){
  const arr=smHistory(pack?.h2h);
  if(!arr.length)return `<div class="sm53-emptyline">Nessun H2H disponibile.</div>`;
  return `<div class="sm53-h2h">${arr.slice().sort((a,b)=>smDateTs(b)-smDateTs(a)).slice(0,5).map(f=>{
    ...
    return `<div><span>${escapeHtml(String(f?.starting_at||'').slice(0,10))}</span><b>${escapeHtml(h.name||'Casa')} ${hg??'—'}–${ag??'—'} ${escapeHtml(a.name||'Trasferta')}</b></div>`;
  }).join('')}</div>`;
}
function smFixtureStatsHtml(detail){
  const stats=Array.isArray(detail?.statistics)?detail.statistics:[];
  if(!stats.length)return `<div class="sm51-empty"><b>GARA NON ANCORA INIZIATA</b><span>Le statistiche live della fixture compariranno qui quando SportMonks le renderà disponibili.</span></div>`;
...
function smDetailModalHtml(state,pack=null,detail=null,error=''){
  if(state==='loading')return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button><div class="sm51-kicker">SPORTMONKS LIVE DATA</div><h2>Caricamento analisi pre-match…</h2><div class="sm51-spinner"></div></div></div>`;
  if(state==='error')return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button><div class="sm51-kicker">SPORTMONKS</div><h2>Impossibile caricare i dati</h2><p>${escapeHtml(error)}</p></div></div>`;
  ...
  const dateLabel=String(fx?.starting_at||detail?.starting_at||isoDate()).slice(0,10);
  const stateName=detail?.state?.name||'Pre-match';
  const hh=smHistory(pack?.home_history),ah=smHistory(pack?.away_history);
  const hs=smStanding(pack,h.id),as=smStanding(pack,a.id);
  return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal sm53-modal" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button>
    <div class="sm51-kicker">SPORTMONKS · DATI REALI</div><h2>ANALISI PRE-MATCH</h2>
    <div class="sm51-meta"><span class="sm52-league-pill">${leagueImg?`<img src="${escapeHtml(leagueImg)}" alt="">`:''}${escapeHtml(league)}</span><span>${escapeHtml(dateLabel)}</span><span>${escapeHtml(f.time||String(fx?.starting_at||'').slice(11,16)||'--:--')}</span></div>
    <div class="sm51-teams sm52-detail-teams"><div>${shield(teamCode(h),'',h.image_path||'')}<b>${escapeHtml(h.name||'Casa')}</b></div><strong>VS</strong><div><b>${escapeHtml(a.name||'Trasferta')}</b>${shield(teamCode(a),'away',a.image_path||'')}</div></div>
    <div class="sm51-state">STATO: <b>${escapeHtml(stateName)}</b> · storico ${escapeHtml(pack?.history_window?.days||90)} giorni</div>
    <div class="sm53-two">${smTeamPrematchCard('CASA',h,hh,hs)}${smTeamPrematchCard('TRASFERTA',a,ah,as)}</div>
    <div class="sm51-section-title">TESTA A TESTA</div>${smH2hHtml(pack)}
    <div class="sm51-section-title">STATISTICHE DELLA FIXTURE</div>${smFixtureStatsHtml(detail)}
    <div class="sm51-note">Medie e forma sono calcolate esclusivamente sui dati storici ricevuti da SportMonks. Nessun valore demo viene usato. Gli indicatori proprietari SPACE16154 saranno aggiunti dopo la calibrazione.</div>
  </div></div>`;
}

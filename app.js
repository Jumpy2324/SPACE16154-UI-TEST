...
async function realMatchDetail(id){
  ...
  try{
    ...
    const v7515FormSeq=(team,preHistory)=>{
      if(!v60FixtureFinished(currentFt)) return v58Metrics(preHistory,team).form;
      const currentResult=realRecent([currentFt],team.id)[0];
      const previous=v58TeamFixtures(preHistory,team.id)
        .filter(f=>String(f?.id||"")!==String(currentFt?.id||""))
        .slice(0,4);
      const seq=realRecent(previous,team.id);
      return currentResult?[...seq,currentResult]:seq;
    };
    const hForm=v7515FormSeq(h,preHfs),aForm=v7515FormSeq(a,preAfs);

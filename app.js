...
const CONFIG={
  version:"12.1.4-v71-4-dashboard-matches",
...
async function realDashboard(){
 ...
 try{
  ...
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   ...
   <section class="v6-hero-combined v714-dashboard-hero">
    <div class="v2-panel-head"><h2>MATCH IN EVIDENZA <span>☆</span></h2><b>${activeMeta.short.toUpperCase()}</b></div>
    <div class="v2-meta">${v60LeagueLogo(comp,true)} ${realEsc(m.leagueName)} <i>•</i> ${activeMeta.short} <i>•</i> ${realEsc(m.time)}${stateLabel}</div>
    <div class="v2-match">
     <div class="v2-club">${realCrest(m.h)}<strong>${realEsc(m.h.name||"").toUpperCase()}</strong></div>
     <div class="v2-center">${v68HeroCenter(m,detail)}<button data-real-match="${m.id}">⌁ &nbsp; ${state==="ft"?"Post-match completo":"Analisi completa"}</button></div>
     <div class="v2-club">${realCrest(m.a)}<strong>${realEsc(m.a.name||"").toUpperCase()}</strong></div>
    </div>
    <div class="v6-prob-divider"></div>
    <div class="v6-prob-title">${state==="ft"?"PROBABILITÀ PRE-MATCH":"ESITO PIÙ PROBABILE"} ${conf!=null?`<span class="v60-dash-sample">Confidence ${conf}%</span>`:""}</div>
    ${pred?(()=>{const b1=v61UniqueBest([pred.one,pred.x,pred.two]),bg=v61UniqueBest([pred.gg,pred.ng]),bo=v61UniqueBest([pred.over,pred.under]);return `
      <div class="v712-market-strip">
       <div class="v712-market-block"><b>1X2</b><div class="v712-three">
       ...
       <div class="v712-market-block"><b>GOL / NO GOL</b><div class="v712-two">
       ...
       <div class="v712-market-block"><b>OVER / UNDER 2.5</b><div class="v712-two">
      ...
      <div class="v68-pred-unavailable">${state==="ft"?"Probabilità pre-match non salvate per questa gara. Dalla prossima partita verranno conservate anche dopo il termine.":"Campione insufficiente per una previsione reale."}</div>`}
   </section>
 ...
 try{
  ...
  const render=()=>{
    ...
    root.innerHTML=`<section class="m13-page v712-match-center">
      ...
      <div class="v714-league-select-wrap">
        <label for="v714LeagueSelect">CAMPIONATO</label>
        <div class="v714-league-select">
          <span class="v714-select-logo">${leagueFilter==="all"?"★":(()=>{const l=leagues.find(x=>x.key===leagueFilter);return l?v60LeagueLogo(l,true):"★"})()}</span>
          <select id="v714LeagueSelect" aria-label="Filtra per campionato">
            <option value="all" ${leagueFilter==="all"?"selected":""}>Tutti i campionati</option>
            ${leagues.map(l=>`<option value="${l.key}" ${leagueFilter===l.key?"selected":""}>${realEsc(l.name)}</option>`).join("")}
          </select>
          <span class="v714-select-chevron">⌄</span>
        </div>
      </div>
    ...
    const leagueSelect=root.querySelector("#v714LeagueSelect");
    if(leagueSelect) leagueSelect.onchange=()=>{leagueFilter=leagueSelect.value;render()};
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=12.1.4-v71-4-dashboard-matches",{updateViaCache:"none"}).catch(()=>{}));

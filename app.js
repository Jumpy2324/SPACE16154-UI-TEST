...
function shell(a="dashboard"){
 return `<header class="topbar v2-topbar v7-topbar">
   ...
   <button class="v2-bell" aria-label="Notifiche">♧<i></i></button>
 </header>
 <main id="main"></main>
...
function dashboard(){
 ...
 document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   <div class="dash-global-head v8-global-head">
     <div><span class="dash-global-kicker">DASHBOARD</span><small>${meta.label}</small></div>
   ...
   <section class="v5-upcoming">
    ...
    <div class="v5-upcoming-list">
      ${dayMatches.slice(1,4).map(x=>`<button class="v6-upcoming-row" data-match="${x.id}">
        <div class="v6-upcoming-side home">${crest(x.h.code,x.h.cls)}<b>${x.h.name}</b></div>
        <div class="v6-upcoming-center"><strong>${x.time}</strong><small>VS</small></div>
        <div class="v6-upcoming-side away"><b>${x.a.name}</b>${crest(x.a.code,x.a.cls)}</div>
        <em>›</em>
      </button>`).join("") || `<div class="empty-note">Nessun'altra partita disponibile per questa data.</div>`}
    </div>
   </section>
...
function v19League(n){let ts=V19T[n]||[];document.querySelector("#main").innerHTML=`<div class="page v19-ranking"><div class="v19-rhead"><button data-v19home>←</button>${v19Logo((V19L.find(x=>x[0]===n)||[])[1]||"")}<div><small>INTELLIGENCE RANKING</small><h1>${n}</h1></div></div><div class="v19-table"><div class="v19-tr th"><span>#</span><span>SQUADRA</span><span>PG</span><span>ULTIME 5</span><span>RATING</span></div>${ts.map((t,i)=>`<button class="v19-tr" data-v19t="${t[0]}" data-v19league="${n}"><span>${i+1}</span><span class="team">${crest(t[1],"")}<b>${t[2]}</b></span><span>${t[3]}</span><span class="form">${v19Form(t.slice(4,9))}</span><strong>${t[9]}</strong></button>`).join("")}</div></div>`;bind();document.querySelector("[data-v19home]").onclick=v19AnalysisHome;document.querySelectorAll("[data-v19t]").forEach(b=>b.onclick=()=>v19Team(b.dataset.v19t,b.dataset.v19league))}
function v19Team(id,l){let t=(V19T[l]||[]).find(x=>x[0]===id);if(!t)return v19League(l);let vals=V19M[id]||[78,72,76,64,74,70],ms=Object.values(V14_MACRO_META),trend=[t[9]-6,t[9]-4,t[9]-5,t[9]-2,t[9]],rank=vals.map((v,i)=>[v,i]).sort((a,b)=>b[0]-a[0]);document.querySelector("#main").innerHTML=`<div class="page v19-profile"><button class="v19-back" data-v19back>← ${l}</button><header class="v19-hero">${crest(t[1],"")}<div><small>PROFILO SQUADRA</small><h1>${t[2]}</h1><p>MODEL RATING <strong>${t[9]}</strong></p></div></header><section class="v19-card"><div class="v19-chead"><b>MODEL RATING</b><span>${t[9]}</span></div><div class="v19-gauge"><i style="width:${t[9]}%"></i></div><p>Valutazione sintetica corrente del modello SPACE16154.</p></section><section class="v19-card"><div class="v19-chead"><b>6 MACROAREE</b><small>INTELLIGENCE PROFILE</small></div><div class="v19-macros">${ms.map((m,i)=>`<div class="v19-macro"><span>${m.icon}</span><div><b>${m.title}</b><small>${m.desc}</small><em class="${v14Band(vals[i])}"><i style="width:${vals[i]}%"></i></em></div><strong>${vals[i]}</strong></div>`).join("")}</div></section><section class="v19-card"><div class="v19-chead"><b>TREND DEL RATING</b><span class="up">+${trend[4]-trend[0]}</span></div><div class="v19-trend">${trend.map((v,i)=>`<div><span style="height:${Math.max(24,(v-50)*2)}px"></span><b>${v}</b><small>G${i+1}</small></div>`).join("")}</div></section><section class="v19-card"><div class="v19-chead"><b>FORMA RECENTE</b><small>ULTIME 5</small></div><div class="v19-recent">${v19Form(t.slice(4,9))}</div></section><section class="v19-card"><div class="v19-chead"><b>INTELLIGENCE SUMMARY</b></div><div class="v19-summary"><div><small>PUNTI DI FORZA</small><p>${ms[rank[0][1]].icon} <b>${ms[rank[0][1]].title}</b><strong>${rank[0][0]}</strong></p><p>${ms[rank[1][1]].icon} <b>${ms[rank[1][1]].title}</b><strong>${rank[1][0]}</strong></p></div><div><small>DA MONITORARE</small><p>${ms[rank.at(-1)[1]].icon} <b>${ms[rank.at(-1)[1]].title}</b><strong>${rank.at(-1)[0]}</strong></p><p>${ms[rank.at(-2)[1]].icon} <b>${ms[rank.at(-2)[1]].title}</b><strong>${rank.at(-2)[0]}</strong></p></div></div></section><section class="v19-card v19-next"><div class="v19-chead"><b>PROSSIMA PARTITA</b></div><div class="match"><span>${crest(t[1],"")}<b>${t[2]}</b></span><em>VS<small>20:45</small></em><span>${crest("ROM","")}<b>Roma</b></span></div><button data-v19match>ANALIZZA MATCH ›</button></section></div>`;bind();document.querySelector("[data-v19back]").onclick=()=>v19League(l);document.querySelector("[data-v19match]").onclick=()=>route("match","milan-roma")}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=7.3.0-ui-test-v19",{updateViaCache:"none"}).catch(()=>{}));
...
function home(){return `<section class="m13-page"><div class="m13-titlebar"><h1>MATCHES</h1>${cal()}</div><div class="m13-leagues">${leagues.map(l=>`<button class="m13-league" data-league="${l[0]}">${logo(l[2])}<span>${l[1]}</span></button>`).join("")}</div></section>`}
function view(l){return `<section class="m13-page"><div class="m13-leaguehead"><button class="m13-back" data-back>←</button><div class="m13-leagueidentity">${logo(l[2])}<h1>${l[1]}</h1></div>${cal()}</div><div class="m13-fixtures">${(games[l[0]]||[]).map((r,i)=>`<article class="m13-match"><div class="m13-team left">${shield(r[0])}<strong>${r[1]}</strong></div><div class="m13-center"><time>${r[2]}</time><button class="m13-analysis" data-analysis="${r[5]||""}">ANALISI</button></div><div class="m13-team right"><strong>${r[3]}</strong>${shield(r[4],"away")}</div></article>`).join("")}</div></section>`}

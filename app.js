...
const CONFIG={
  version:"10.4.0-ui-test-v54-real-restore",
...
/* ===== V52 REAL-DATA-ONLY SCREENS ===== */
function v52RealOnlyPage(title,subtitle){
 document.querySelector("#main").innerHTML=`<div class="page sm52-real-page"><div class="eyebrow">SPORTMONKS LIVE DATA</div><h1>${title}</h1><p class="subtitle">${subtitle}</p><section class="sm52-real-card"><b>NESSUN DATO DEMO</b><p>Questa area verrà popolata esclusivamente con dati reali SportMonks. Usa <strong>Matches → DATI</strong> per aprire subito la fixture reale disponibile.</p><button class="primary-btn" data-nav="matches">VAI A MATCHES</button></section></div>`;bind();
}
function v19AnalysisHome(){v52RealOnlyPage("ANALYSIS","Classifiche, forma, xG, FPI e tabelle squadra verranno collegate ai prossimi endpoint reali.")}
function signalsPage(){v52RealOnlyPage("TOP SIGNALS","I segnali compariranno solo quando il motore SPACE16154 sarà alimentato e calibrato con dati reali.")}
function performance(){v52RealOnlyPage("PERFORMANCE","Lo storico del modello inizierà dal primo backtest reale. Nessun risultato demo viene più mostrato.")}
function confidencePage(){document.querySelector("#main").innerHTML=`<div class="page sm52-real-page"><button class="back-btn" data-nav="more">‹ ALTRO</button><div class="eyebrow">MODEL CONFIDENCE</div><h1>CONFIDENCE</h1><p class="subtitle">Misura la convergenza del quadro, non la probabilità dell'esito.</p><section class="sm52-real-card"><b>CALIBRAZIONE REALE IN PREPARAZIONE</b><p>La Confidence sarà calcolata separatamente dalla probabilità dell'esito quando avremo collegato le statistiche necessarie e completato il backtesting.</p></section></div>`;bind();}
function more(){document.querySelector("#main").innerHTML=`<div class="page v14-more sm52-more"><div class="eyebrow">SPACE16154</div><h1>ALTRO</h1><p class="subtitle">Solo funzioni e informazioni basate sul progetto reale.</p><button class="more-row2" data-route="confidence"><b>Model Confidence</b><span>Come verrà costruita la Confidence ›</span></button><button class="more-row2" data-info="${encodeURIComponent("Dati SportMonks → normalizzazione → indicatori proprietari → probabilità → Confidence. Pesi e formule definitive verranno calibrati sui dati reali e validati con backtesting.")}"><b>Metodologia</b><span>Come ragionerà SPACE16154 ›</span></button></div>`;bind();}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=10.4.0-ui-test-v54-real-restore",{updateViaCache:"none"}).catch(()=>{}));
...
function smDetailModalHtml(state,pack=null,detail=null,error=''){
  ...
  return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal sm54-modal" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button>
    <div class="sm51-kicker">SPORTMONKS · DATI REALI</div><h2>ANALISI PRE-MATCH</h2>
    ...
    ${smCompareTable(h,hh,a,ah)}
    <div class="sm51-section-title sm54-title">TESTA A TESTA</div>${smH2hHtml(pack)}
    <div class="sm51-section-title sm54-title">DATI DELLA PARTITA</div>${smFixtureStatsHtml(detail)}
    <div class="sm54-note"><b>DATI REALI, ZERO DEMO</b><span>I trattini indicano dati non disponibili. Gli indicatori proprietari e le probabilità SPACE16154 verranno mostrati solo dopo calibrazione e backtesting.</span></div>
  </div></div>`;
}

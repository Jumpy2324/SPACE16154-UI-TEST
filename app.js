...
function v754MacroTone(v){
  if(v==null || !Number.isFinite(Number(v))) return "neutral";
  v=Number(v);
  return v>=80?"good":v>=60?"mid":"bad";
}
function v754MacroCompareRow(meta,hv,av,hName,aName){
  const hc=v754MacroTone(hv), ac=v754MacroTone(av);
  return `<div class="v754-macro-row">
    <span class="v754-macro-icon">${meta.icon}</span>
    <div class="v754-macro-main">
      <div class="v754-macro-title">
        <span>${realEsc(meta.title)}</span>
        <button class="info v754-info" data-info-title="${realEsc(meta.title)}" data-info="${encodeURIComponent(meta.parts)}">i</button>
      </div>
      <div class="v754-macro-compare">
        <div class="v754-side home">
          <div><small>${realEsc(hName)}</small><strong class="${hc}">${hv??"—"}</strong></div>
          <em class="${hc}"><i style="width:${hv??0}%"></i></em>
        </div>
        <div class="v754-side away">
          <div><small>${realEsc(aName)}</small><strong class="${ac}">${av??"—"}</strong></div>
          <em class="${ac}"><i style="width:${av??0}%"></i></em>
        </div>
      </div>
    </div>
  </div>`;
  ...
  try{
    ...
    document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page">
      ...
      <section class="v14-card v14-model">
        ...
        <div class="v754-macro-card">${macros.map(([k])=>{const meta=V14_MACRO_META[k],hv=hm.sample<3?null:hp[k],av=am.sample<3?null:ap[k];return v754MacroCompareRow(meta,hv,av,h.name,a.name)}).join("")}</div>

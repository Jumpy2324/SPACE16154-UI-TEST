...
const CONFIG={
  version:"10.8.0-v58-real-prematch",
...
async function realDashboard(){
 const meta=dashDateMeta();
 document.querySelector("#main").innerHTML=`<div class="page v2-dashboard"><div class="dash-global-head v8-global-head"><div class="v20-dashboard-label"><span class="dash-global-kicker">DASHBOARD</span></div><div class="v8-date-pair"><button class="v8-date-control"><small>MESE</small><strong>${MONTHS_IT[DASH_MONTH].slice(0,3).toUpperCase()}</strong></button><button class="v8-date-control day"><small>GIORNO</small><strong>${String(DASH_DAY).padStart(2,"0")}</strong></button></div></div><section class="v6-hero-combined"><div class="empty-note">Caricamento SportMonks…</div></section></div>`;bind();
 try{const fs=await realFixtures(meta.id),m=fs[0];if(!m){document.querySelector("#main").innerHTML=`<div class="page v2-dashboard"><div class="dash-global-head v8-global-head"><div class="v20-dashboard-label"><span class="dash-global-kicker">DASHBOARD</span></div></div><section class="v6-hero-combined"><div class="empty-note">Nessuna partita dei 5 campionati per ${meta.short}.</div></section></div>`;bind();return}
 document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
 ...
 <section class="v6-hero-combined"><div class="v2-panel-head"><h2>MATCH IN EVIDENZA <span>☆</span></h2><b>${meta.short.toUpperCase()}</b></div><div class="v2-meta">${realEsc(m.leagueName)} <i>•</i> ${meta.short} <i>•</i> ${realEsc(m.time)}</div><div class="v2-match"><div class="v2-club">${realCrest(m.h)}<strong>${realEsc(m.h.name||'CASA').toUpperCase()}</strong></div><div class="v2-center"><b>VS</b><button data-real-match="${m.id}">⌁ &nbsp; Analisi completa</button></div><div class="v2-club">${realCrest(m.a)}<strong>${realEsc(m.a.name||'TRASFERTA').toUpperCase()}</strong></div></div><div class="v6-prob-divider"></div><div class="v6-prob-title">ESITO PIÙ PROBABILE</div><div class="v2-1x2"><div><span>1</span><strong>—</strong><i></i></div><div><span>X</span><strong>—</strong><i></i></div><div><span>2</span><strong>—</strong><i></i></div></div><div class="v2-other-markets"><div class="v2-submarket"><b>GOL / NO GOL</b><div><span><small>GOL</small><strong>—</strong></span><span><small>NO GOL</small><strong>—</strong></span></div></div><div class="v2-submarket"><b>OVER / UNDER 2.5</b><div><span><small>OVER</small><strong>—</strong></span><span><small>UNDER</small><strong>—</strong></span></div></div></div></section>
 <section class="v5-upcoming"><div class="v2-panel-head"><h2>PROSSIME PARTITE</h2><button data-nav="matches">Vedi tutte ›</button></div><div class="v5-upcoming-list">${fs.slice(1,4).map(x=>`<div class="v23-match-card"><div class="v23-time">${realEsc(x.time)}</div><div class="v23-match-row"><div class="v23-team home">${realCrest(x.h)}<b>${realEsc(x.h.name||'')}</b></div><button class="v23-analysis" data-real-match="${x.id}">ANALISI</button><div class="v23-team away"><b>${realEsc(x.a.name||'')}</b>${realCrest(x.a)}</div></div></div>`).join('')||`<div class="empty-note">Nessun'altra partita disponibile.</div>`}</div></section>
 <section class="v2-signals"><div class="v2-panel-head"><h2>TOP SIGNALS</h2><button data-nav="signals">Tutti ›</button></div><div class="empty-note">Segnali in attesa di calibrazione sui dati reali.</div></section></div>`;
 bind();document.querySelectorAll('[data-real-match]').forEach(b=>b.onclick=()=>route('match',b.dataset.realMatch));
 }catch(e){document.querySelector("#main").innerHTML=`<div class="page v2-dashboard"><div class="dash-global-head v8-global-head"><div class="v20-dashboard-label"><span class="dash-global-kicker">DASHBOARD</span></div></div><section class="v6-hero-combined"><div class="empty-note">${realEsc(e.message)}</div></section></div>`;bind()}
}

async function realMatches(){
 const date=dashDateMeta().id;document.querySelector("#main").innerHTML=`<section class="m13-page"><div class="m13-titlebar"><h1 class="v23-matches-title">MATCHES</h1></div><div class="empty-note">Caricamento SportMonks…</div></section>`;
 try{const fs=await realFixtures(date);const leagues=[["serie-a","Serie A","A"],["premier","Premier League","PL"],["laliga","La Liga","LL"],["bundesliga","Bundesliga","BL"],["ligue1","Ligue 1","L1"]];let active=null;const root=document.querySelector('#main');const home=()=>{root.innerHTML=`<section class="m13-page"><div class="m13-titlebar"><h1 class="v23-matches-title">MATCHES</h1><div class="m13-datepair"><button class="m13-datebtn"><small>MESE</small><b>${MONTHS_IT[DASH_MONTH].slice(0,3).toUpperCase()}</b></button><button class="m13-datebtn"><small>GIORNO</small><b>${DASH_DAY}</b></button></div></div><div class="m13-leagues">${leagues.map(l=>`<button class="m13-league" data-rl="${l[0]}"><span class="m13-league-logo">${l[2]}</span><span>${l[1]}</span></button>`).join('')}</div></section>`;root.querySelectorAll('[data-rl]').forEach(b=>b.onclick=()=>view(leagues.find(l=>l[0]===b.dataset.rl)))};const view=l=>{const arr=fs.filter(f=>f.leagueName===l[1]);root.innerHTML=`<section class="m13-page v23-matches-league"><div class="m13-leaguehead v23-leaguehead"><button class="m13-back" data-rback>←</button><div class="m13-leagueidentity"><span class="m13-league-logo">${l[2]}</span><h1>${l[1]}</h1></div></div><div class="m13-fixtures v23-league-cards">${arr.length?arr.map(f=>`<article class="v23-match-card"><div class="v23-time">${realEsc(f.time)}</div><div class="v23-match-row"><div class="v23-team home">${realCrest(f.h)}<b>${realEsc(f.h.name||'')}</b></div><button class="v23-analysis" data-real-match="${f.id}">ANALISI</button><div class="v23-team away"><b>${realEsc(f.a.name||'')}</b>${realCrest(f.a)}</div></div></article>`).join(''):`<div class="empty-note">Nessuna partita reale disponibile.</div>`}</div></section>`;root.querySelector('[data-rback]').onclick=home;root.querySelectorAll('[data-real-match]').forEach(b=>b.onclick=()=>route('match',b.dataset.realMatch))};home();
...
function v58Metrics(fs,t,side=null){
  ...
  const last10=all.slice(0,10), last5=all.slice(0,5);
  ...
  return {
    ...
    pos:realAvgStat(last10,t.id,"pos"),
    shots:realAvgStat(last10,t.id,"shots"),
    sot:realAvgStat(last10,t.id,"sot"),
    corners:realAvgStat(last10,t.id,"corners"),
    attacks:realAvgStat(last10,t.id,"attacks"),
    danger:realAvgStat(last10,t.id,"danger"),
    fouls:realAvgStat(last10,t.id,"fouls"),
    oppShots:v58AvgOppStat(last10,t.id,"shots"),
    oppSot:v58AvgOppStat(last10,t.id,"sot")
  };
}
...
function v58Technical(hm,am){return [["xG",hm.xg,am.xg],["xGA",hm.xga,am.xga],["Gol fatti",hm.gf,am.gf],["Gol subiti",hm.ga,am.ga],["Clean sheet",hm.clean,am.clean],["Possesso",hm.pos,am.pos],["Tiri",hm.shots,am.shots],["Tiri in porta",hm.sot,am.sot],["Corner",hm.corners,am.corners],["Attacchi pericolosi",hm.danger,am.danger],["Attacchi",hm.attacks,am.attacks],["Falli",hm.fouls,am.fouls]]}
async function realMatchDetail(id){
  ...
  try{
    ...
    const hm=v58Metrics(hfs,h),am=v58Metrics(afs,a),hs=v58Metrics(hfs,h,"home"),aspl=v58Metrics(afs,a,"away");
    const hp=v58Macro(hm),ap=v58Macro(am),hscore=v58ModelScore(hp),ascore=v58ModelScore(ap);
    const pred=v58Prediction(hm,am,hs,aspl),confidence=v58Confidence(pred,hm,am);
    ...
    const macros=[["threat","Threat Creation"],["defense","Defense Stability"],["form","Forma"],["opponent","Opponent Strength"],["pressing","Pressing / PPDA"],["availability","Availability"]];
    const tech=v58Technical(hm,am),markets=[["1X2",[["1",pred.one],["X",pred.x],["2",pred.two]]],["GOL / NO GOL",[["GOL",pred.gg],["NO GOL",pred.ng]]],["OVER / UNDER 2.5",[["OVER",pred.over],["UNDER",pred.under]]]];
    const ht=v58PerfTrend(hfs,h.id),at=v58PerfTrend(afs,a.id),diff=(hscore!=null&&ascore!=null)?hscore-ascore:null,winner=diff==null?"—":diff>=0?h.name:a.name;
    document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page">
      <div class="v14-analysis-top"><button class="v14-back" data-nav="matches">←</button><button class="v14-star">☆</button></div>
      <header class="v14-match-head"><h1>${realEsc(h.name)} — ${realEsc(a.name)}</h1><p>${realEsc(fx.league?.name||detail?.league?.name||"")} · ${realEsc(String(fx.starting_at||"").slice(0,10))} · ${realEsc(String(fx.starting_at||"").slice(11,16))}${ref?` · Arbitro: ${realEsc(ref)}`:""}</p><div class="v14-versus"><div>${realCrest(h)}<b>${realEsc(h.name)}</b><small>${standingsH?`${standingsH}° in classifica`:"Classifica —"}</small></div><div class="v14-vs-center"><strong>VS</strong></div><div>${realCrest(a)}<b>${realEsc(a.name)}</b><small>${standingsA?`${standingsA}° in classifica`:"Classifica —"}</small></div></div></header>
      <section class="v14-card"><div class="v14-card-title">PROBABILITÀ DEL MODELLO ${info("Probabilità","Stima preliminare SPACE16154 calcolata dai dati reali SportMonks con modello Poisson e forma recente. Non è una previsione SportMonks.")}</div><div class="v14-markets">${markets.map(([title,vals])=>{const max=Math.max(...vals.map(v=>v[1]));return `<div class="v14-market"><b>${title}</b><div>${vals.map(v=>`<span class="${v[1]===max?"best":""}"><small>${v[0]}</small><strong>${v[1]}%</strong></span>`).join("")}</div></div>`}).join("")}</div><div class="v58-confidence"><span>CONFIDENCE PRELIMINARE ${info("Confidence","Misura solidità e completezza del segnale, separatamente dalla probabilità dell'esito. Verrà calibrata con il backtesting.")}</span><strong>${confidence}%</strong></div></section>
      <section class="v14-card v14-model"><div class="v14-card-title">ANALISI DEL MODELLO</div><div class="v14-score-head"><div class="v14-score-team">${realCrest(h)}<b>${realEsc(h.name)}</b><strong>${hscore??"—"}</strong></div><div class="v14-score-diff"><small>MODEL SCORE</small><strong>${diff==null?"—":(diff>=0?"+":"")+diff}</strong><span>${diff==null?"dati insufficienti":`Vantaggio ${realEsc(winner)}`}</span></div><div class="v14-score-team">${realCrest(a)}<b>${realEsc(a.name)}</b><strong>${ascore??"—"}</strong></div></div><div class="v14-macros">${macros.map(([k])=>{const meta=V14_MACRO_META[k],hv=hp[k],av=ap[k],dv=hv!=null&&av!=null?hv-av:null;return `<div class="v14-macro"><div class="v14-macro-name"><span class="v14-macro-icon">${meta.icon}</span><div><b>${meta.title} <button class="info" data-info="${encodeURIComponent(meta.parts)}">i</button></b><small>${meta.desc}</small></div></div><div class="v14-macro-values"><div><strong>${hv??"—"}</strong><span class="v14-bar ${hv==null?"":v14Band(hv)}"><i style="width:${hv??0}%"></i></span></div><em class="${dv==null?"neu":dv>=0?"pos":"neg"}">${dv==null?"—":(dv>=0?"+":"")+dv}</em><div><strong>${av??"—"}</strong><span class="v14-bar ${av==null?"":v14Band(av)}"><i style="width:${av??0}%"></i></span></div></div></div>`}).join("")}</div><p class="real-note">Macroaree disponibili = elaborazione preliminare SPACE16154 su dati reali. Opponent Strength, Pressing/PPDA e Availability restano — finché non abbiamo una base dati sufficiente e una formula validata.</p></section>
      <section class="panel v58-trend-card"><div class="section-head"><h2>TREND MODEL SCORE</h2><span>ULTIME 5</span></div><div class="trend-grid2"><div class="trend-card2"><div class="trend-card-head"><b>${realEsc(h.name)}</b><strong>${ht.at(-1)??"—"}</strong></div>${v58TrendSvg(ht)}</div><div class="trend-card2"><div class="trend-card-head"><b>${realEsc(a.name)}</b><strong>${at.at(-1)??"—"}</strong></div>${v58TrendSvg(at)}</div></div><div class="trend-note">Trend prestazionale derivato da risultati e xG delle gare recenti.</div></section>
      <div class="v14-accordions">
        <details class="v14-fold" open><summary>DATI TECNICI <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><div class="compare-head"><span>INDICATORE</span><b>${realTeamCode(h)}</b><b>${realTeamCode(a)}</b></div>${tech.map(r=>`<div class="compare-row"><span>${r[0]}</span><b>${v58FmtMetric(r[0],r[1])}</b><b>${v58FmtMetric(r[0],r[2])}</b></div>`).join("")}<p class="real-note">Medie sulle gare disponibili prima del match. Casa/trasferta viene usato nel motore probabilistico.</p></div></details>
        <details class="v14-fold"><summary>FORMA RECENTE <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><div class="compare-row"><span>${realEsc(h.name)}</span><b>${realFormDots(hm.form)}</b><b>${hm.sample} gare</b></div><div class="compare-row"><span>${realEsc(a.name)}</span><b>${realFormDots(am.form)}</b><b>${am.sample} gare</b></div></div></details>
        <details class="v14-fold"><summary>TESTA A TESTA <span class="v18-toggle">+</span></summary><div class="v14-fold-body">${h2.length?h2.map(f=>{const hh=realLoc(f,"home")||{},aa=realLoc(f,"away")||{};return `<div class="compare-row"><span>${realEsc(String(f.starting_at||"").slice(0,10))}</span><b>${realEsc(hh.name||"")} ${realScore(f,hh.id)??"—"}–${realScore(f,aa.id)??"—"} ${realEsc(aa.name||"")}</b><b></b></div>`}).join(""):`<div class="empty-note">Nessun H2H disponibile.</div>`}</div></details>
        <details class="v14-fold"><summary>PROBABILI FORMAZIONI <span class="v18-toggle">+</span></summary><div class="v14-fold-body v14-lineups">${v58LineupHtml(ctx,h)}${v58LineupHtml(ctx,a)}</div></details>
        <details class="v14-fold"><summary>ASSENZE <span class="v18-toggle">+</span></summary><div class="v14-fold-body">${v58SidelinedHtml(ctx,h,a)}</div></details>
        <details class="v14-fold"><summary>ARBITRO <span class="v18-toggle">+</span></summary><div class="v14-fold-body">${ref?`<div class="v14-ref-grid"><div><small>Arbitro</small><b>${realEsc(ref)}</b></div></div>`:`<div class="empty-note">Non ancora disponibile.</div>`}</div></details>
      </div>
    </div>`;bind();
  }catch(e){document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page"><button class="back-btn" data-nav="matches">‹ MATCHES</button><section class="v14-card"><div class="empty-note">${realEsc(e.message)}</div></section></div>`;bind()}
}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=10.8.0-v58-prematch",{updateViaCache:"none"}).catch(()=>{}));

...
async function realMatchDetail(id){
  ...
  try{
    ...
    // V77.7 — MATCH FORM must use the same REAL league history used by Team Detail.
    // Prematch history remains dedicated to frozen predictions; it must not drive
    // the visible post-match "Forma" strip because that pack can contain a
    // different/stale 5-match window.
    const v777ActualLeagueForm=(team,rawHistory)=>{
      const byId=new Map();
      [...(Array.isArray(rawHistory)?rawHistory:[]),currentFt].forEach(f=>{
        if(f?.id!=null) byId.set(String(f.id),f);
      });
      const latest=v76LeagueHistory([...byId.values()],analysisLeagueId,5);
      return realMetrics(latest,team).form;
    };
    const actualHForm=v777ActualLeagueForm(h,allHHistory);
    const actualAForm=v777ActualLeagueForm(a,allAHistory);
    const hForm=actualHForm.length?actualHForm:v7522RollForm(h,a,preHm,hmRaw);
    const aForm=actualAForm.length?actualAForm:v7522RollForm(a,h,preAm,amRaw);
    ...
    document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page">
      ...
      <div class="v14-accordions">
        ${v67FixtureState(detail)==="ft"?"":v722TechCompare(tech,h,a)}

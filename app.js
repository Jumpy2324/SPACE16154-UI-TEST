...
const CONFIG={
  version:"11.6.0-v66-live-polish",
...
async function v60StandingsPage(initialKey="la-liga"){
 ...
 try{
  ...
  const load=async key=>{
   ...
   ${merged.map((x,i)=>{const r=x.row||{},gf=v60StandingValue(r,"gf"),ga=v60StandingValue(r,"ga");return `<div class="v60-standing-row v65-standing-grid"><div class="v65-standing-team"><span class="v65-pos">${r?.position??i+1}</span>${realCrest(x.team)}<b>${realEsc(x.team?.name||"—")}</b></div><span>${v60StandingValue(r,"played")}</span><span>${v60StandingValue(r,"won")}</span><span>${v60StandingValue(r,"draw")}</span><span>${v60StandingValue(r,"lost")}</span><span>${gf}</span><span>${ga}</span><span>${gf-ga}</span><strong>${v60StandingValue(r,"pts")}</strong></div>`}).join("")}
   ${merged.length?`<div class="v60-standing-foot ${b.live?"is-live":""}"><b>${b.live?'<i class="v66-live-dot"></i> CLASSIFICA LIVE':"AGGIORNAMENTO AUTOMATICO"}</b> · ${realEsc(b.season?.name||"")}<br><span>${b.live?"Include i risultati parziali delle gare in corso.":"Classifica ufficiale corrente."}</span></div>`:`<div class="empty-note v62-standing-empty">Classifica non ricevuta da SportMonks.</div>`}`;
  };
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=11.6.0-v66-live-polish",{updateViaCache:"none"}).catch(()=>{}));

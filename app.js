...
const V25_MACROS = {
  ...
  "Opponent Strength": {
    ...
    groups:[
      ...
      ["DIFFICOLTÀ CALENDARIO","Valuta la difficoltà complessiva del calendario recente considerando forza degli avversari, trasferte e impegni aggiuntivi. Serve a interpretare correttamente risultati e statistiche ottenuti in periodi più o meno complessi.",[["SOS INDEX","61","","-5"],["TRASFERTE","2","ULTIME 5",""],["MATCH EUROPEI","1","ULTIME 5",""]],61],
      ["CONTESTO RISULTATI","Confronta il rendimento della squadra contro fasce diverse di avversari. Aiuta a capire se i risultati restano solidi anche quando aumenta il livello della competizione oppure se dipendono soprattutto dalle gare più favorevoli.",[["PTS vs TOP","1.3","PER GARA","+0.1"],["PTS vs MID","2.1","PER GARA","+0.3"],["PTS vs LOW","2.7","PER GARA","+0.2"]],66]
    ]
  },
  "Pressing / PPDA": {
    ...
    groups:[
      ...
      ["EFFICIENZA","Valuta quanto il lavoro di pressione si trasforma realmente in vantaggio. Considera la percentuale di pressioni riuscite, i turnover ottenuti in zone avanzate e le conclusioni create subito dopo un recupero.",[["SUCCESS RATE","34%","","+4%"],["TURNOVER ALTI","6.1","PER 90 MIN","+0.8"],["SHOT AFTER PRESS","2.3","PER 90 MIN","+0.4"]],82],
      ["CONTINUITÀ","Misura la continuità del pressing durante la partita. Confronta l'intensità tra primo e secondo tempo e la sua variabilità: un buon valore indica che la squadra riesce a mantenere il proprio comportamento senza cali marcati.",[["1° TEMPO","87","","+5"],["2° TEMPO","81","","+3"],["VARIANZA","7","","-2"]],84]
    ]
  },
  "Availability": {
    ...
    groups:[
      ...
      ["QUALITÀ SOSTITUTI","Valuta la profondità della rosa e quanto le alternative riescono a limitare il calo di rendimento quando manca un titolare. Un buon rating indica sostituti affidabili e un impatto contenuto delle assenze.",[["DEPTH SCORE","84","","+4"],["BENCH IMPACT","79","","+6"],["DROP-OFF","6","","-3"]],84],
 ...
 document.querySelector("#main").innerHTML=`<div class="page v25-macro-page">
   ...
   <div class="v25-groups">
     ${cfg.groups.map((g,gi)=>`<section class="v25-group">
       ...
       <div class="v25-metrics">${g[2].map(m=>`<div><small>${m[0]}</small><strong>${m[1]}</strong>${m[2]?`<em>${m[2]}</em>`:""}<i class="${m[3].startsWith("-")?"down":"up"}">${m[3]}</i><u>VS MEDIA CAMPIONATO</u></div>`).join("")}</div>
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=7.9.0-ui-test-v26",{updateViaCache:"none"}).catch(()=>{}));
...
/* ===== V26 NATIVE BACK NAVIGATION ===== */
(function(){
  let restoring=false, lastHTML="", lastScroll=0, ready=false;
  function main(){ return document.querySelector("#main"); }
  function snapshot(){
    const m=main(); if(!m) return null;
    return {space16154:true, html:m.innerHTML, scroll:window.scrollY||0};
  }
  function install(){
    const m=main(); if(!m || ready) return;
    ready=true;
    lastHTML=m.innerHTML;
    history.replaceState(snapshot(),"",location.href);
    const obs=new MutationObserver(()=>{
      if(restoring) return;
      const cur=m.innerHTML;
      if(!cur || cur===lastHTML) return;
      const previous={space16154:true,html:lastHTML,scroll:lastScroll};
      history.replaceState(previous,"",location.href);
      lastHTML=cur;
      lastScroll=0;
      history.pushState({space16154:true,html:cur,scroll:0},"",location.href);
    });
    obs.observe(m,{childList:true,subtree:false});
    window.addEventListener("scroll",()=>{ lastScroll=window.scrollY||0; },{passive:true});
    window.addEventListener("popstate",e=>{
      if(!e.state || !e.state.space16154) return;
      const mm=main(); if(!mm) return;
      restoring=true;
      mm.innerHTML=e.state.html||"";
      lastHTML=mm.innerHTML;

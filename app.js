...
function realRecent(fs,id){return fs.slice().sort((a,b)=>String(b?.starting_at||"").localeCompare(String(a?.starting_at||""))).slice(0,5).reverse().map(f=>{const o=realOpponent(f,id),gf=realScore(f,id),ga=o?realScore(f,o.id):null;return {gf,ga,r:gf==null||ga==null?"—":gf>ga?"W":gf<ga?"L":"D"}})}
...
async function v67RealTeamProfile(payload){
  ...
  try{
    ...
    const history=v76LeagueHistory(mergedHistory,leagueId,15);

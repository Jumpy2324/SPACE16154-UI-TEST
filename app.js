 ...
 document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   ...
   <section class="v5-upcoming">
    ...
    <div class="v5-upcoming-list">
      ${dayMatches.slice(1,4).map(x=>`<div class="v22-match-card">
        <div class="v22-match-time">${x.time}</div>
        <div class="v22-match-line">
          <div class="v22-team home">${crest(x.h.code,x.h.cls)}<b>${x.h.name}</b></div>
          <button class="v22-analysis-btn" data-match="${x.id}">ANALISI</button>
          <div class="v22-team away"><b>${x.a.name}</b>${crest(x.a.code,x.a.cls)}</div>
        </div>
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=7.5.0-ui-test-v22",{updateViaCache:"none"}).catch(()=>{}));
...
function view(l){return `<section class="m13-page v22-matches-league">
  <div class="v22-calendar-top">${cal()}</div>
  <div class="m13-leaguehead v22-leaguehead"><button class="m13-back" data-back>←</button><div class="m13-leagueidentity">${logo(l[2])}<h1>${l[1]}</h1></div></div>
  <div class="m13-fixtures v22-league-fixtures">${(games[l[0]]||[]).map((r,i)=>`<article class="v22-match-card">
    <div class="v22-match-time">${r[2]}</div>
    <div class="v22-match-line">
      <div class="v22-team home">${shield(r[0])}<b>${r[1]}</b></div>
      <button class="v22-analysis-btn" data-analysis="${r[5]||""}">ANALISI</button>
      <div class="v22-team away"><b>${r[3]}</b>${shield(r[4],"away")}</div>
    </div>

...
const DASH_DATES = [
  {id:"2026-08-11", label:"Oggi", short:"11 Ago", offset:0},
  {id:"2026-08-12", label:"Domani", short:"12 Ago", offset:1},
  {id:"2026-08-13", label:"Gio 13", short:"13 Ago", offset:2},
  {id:"2026-08-14", label:"Ven 14", short:"14 Ago", offset:3},
  {id:"2026-08-15", label:"Sab 15", short:"15 Ago", offset:4}
];
let DASH_DATE = DASH_DATES[0].id;

function dashDateMeta(){
  return DASH_DATES.find(d=>d.id===DASH_DATE) || DASH_DATES[0];
}
function matchesForSelectedDate(){
  const meta=dashDateMeta();
  // Demo: distribuiamo le partite sui giorni in modo deterministico.
  // Con SportMonks verrà sostituito dal fixture.date reale.
  const selected=MATCHES.filter((m,i)=>i%5===meta.offset%5 || (meta.offset===0 && i<3));
  return selected.length ? selected : MATCHES.slice(meta.offset%MATCHES.length, (meta.offset%MATCHES.length)+2);
}
function renderDateMenu(){
  const old=document.querySelector(".dash-date-popover");
  if(old){old.remove();return}
  const menu=document.createElement("div");
  menu.className="dash-date-popover";
  menu.innerHTML=`<div class="dash-date-title">SELEZIONA DATA</div>${DASH_DATES.map(d=>`<button class="${d.id===DASH_DATE?"active":""}" data-dash-date="${d.id}"><span>${d.label}</span><small>${d.short}</small></button>`).join("")}`;
  document.body.appendChild(menu);
  menu.querySelectorAll("[data-dash-date]").forEach(b=>b.onclick=()=>{
    DASH_DATE=b.dataset.dashDate;
    menu.remove();
    route("dashboard");
  });
  setTimeout(()=>document.addEventListener("click",function close(e){
    if(!menu.contains(e.target) && !e.target.closest(".dash-date-btn")){menu.remove();document.removeEventListener("click",close)}
  }),0);
}
...
function dashboard(){
 ...
 document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   <div class="dash-global-head">
     <div><span class="dash-global-kicker">DASHBOARD</span><small>${meta.label} · ${meta.short}</small></div>
     <button class="dash-date-btn" type="button">▣ <span>${meta.label}</span>⌄</button>
   </div>

   <section class="v2-featured">
    <div class="v2-panel-head"><h2>MATCH IN EVIDENZA <span>☆</span></h2><b>${meta.label.toUpperCase()}</b></div>
   ...
   <section class="v5-upcoming">
    ...
    <div class="v5-upcoming-list">
      ${dayMatches.slice(1,4).map(x=>`<button class="v5-upcoming-row" data-match="${x.id}">
        <span class="v5-upcoming-time">${x.time}</span>
        <div class="v5-upcoming-team">${crest(x.h.code,x.h.cls)}<b>${x.h.name}</b></div>
        <em>VS</em>
        <div class="v5-upcoming-team away"><b>${x.a.name}</b>${crest(x.a.code,x.a.cls)}</div>
        <strong>›</strong>
      </button>`).join("") || `<div class="empty-note">Nessun'altra partita disponibile per questa data.</div>`}
    </div>
   </section>

   <section class="v2-prob">
    <div class="v2-panel-head"><h2>ESITO PIÙ PROBABILE</h2></div>
   ...
   <section class="v2-overview">
    <div class="v2-panel-head"><h2>PANORAMICA ${meta.label.toUpperCase()}</h2></div>
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=5.4.0-ui-test-v5",{updateViaCache:"none"}).catch(()=>{}));

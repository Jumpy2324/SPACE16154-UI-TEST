...
async function realMatchDetail(id){
  ...
  try{
    ...
    const v7519FormSeq=(team,opp,preHistory)=>{
      // PREVIOUS FOUR ONLY: explicitly calculate the four most recent finished
      // fixtures BEFORE the current match, then append the current FT result.
      const currentId=String(detail?.id||fx?.id||"");
      const previousFour=v58TeamFixtures(preHistory,team.id)
        .filter(f=>String(f?.id||"")!==currentId)
        .slice(0,4);
      const previousSeq=realRecent(previousFour,team.id);
      if(!v60FixtureFinished(detail)) return previousSeq;

      const gf=realScore(detail,team.id),ga=realScore(detail,opp.id);
      if(gf==null||ga==null) return previousSeq;

      const currentResult={gf,ga,r:gf>ga?"W":gf<ga?"L":"D"};
      return [...previousSeq,currentResult].slice(-5);
    };
    const hForm=v7519FormSeq(h,a,preHfs),aForm=v7519FormSeq(a,h,preAfs);

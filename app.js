...
const V25_MACROS = {
  "Threat Creation": {
    ...
    groups:[
      ["PRODUZIONE OFFENSIVA","Quantità di occasioni e tiri generati.",[["xG","1.89","PER 90 MIN","+0.21"],["TIRI","14.6","PER 90 MIN","+1.8"],["TIRI IN PORTA","5.1","PER 90 MIN","+0.6"]],84],
      ["QUALITÀ DELLE OCCASIONI","Pericolosità media delle occasioni create.",[["xG PER TIRO","0.13","","+0.01"],["BIG CHANCES","2.7","PER 90 MIN","+0.4"],["xG DA AZIONE","1.45","PER 90 MIN","+0.16"]],79],
      ["EFFICACIA","Capacità di trasformare le occasioni in gol.",[["GOL / xG","1.12","","+0.10"],["CONVERSIONE %","15.0%","","+1.6%"],["GOL","2.1","PER 90 MIN","+0.3"]],81]
    ]
  },
  "Defense Stability": {
    icon:"🛡️", desc:"Solidità difensiva e capacità di limitare il pericolo concesso.",
    groups:[
      ["PROTEZIONE AREA","Quanto bene la squadra limita occasioni pulite.",[["xGA","0.98","PER 90 MIN","-0.08"],["TIRI CONCESSI","9.1","PER 90 MIN","-1.2"],["TIRI IN PORTA CONCESSI","3.0","PER 90 MIN","-0.4"]],76],
      ["QUALITÀ CONCESSA","Pericolosità media delle occasioni subite.",[["xGA PER TIRO","0.10","","-0.01"],["BIG CHANCES CONCESSE","1.3","PER 90 MIN","-0.2"],["GOL CONCESSI","0.9","PER 90 MIN","-0.2"]],73],
      ["RESILIENZA","Capacità di proteggere il risultato sotto pressione.",[["CLEAN SHEET","42%","","+5%"],["STOP RATE","71%","","+4%"],["ERRORI GRAVI","0.18","PER 90 MIN","-0.05"]],72]
    ]
  },
  "Forma": {
    icon:"↗", desc:"Rendimento recente pesato per la forza degli avversari.",
    groups:[
      ["RISULTATI RECENTI","Andamento delle ultime gare ufficiali.",[["PUNTI/GARA","2.20","ULTIME 5","+0.40"],["GOL FATTI","2.0","PER GARA","+0.4"],["GOL SUBITI","0.8","PER GARA","-0.3"]],83],
      ["QUALITÀ PERFORMANCE","Prestazioni sottostanti rispetto ai risultati.",[["xG DIFF","0.72","PER GARA","+0.18"],["FIELD TILT","58%","","+6%"],["SHOT DIFF","+5.2","PER GARA","+1.4"]],81],
      ["PESO AVVERSARI","Contesto competitivo delle ultime partite.",[["RATING MEDIO AVV.","72","","+3"],["TRASFERTA","3","ULTIME 5",""],["TOP-6 AFFRONTATE","2","ULTIME 5",""]],78]
    ]
  },
  "Opponent Strength": {
    icon:"🎯", desc:"Livello medio degli avversari affrontati recentemente.",
    groups:[
      ["FORZA MEDIA","Qualità complessiva degli avversari recenti.",[["RATING MEDIO","68","","-4"],["POSIZIONE MEDIA","10.4","","+1.8"],["TOP-6","1","ULTIME 5","-1"]],64],
      ["DIFFICOLTÀ CALENDARIO","Quanto è impegnativo il campione recente.",[["SOS INDEX","61","","-5"],["TRASFERTE","2","ULTIME 5",""],["MATCH EUROPEI","1","ULTIME 5",""]],61],
      ["CONTESTO RISULTATI","Quanto i risultati dipendono dalla qualità avversaria.",[["PTS vs TOP","1.3","PER GARA","+0.1"],["PTS vs MID","2.1","PER GARA","+0.3"],["PTS vs LOW","2.7","PER GARA","+0.2"]],66]
    ]
  },
  "Pressing / PPDA": {
    icon:"🏃", desc:"Intensità della pressione e capacità di recuperare alto.",
    groups:[
      ["INTENSITÀ PRESSING","Quanto rapidamente la squadra prova a riconquistare palla.",[["PPDA","8.0","","-0.8"],["RECUPERI ALTI","9.4","PER 90 MIN","+1.1"],["PRESSIONI","151","PER 90 MIN","+12"]],86],
      ["EFFICIENZA","Quanto il pressing produce recuperi utili.",[["SUCCESS RATE","34%","","+4%"],["TURNOVER ALTI","6.1","PER 90 MIN","+0.8"],["SHOT AFTER PRESS","2.3","PER 90 MIN","+0.4"]],82],
      ["CONTINUITÀ","Quanto il pressing resta stabile nelle diverse fasi.",[["1° TEMPO","87","","+5"],["2° TEMPO","81","","+3"],["VARIANZA","7","","-2"]],84]
    ]
  },
  "Availability": {
    icon:"👥", desc:"Impatto di assenze e disponibilità della rosa.",
    groups:[
      ["DISPONIBILITÀ ROSA","Percentuale di giocatori chiave disponibili.",[["TITOLARI DISP.","91%","","+3%"],["MINUTI PERSI","248","ULTIME 5","-62"],["ASSENTI CHIAVE","1","","-1"]],90],
      ["QUALITÀ SOSTITUTI","Capacità di coprire le assenze con alternative affidabili.",[["DEPTH SCORE","84","","+4"],["BENCH IMPACT","79","","+6"],["DROP-OFF","6","","-3"]],84],
      ["STABILITÀ XI","Continuità della formazione titolare.",[["XI CONFERMATO","8.4","MEDIA","+0.8"],["CAMBI/GARA","2.6","","-0.5"],["MINUTI CORE","86%","","+4%"]],88]
    ]
  }
};
 ...
 document.querySelector("#main").innerHTML=`<div class="page v25-macro-page">
   ...
   <div class="v25-groups">
     ${cfg.groups.map((g,gi)=>`<section class="v25-group">
       ...
       <div class="v25-metrics">${g[2].map(m=>`<div><small>${m[0]}</small><strong>${m[1]}</strong>${m[2]?`<em>${m[2]}</em>`:""}<i class="${m[3].startsWith("-")?"down":"up"}">${m[3]}</i></div>`).join("")}</div>
   ...
   <details class="v25-params">
     <summary><span>ⓘ</span><b>TUTTI I PARAMETRI UTILIZZATI</b><em class="v18-toggle">+</em></summary>
     <div>${cfg.groups.map(g=>`<p><strong>${g[0]}</strong>${g[2].map(m=>m[0]).join(" · ")}</p>`).join("")}</div>
   </details>
 </div>`;
 bind();
 document.querySelector("[data-v25-back]").onclick=()=>v19Team(teamId,league);
}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=7.8.0-ui-test-v25",{updateViaCache:"none"}).catch(()=>{}));

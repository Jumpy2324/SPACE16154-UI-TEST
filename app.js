...
function v722OfficialLineups(ctx,h,a){
  const xs=Array.isArray(ctx?.lineups)?ctx.lineups:[];
  const confirmed=t=>{
    const rows=xs.filter(x=>String(x?.team_id??x?.participant_id)===String(t.id));
  ...
  return confirmed(h)&&confirmed(a);
}
...
function v758LineupTitle(ctx,h,a,detail){
  if(v59HasOfficialLineups(ctx,h,a)) return "FORMAZIONI";
  ...
  if((state==="live"||state==="ft") && homeStarters>=7 && awayStarters>=7) return "FORMAZIONI";
  return (v7714LineupsOfficial(fx,homeLineups,awayLineups)?"FORMAZIONI":"PROBABILI FORMAZIONI");
}

...
const V27_METRIC_DIRECTION={
 ...
 "RATING MEDIO":"neutral","POSIZIONE MEDIA":"neutral","TOP-6":"neutral","SOS INDEX":"neutral","RATING AVV. RECENTE":"neutral","RATING AVV. MENO RECENTE":"neutral",
...
function v33RatingTrend(teamId,league){
 const t=V19_TEAMS[teamId]||V19_TEAMS.MIL, name=t[2], score=t[3];
 const vals=[76,80,82,85,88], dates=["20 LUG","24 LUG","31 LUG","4 AGO","11 AGO"];
 const selected=3, delta=3;
 const factors=[
  ["⚡","Threat Creation","+2.1","up",86],
  ["🛡","Defense Stability","+1.4","up",74],
  ["↗","Forma","+1.2","up",68],
  ["🎯","Opponent Strength","+0.6","neutral",39],
  ["⌁","Pressing / PPDA","+0.4","up",52],
  ["♟","Availability","-0.7","down",28]
 ];
 const before=[["Threat Creation",78,84,6],["Defense Stability",76,81,5],["Forma",74,80,6],["Opponent Strength",70,72,2],["Pressing / PPDA",77,81,4],["Availability",76,73,-3]];
 v27History({kind:"rating-trend",team:teamId,league});
 const pts=vals.map((v,i)=>({x:10+i*21,y:82-(v-72)*3.0,v}));
 const seg=pts.slice(0,-1).map((p,i)=>{
   const q=pts[i+1], cls=q.v>p.v?"up":q.v<p.v?"down":"flat";
   return `<line class="${cls}" x1="${p.x}" y1="${p.y}" x2="${q.x}" y2="${q.y}"/>`;
 }).join("");
 const circles=pts.map((p,i)=>`<g class="${i===selected?"sel":""}"><circle cx="${p.x}" cy="${p.y}" r="${i===selected?3.2:2.3}"/><text x="${p.x}" y="${p.y-7}">${p.v}</text><text class="date" x="${p.x}" y="96">${dates[i]}</text></g>`).join("");
 document.querySelector("#main").innerHTML=`<div class="v19-page v33-page">
   <button class="v19-back" data-v33-back>← ${league}</button>
   <header class="v33-team-head">${crest(t[1],"")}<div><h1>${name}</h1><small>${league}</small></div><div class="v33-current"><small>MODEL RATING</small><b>${score}</b><strong>+6</strong></div></header>
   <section class="v19-card v33-chart-card">
    <div class="v19-chead"><b>MODEL RATING TREND</b><span>ULTIME VALUTAZIONI</span></div>
    <p class="v33-sub">L'evoluzione del Model Rating nel tempo</p>
    <div class="v33-key"><span>● Partita</span><span>◆ Aggiornamento modello</span></div>
    <svg class="v33-chart" viewBox="0 0 100 104" preserveAspectRatio="none">
      <g class="grid"><line x1="8" y1="22" x2="96" y2="22"/><line x1="8" y1="42" x2="96" y2="42"/><line x1="8" y1="62" x2="96" y2="62"/><line x1="8" y1="82" x2="96" y2="82"/></g>
      <g class="segments">${seg}</g><g class="points">${circles}</g>
    </svg>
    <div class="v33-event"><small>4 AGOSTO · PARTITA</small><b>Milan 2–0 Roma</b><span>Rating <strong>82</strong> <em>+3</em></span></div>
   </section>
   <section class="v19-card v33-why">
    <div class="v19-chead"><b>PERCHÉ È CAMBIATO IL RATING</b></div>
    <div class="v33-factors">${factors.map(f=>`<div><span class="ico">${f[0]}</span><b>${f[1]}</b><i><u class="${f[3]}" style="width:${f[4]}%"></u></i><strong class="${f[3]}">${f[2]}</strong></div>`).join("")}</div>
    <p class="v33-explain">Il Model Rating è aumentato soprattutto grazie alla crescita nella produzione offensiva e alla solidità difensiva. Il livello dell'avversario ha inciso positivamente, mentre la disponibilità della rosa ha avuto un impatto negativo.</p>
   </section>
   <section class="v19-card v33-compare">
    <div class="v19-chead"><b>CONFRONTO PRIMA / DOPO</b></div>
    <div class="v33-score-change"><div><small>PRIMA</small><b>79</b></div><span>→</span><div><small>DOPO</small><b>82</b><em>+3</em></div></div>
    <div class="v33-before">${before.map(x=>`<div><b>${x[0]}</b><span>${x[1]} <i>→</i> ${x[2]}</span><strong class="${x[3]>=0?"up":"down"}">${x[3]>=0?"+":""}${x[3]}</strong></div>`).join("")}</div>
   </section>
   <section class="v19-card v33-history">
    <div class="v19-chead"><b>ULTIME VARIAZIONI</b></div>
    ${[["10 AGO","Aggiornamento modello","+1","up"],["7 AGO","Milan 1–0 Torino","+2","up"],["4 AGO","Milan 2–0 Roma","+3","up"],["31 LUG","Milan 3–1 Bologna","+3","up"],["28 LUG","Aggiornamento modello","-1","down"]].map(x=>`<div><i class="${x[3]}"></i><small>${x[0]}</small><b>${x[1]}</b><strong class="${x[3]}">${x[2]}</strong></div>`).join("")}
   </section>
 </div>`;
 bind();
 document.querySelector("[data-v33-back]").onclick=()=>history.length>1?history.back():v19Team(teamId,league);
}
...
const V29_TEAM_STATS = {
  generale: [
    ...
    ["DINAMICHE COLLETTIVE","Intensità, stabilità e gestione delle assenze",[["INTENSITÀ STAMPA","72.6","+8.4","higher"],["STABILITÀ PRESSING","84.2","+6.7","higher"],["CALO SENZA TITOLARI","-7.8%","-3.1%","lower"]]]
  ],
  attacco: [
...
function v19Team(id,l){
 ...
 document.querySelector("#main").innerHTML=`<div class="page v19-profile v20-profile">
   ...
   <button class="v19-card v20-trend-card v33-trend-entry" data-v33-trend>
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=8.5.0-ui-test-v32",{updateViaCache:"none"}).catch(()=>{}));

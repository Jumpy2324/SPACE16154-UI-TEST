...
function v19League(n){
 let ts=V19T[n]||[];
 document.querySelector("#main").innerHTML=`<div class="page v19-ranking v20-ranking">
   <div class="v19-rhead v20-rhead"><button data-v19home>←</button>${v19Logo((V19L.find(x=>x[0]===n)||[])[1]||"")}<div><small>INTELLIGENCE RANKING</small><h1>${n}</h1></div></div>
   <div class="v20-rank-list">
     <div class="v20-rank-labels"><span># / SQUADRA</span><span>PG</span><span>ULTIME 5</span><span>RATING</span><span></span></div>
     ${ts.map((t,i)=>{
       const vals=t.slice(4,9), trend=vals.filter(x=>x==="V").length-vals.filter(x=>x==="P").length;
       const tc=trend>0?"":trend<0?"down":"flat", ti=trend>0?"↗":trend<0?"↘":"→";
       return `<button class="v20-rank-row" data-v19t="${t[0]}" data-v19league="${n}">
         <span class="v20-pos">${i+1}</span>
         <span class="v20-team">${crest(t[1],"")}<span><b>${t[2]}</b><small>${t[1]}</small></span></span>
         <span class="v20-pg">${t[3]}</span>
         <span class="v20-form">${v19Form(vals)}</span>
         <span class="v20-rating ${v20RatingClass(t[9])}"><strong>${t[9]}</strong><i><em style="width:${t[9]}%"></em></i></span>
         <span class="v21-rank-trend ${tc}">${ti}</span>
       </button>`;
     }).join("")}
   </div>
 </div>`;
...
function v19Team(id,l){
 ...
 document.querySelector("#main").innerHTML=`<div class="page v19-profile v20-profile">
   ...
   <header class="v19-hero v20-hero">
     ${crest(t[1],"")}
     <div><small>PROFILO SQUADRA</small><h1>${t[2]}</h1>
       <div class="v20-hero-rating"><span>MODEL RATING</span><strong>${t[9]}</strong><i class="${v20RatingClass(t[9])}"></i><small class="v21-hero-trend ${trend[4]>trend[0]?"":trend[4]<trend[0]?"down":"flat"}">${trend[4]>trend[0]?"↗":trend[4]<trend[0]?"↘":"→"} ${trend[4]-trend[0]>=0?"+":""}${trend[4]-trend[0]} ultime 5 valutazioni</small></div>
     </div>
   </header>
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=7.5.0-ui-test-v21",{updateViaCache:"none"}).catch(()=>{}));

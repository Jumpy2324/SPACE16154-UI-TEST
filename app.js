...
function showInfo(t,title="INFO"){let o=document.createElement("div");o.className="modal-backdrop";o.innerHTML=`<div class="info-modal v74-info-modal"><button class="modal-x">×</button><div class="eyebrow">${realEsc(title)}</div><h3>COSA MISURA</h3><p>${realEsc(t)}</p><div class="v74-info-foot"><small>DATI CONSIDERATI</small><span>${realEsc(t)}</span></div></div>`;document.body.appendChild(o);o.querySelector(".modal-x").onclick=()=>o.remove();o.onclick=e=>{if(e.target===o)o.remove()}}
...
async function v67RealTeamProfile(payload){
  ...
  try{
    ...
    const recent=v59FormSeq(hist||history||[],team).slice(0,5);
    ...
    root.innerHTML=`<div class="page v67-team-page">
      ...
      <header class="v67-team-head">${realCrest(team)}<div><small>${realEsc(comp.name||"")}</small><h1>${realEsc(team.name||"Squadra")}</h1><span>${row?.position?`${row.position}° in classifica`:"Classifica —"}</span></div></header>
...
async function realMatches(){
 ...
 try{
  ...
  let statusFilter="all", leagueFilter=leagues[0]?.key||"all";
  ...
  const render=()=>{
    ...
    root.querySelectorAll("[data-v716-league]").forEach(btn=>btn.onclick=(ev)=>{
      ev.preventDefault();
      ev.stopPropagation();
      leagueFilter=btn.dataset.v716League||"all";
      closeSheet();
...
function v741MacroInfo(name){
  const key=String(name||"").toLowerCase();
  const map={
    "threat creation":{
      title:"Threat Creation",
      measure:"Misura quanto una squadra riesce a creare pericoli di qualità e con continuità.",
      data:["xG","tiri","tiri in porta","presenza ultimo terzo","volume offensivo"]
    },
    "defense stability":{
      title:"Defense Stability",
      measure:"Misura quanto una squadra limita occasioni, tiri pericolosi e qualità concessa agli avversari.",
      data:["xGA","tiri concessi","tiri in porta concessi","qualità occasioni concesse","solidità difensiva"]
    },
    "forma":{
      title:"Forma",
      measure:"Valuta il rendimento recente, pesandolo per la qualità degli avversari affrontati.",
      data:["ultime 5","risultati","gol fatti/subiti","forza avversari","continuità"]
    },
    "opponent strength":{
      title:"Opponent Strength",
      measure:"Stima il livello medio degli avversari affrontati di recente, così da dare il giusto peso ai risultati.",
      data:["classifica avversari","rating avversari","forma avversari","casa/trasferta","livello campionato"]
    },
    "pressing / ppda":{
      title:"Pressing / PPDA",
      measure:"Misura l'intensità della pressione e la capacità di recuperare il pallone in zone avanzate.",
      data:["PPDA","recuperi alti","pressione offensiva","possesso avversario","azioni difensive alte"]
    },
    "availability":{
      title:"Availability",
      measure:"Misura quanto la disponibilità della rosa può incidere sul rendimento atteso della squadra.",
      data:["assenze","titolari indisponibili","peso giocatori chiave","rotazioni","profondità rosa"]
    }
  };
  return map[key]||{title:name||"Info",measure:"Descrizione non disponibile.",data:[]};
}
function v741MacroPopup(name){
  const m=v741MacroInfo(name);
  return `<div class="v741-info-card">
    <div class="v741-info-eyebrow">${realEsc(m.title)}</div>
    <h3>COSA MISURA</h3>
    <p>${realEsc(m.measure)}</p>
    <div class="v741-info-divider"></div>
    <small>DATI CONSIDERATI</small>
    <div class="v741-info-chips">${m.data.map(x=>`<span>${realEsc(x)}</span>`).join("")}</div>
  </div>`;
}
  ...
  try{
    ...
    document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page">
      ...
      <div class="v14-accordions">
        ...
        <section class="v74-form-strip v741-match-form"><div class="v74-strip-head"><b>FORMA RECENTE</b><small>ULTIME 5</small></div><div class="v59-form-row"><span>${realEsc(h.name)}</span><b>${realFormDots(hm.form)}</b><em>${hm.sample}/5</em></div><div class="v59-form-row"><span>${realEsc(a.name)}</span><b>${realFormDots(am.form)}</b><em>${am.sample}/5</em></div></section>

...
function shell(a="dashboard"){
 return `<header class="topbar v2-topbar">
   <div class="v2-brand"><div class="v2-planet"><i></i></div><div><div class="brand-name">SPACE16154</div><div class="brand-sub">FOOTBALL INTELLIGENCE</div></div></div>
   <button class="v2-bell" aria-label="Notifiche">♧<i></i></button>
 </header>
 <main id="main"></main>
 <nav class="bottom-nav v2-bottom-nav">${nav("dashboard","⌂","Dashboard",a)}${nav("matches","□","Matches",a)}${nav("analysis","▥","Analysis",a)}${nav("signals","◎","Top Signals",a)}${nav("more","•••","Altro",a)}</nav>`;
}
...
let DASH_YEAR = 2026;
let DASH_MONTH = 7; // Agosto, 0-based
let DASH_DAY = 11;

function dashDateMeta(){
  const d = new Date(DASH_YEAR, DASH_MONTH, DASH_DAY);
  return {
    id:`${DASH_YEAR}-${String(DASH_MONTH+1).padStart(2,"0")}-${String(DASH_DAY).padStart(2,"0")}`,
    label:`${DASH_DAY} ${MONTHS_IT[DASH_MONTH]}`,
    short:`${String(DASH_DAY).padStart(2,"0")} ${MONTHS_IT[DASH_MONTH].slice(0,3)}`
  };
}
function daysInMonth(year,month){ return new Date(year,month+1,0).getDate(); }
function normalizeDashDay(){
  DASH_DAY=Math.min(DASH_DAY,daysInMonth(DASH_YEAR,DASH_MONTH));
}
function matchesForSelectedDate(){
  // Demo deterministico: distribuisce le fixture sul calendario.
  // Con SportMonks questa funzione userà direttamente fixture.date.
  const seed=(DASH_MONTH*31 + DASH_DAY) % MATCHES.length;
  const count=3;
  return Array.from({length:count},(_,i)=>MATCHES[(seed+i)%MATCHES.length]);
}
function renderDateMenu(){
  const old=document.querySelector(".dash-date-popover");
  if(old){old.remove();return}
  const menu=document.createElement("div");
  menu.className="dash-date-popover dash-date-calendar";
  menu.innerHTML=`
    <div class="dash-date-title">SELEZIONA DATA</div>
    <div class="dash-month-nav">
      <button data-month-prev>‹</button>
      <strong>${MONTHS_IT[DASH_MONTH]} ${DASH_YEAR}</strong>
      <button data-month-next>›</button>
    </div>
    <div class="dash-month-selects">
      <select data-month-select>${MONTHS_IT.map((m,i)=>`<option value="${i}" ${i===DASH_MONTH?"selected":""}>${m}</option>`).join("")}</select>
      <select data-year-select>${[2025,2026,2027].map(y=>`<option value="${y}" ${y===DASH_YEAR?"selected":""}>${y}</option>`).join("")}</select>
    </div>
    <div class="dash-weekdays">${["L","M","M","G","V","S","D"].map(x=>`<span>${x}</span>`).join("")}</div>
    <div class="dash-days">${renderDashDays()}</div>`;
  document.body.appendChild(menu);

  const redraw=()=>{
    normalizeDashDay();
    menu.querySelector(".dash-month-nav strong").textContent=`${MONTHS_IT[DASH_MONTH]} ${DASH_YEAR}`;
    menu.querySelector("[data-month-select]").value=String(DASH_MONTH);
    menu.querySelector("[data-year-select]").value=String(DASH_YEAR);
    menu.querySelector(".dash-days").innerHTML=renderDashDays();
    menu.querySelectorAll("[data-dash-day]").forEach(b=>b.onclick=()=>{
      DASH_DAY=Number(b.dataset.dashDay);
      menu.remove();
      route("dashboard");
    });
  };
  menu.querySelector("[data-month-prev]").onclick=()=>{DASH_MONTH--;if(DASH_MONTH<0){DASH_MONTH=11;DASH_YEAR--}redraw()};
  menu.querySelector("[data-month-next]").onclick=()=>{DASH_MONTH++;if(DASH_MONTH>11){DASH_MONTH=0;DASH_YEAR++}redraw()};
  menu.querySelector("[data-month-select]").onchange=e=>{DASH_MONTH=Number(e.target.value);redraw()};
  menu.querySelector("[data-year-select]").onchange=e=>{DASH_YEAR=Number(e.target.value);redraw()};
  redraw();

  setTimeout(()=>document.addEventListener("click",function close(e){
    if(!menu.contains(e.target) && !e.target.closest(".dash-date-btn")){
      menu.remove();document.removeEventListener("click",close)
    }
  }),0);
}
function renderDashDays(){
  const first=new Date(DASH_YEAR,DASH_MONTH,1);
  const jsDay=first.getDay(); // 0 Sun
  const mondayIndex=(jsDay+6)%7;
  const days=daysInMonth(DASH_YEAR,DASH_MONTH);
  let html="";
  for(let i=0;i<mondayIndex;i++) html+=`<span class="empty"></span>`;
  for(let d=1;d<=days;d++) html+=`<button data-dash-day="${d}" class="${d===DASH_DAY?"active":""}">${d}</button>`;
  return html;
}
...
function dashboard(){
 ...
 document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   <div class="dash-global-head">
     <div><span class="dash-global-kicker">DASHBOARD</span><small>${meta.label}</small></div>
     <button class="dash-date-btn" type="button">▣ <span>${meta.short}</span>⌄</button>
   </div>
 ...
 bind();
 document.querySelector(".dash-date-btn").onclick=e=>{e.stopPropagation();renderDateMenu()};
}
function matches(){
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=5.5.0-ui-test-v6",{updateViaCache:"none"}).catch(()=>{}));

...
async function v67RealTeamProfile(payload){
  ...
  try{
    ...
    const v70SeasonId=b?.season?.id||null;
    const historyPack=await realApi(`/team-history?team=${encodeURIComponent(teamId)}${v70SeasonId?`&season=${encodeURIComponent(v70SeasonId)}`:""}&limit=5`).catch(()=>({data:[]}));
    const history=Array.isArray(historyPack?.data)?historyPack.data:(Array.isArray(historyPack?.fixtures)?historyPack.fixtures:[]);
...
async function v69SafeTeamHistory(teamId, seasonId=null){
  const load=async useSeason=>{
    const q=new URLSearchParams({team:String(teamId),limit:"5"});
  ...
  try{
    const filtered=await load(true);
    if(filtered.length)return filtered;
    if(seasonId)return await load(false);
  }catch{
...
async function realMatchDetail(id){
  ...
  try{
    ...
    const preHfs=realHistory(pack,"home_history"),preAfs=realHistory(pack,"away_history");
    ...
    const v7520RollForm=(team,opp,preMetrics,postMetrics)=>{
      const pre=Array.isArray(preMetrics?.form)?preMetrics.form.slice():[];
      const post=Array.isArray(postMetrics?.form)?postMetrics.form.slice():[];
      if(!v60FixtureFinished(detail)) return pre;

      const gf=realScore(detail,team.id),ga=realScore(detail,opp.id);
      if(gf==null||ga==null) return post.length?post:pre;
      const current={gf,ga,r:gf>ga?"W":gf<ga?"L":"D"};

      // If there were fewer than 5 matches before kick-off, the normal post-match
      // metrics can simply grow by one. With a full 5/5 sample, explicitly roll
      // the PRE-MATCH sequence: remove the oldest result and append the FT result.
      if((preMetrics?.sample||0)<5){
        const withoutCurrent=pre.slice(-4);
        return [...withoutCurrent,current].slice(-5);
      }
      return [...pre.slice(1),current].slice(-5);
    };

    const hForm=v7520RollForm(h,a,preHm,hmRaw),aForm=v7520RollForm(a,h,preAm,amRaw);
    const v7521SeqText=seq=>(Array.isArray(seq)?seq:[]).map(x=>x?.r||"—").join(" ");
    const v7521HomeScore=`${realScore(detail,h.id)??"—"}-${realScore(detail,a.id)??"—"}`;
    const v7521AwayScore=`${realScore(detail,a.id)??"—"}-${realScore(detail,h.id)??"—"}`;
    ...
    document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page">
      ...
      <div class="v14-accordions">
        ...
        <section class="v742-match-form">
          ...
          <div class="v7521-debug">
            <b>DEBUG 1.21</b>
            <span>PRE H: ${v7521SeqText(preHm.form)} · FT: ${v7521HomeScore} · OUT H: ${v7521SeqText(hForm)}</span>
            <span>PRE A: ${v7521SeqText(preAm.form)} · FT: ${v7521AwayScore} · OUT A: ${v7521SeqText(aForm)}</span>
          </div>

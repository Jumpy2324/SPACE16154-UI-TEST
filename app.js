...
const CONFIG={
  version:"12.1.10-v71-10-dashboard-final-polish",
...
function v722H2HStrip(h2,h,a){
  ...
  const chips=h2.slice(0,5).map(f=>{
    ...
    return `<span>${hg}-${ag}</span>`;
  }).filter(Boolean).join("");
  return `<div class="v722-h2h">
    <div class="v722-h2h-top"><b>ULTIMI 5 TESTA A TESTA</b><small>${hw} V ${realTeamCode(h)} · ${d} P · ${aw} V ${realTeamCode(a)}</small></div>
    <div class="v722-h2h-results">${chips}</div>
  </div>`;
}
...
function v722TechCompare(tech,h,a){
  ...
  return `<section class="v722-tech-card">
    <header><h2>DATI TECNICI</h2><span>MEDIA ULTIME 5</span></header>
...
function v722TeamTech(metrics,team,sample){
  ...
  if(!rows.length)return "";
  return `<section class="v19-card v722-team-tech">
    <div class="v19-chead"><b>DATI TECNICI</b><span>ULTIME ${sample||0}/5</span></div>
    <div class="v722-team-tech-id">${realCrest(team)}<b>${realEsc(team.name||"")}</b></div>
    <div class="v722-team-tech-grid">
      ${rows.map(r=>`<div><span>${r[0]}</span><strong>${r[2]==="pct"?realFmt(r[1],0)+"%":realFmt(r[1],r[2])}</strong></div>`).join("")}
    </div>
  </section>`;
}
...
function v59HasOfficialLineups(ctx,h,a){return v722OfficialLineups(ctx,h,a)}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=12.1.10-v71-10-dashboard-final-polish",{updateViaCache:"none"}).catch(()=>{}));

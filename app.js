...
async function realMatchDetail(id){
  ...
  try{
    ...
    const v7518FormSeq=(team,opp,preMetrics)=>{
      const base=Array.isArray(preMetrics?.form)?preMetrics.form.slice(-5):[];
      // Use the SAME detail payload already proven by POST-MATCH (FT + 2-0),
      // not the merged/history fixture.
      if(!v60FixtureFinished(detail)) return base;
      const gf=realScore(detail,team.id),ga=realScore(detail,opp.id);
      if(gf==null||ga==null) return base;
      const currentResult={gf,ga,r:gf>ga?"W":gf<ga?"L":"D"};
      return [...base.slice(-4),currentResult].slice(-5);
    };
    const hForm=v7518FormSeq(h,a,preHm),aForm=v7518FormSeq(a,h,preAm);

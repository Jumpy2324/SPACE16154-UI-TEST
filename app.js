...
async function v67RealTeamProfile(payload){
  ...
  try{
    ...
    root.innerHTML=`<div class="page v67-team-page">
      ...
      <section class="v19-card"><div class="v19-chead"><b>DATI STAGIONE</b><span>${realEsc(b.season?.name||"")}</span></div>
        <div class="v67-season-grid">
          ${[["PG",v60StandingValue(row,"played")],["V",v60StandingValue(row,"won")],["P",v60StandingValue(row,"draw")],["S",v60StandingValue(row,"lost")],["GF",gf],["GS",ga],["DR",gf-ga],["PT",v60StandingValue(row,"pts")]].map(x=>`<div><small>${x[0]}</small><b>${x[1]}</b></div>`).join("")}
        </div>
      </section>

      <section class="v19-card"><div class="v19-chead"><b>DATI TECNICI REALI</b><span>${sample?`${sample} GARA${sample===1?"":"E"}`:"NESSUN CAMPIONE"}</span></div>
        <div class="v67-tech-grid">${[
          ["xG",metrics.xg==null?"—":realFmt(metrics.xg,2)],
          ["Gol fatti",metrics.gf==null?"—":realFmt(metrics.gf,1)],
          ["Gol subiti",metrics.ga==null?"—":realFmt(metrics.ga,1)],
          ["Possesso",metrics.pos==null?"—":realFmt(metrics.pos,0)+"%"],
          ["Tiri",metrics.shots==null?"—":realFmt(metrics.shots,1)],
          ["Tiri in porta",metrics.sot==null?"—":realFmt(metrics.sot,1)],
          ["Corner",metrics.corners==null?"—":realFmt(metrics.corners,1)]
        ].map(x=>`<div><small>${x[0]}</small><b>${x[1]}</b></div>`).join("")}</div>
      </section>
    </div>`;
    bind();
...
function v59HasOfficialLineups(ctx,h,a){
  return v59StarterLineups(ctx,h.id).length>=7 && v59StarterLineups(ctx,a.id).length>=7;
}
...
/* V72.0 — mobile Analysis helpers; data remains sourced from existing real pipeline */
(function(){
 const INFO={
  confidence:"La confidence indica quanto i segnali disponibili concordano tra loro. È separata dalla probabilità dell'esito e verrà calibrata progressivamente con le nuove giornate.",
  macro:"Questa macroarea sintetizza i dati reali disponibili per questa dimensione di gioco. Le componenti mancanti non vengono inventate e non entrano nella valutazione."
 };
 function showInfo(title,text){
   let x=document.getElementById("v72Info");
   if(!x){x=document.createElement("div");x.id="v72Info";x.className="v72-info-bg";x.innerHTML='<div class="v72-info-box"><button>×</button><small>MODEL INFO</small><h3></h3><p></p></div>';document.body.appendChild(x);x.onclick=e=>{if(e.target===x||e.target.closest(".v72-info-box>button"))x.classList.remove("open")}}
   x.querySelector("h3").textContent=title;x.querySelector("p").textContent=text;x.classList.add("open");
 }
 function enhance(){
   const root=document.getElementById("main"); if(!root)return;
   // Long team names in Matches: wrap instead of ellipsis.
   root.querySelectorAll(".v712-team b,.v23-team b").forEach(e=>{e.classList.add("v72-long-team");e.title=(e.textContent||"").trim()});
   // Analysis visual language.
   const isAnalysis=/analysis|match/i.test(location.hash||"") || !!root.querySelector(".analysis-page,.m13-analysis");
   if(!isAnalysis)return;
   root.classList.add("v72-analysis");
   // Add useful info buttons to confidence and macro-area headings.
   root.querySelectorAll("h2,h3,.section-title,.macro-title").forEach(h=>{
     if(h.querySelector(".v72-i"))return;
     const t=(h.textContent||"").trim();
     if(/confidence/i.test(t)||/creaz|final|poss|press|difes|forma/i.test(t.toLowerCase())){
       const b=document.createElement("button");b.className="v72-i";b.textContent="i";
       b.onclick=e=>{e.preventDefault();e.stopPropagation();showInfo(/confidence/i.test(t)?"CONFIDENCE":t,/confidence/i.test(t)?INFO.confidence:INFO.macro)};
       h.appendChild(b);
     }
   });
   // Restore the V47-style gold marker on recognizable macro cards.
   root.querySelectorAll(".macro-card,.v60-macro-card,.analysis-macro,.metric-card,.m13-macro").forEach(c=>{
     if(/creaz|final|poss|press|difes|forma/i.test((c.textContent||"").toLowerCase()))c.classList.add("v72-macro");
   });
   // Mixed-sample indicator next to recent form. Current count stays 0 unless existing UI exposes it.
   const fh=[...root.querySelectorAll("h2,h3,.section-title")].find(x=>/forma recente|ultime 5/i.test(x.textContent||""));
   if(fh&&!root.querySelector(".v72-sample")){
     let current=0, txt=(fh.parentElement?.textContent||"");
     let m=txt.match(/(\d)\s*\/\s*5\s*(?:stagione\s*)?attuale/i); if(m)current=Math.min(5,Math.max(0,+m[1]));
     const d=document.createElement("div");d.className="v72-sample";d.innerHTML=`<b>CAMPIONE MISTO</b><span>${current}/5 attuale</span><i>•</i><span>${5-current}/5 precedente</span>`;fh.after(d);
   }
   // Empty sections: hide only when they are genuinely just a heading + empty marker.
   root.querySelectorAll("section,.analysis-section,.m13-card").forEach(s=>{
     const t=(s.textContent||"").replace(/\s+/g," ").trim();
     if(/^(arbitro|dati tecnici|dati stagione)\s*(?:n\/d|—|-|non disponibile)?$/i.test(t))s.classList.add("v72-empty");
   });
 }
 if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",enhance);else enhance();
 try{new MutationObserver(()=>requestAnimationFrame(enhance)).observe(document.documentElement,{subtree:true,childList:true})}catch(e){}
})();

/* ================= V72.1 COMPACT ANALYSIS ================= */
(function(){
 const n=s=>(s||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"");
 const infoText={
  confidence:"La confidence indica quanto i segnali disponibili concordano tra loro. Non è la probabilità dell'esito: misura la solidità complessiva della lettura in base ai dati disponibili.",
  macro:"La macroarea sintetizza i dati reali disponibili per questa dimensione di gioco. Un dato non disponibile non viene stimato né inventato."
 };
 function info(title,text){
  let bg=document.querySelector("#v721Info");
  if(!bg){bg=document.createElement("div");bg.id="v721Info";bg.className="v721-info-bg";bg.innerHTML='<div class="v721-info"><div class="v721-info-top"><div><small>SPACE MODEL</small><h3></h3></div><button>×</button></div><div class="v721-info-line"></div><p></p></div>';document.body.appendChild(bg);bg.onclick=e=>{if(e.target===bg||e.target.closest(".v721-info button"))bg.classList.remove("open")}}
  bg.querySelector("h3").textContent=title;bg.querySelector("p").textContent=text;bg.classList.add("open");
 }
 function sectionByHeading(root,rx){
  const h=[...root.querySelectorAll("h1,h2,h3,.section-title,.card-title")].find(x=>rx.test(n(x.textContent)));
  return h ? (h.closest("section,.card,.analysis-section,.m13-card")||h.parentElement) : null;
 }
 function hideSeason(root){
  const s=sectionByHeading(root,/dati stagione/); if(s)s.classList.add("v721-remove");
 }
 function compactMacros(root){
  root.querySelectorAll(".v72-macro,.macro-card,.v60-macro-card,.analysis-macro,.m13-macro").forEach(c=>{
   if(!/creaz|final|poss|control|press|difes|forma/.test(n(c.textContent)))return;
   c.classList.add("v721-macro");
   if(!c.querySelector(".v721-signal")){let x=document.createElement("span");x.className="v721-signal";c.prepend(x)}
  });
  root.querySelectorAll(".v72-i").forEach(b=>{
   const h=b.parentElement, t=(h?.textContent||"").replace(/\bi\s*$/i,"").trim();
   b.onclick=e=>{e.preventDefault();e.stopPropagation();info(/confidence/i.test(t)?"CONFIDENCE":t,/confidence/i.test(t)?infoText.confidence:infoText.macro)}
  });
 }
 function technical(root){
  const s=sectionByHeading(root,/dati tecnici(?: reali)?/); if(!s||s.classList.contains("v721-tech"))return;
  s.classList.add("v721-tech");
  const h=s.querySelector("h2,h3,.section-title,.card-title");
  if(h){h.textContent="DATI TECNICI";let sub=document.createElement("div");sub.className="v721-tech-sub";sub.textContent="MEDIA ULTIME 5";h.after(sub)}
  // Keep only actual rows already rendered; style as a three-column comparison.
  s.querySelectorAll("table").forEach(t=>t.classList.add("v721-compare-table"));
 }
 function compactForm(root){
  const s=sectionByHeading(root,/forma recente|ultime 5/); if(s)s.classList.add("v721-compact-list");
 }
 function compactH2H(root){
  const s=sectionByHeading(root,/testa a testa|head.to.head|h2h/);
  if(!s)return;
  s.classList.add("v721-h2h-compact");
  const matchHero=sectionByHeading(root,/match analysis|analisi partita|probabil|esito/);
  if(matchHero && s!==matchHero && !matchHero.querySelector(".v721-h2h-inline")){
    const box=document.createElement("div");box.className="v721-h2h-inline";
    box.innerHTML='<div class="v721-h2h-title"><b>ULTIMI 5 TESTA A TESTA</b><span>H2H</span></div>';
    const clone=s.cloneNode(true); clone.classList.add("v721-h2h-clone");
    box.appendChild(clone); matchHero.appendChild(box); s.classList.add("v721-remove");
  }
 }
 function lineups(root){
  const s=sectionByHeading(root,/formazion/); if(!s)return;
  s.classList.add("v721-lineups");
  const h=s.querySelector("h2,h3,.section-title,.card-title");
  if(h){
   const text=n(s.textContent);
   // Only label official if rendered source explicitly says confirmed/official.
   h.childNodes[0] && (h.childNodes[0].textContent=(/confermat|ufficial|confirmed|official/.test(text)?"FORMAZIONI ":"PROBABILI FORMAZIONI "));
  }
  // Existing pitch dots become shirt silhouettes visually; player data remains untouched.
  s.querySelectorAll(".player-dot,.lineup-player-dot,.pitch-player,.formation-player").forEach(x=>x.classList.add("v721-shirt"));
 }
 function enhance(){
  const root=document.getElementById("main"); if(!root)return;
  root.querySelectorAll(".v712-team b,.v23-team b").forEach(e=>e.classList.add("v72-long-team"));
  const analysis=sectionByHeading(root,/match analysis|analisi partita|probabil|macroare/);
  if(!analysis && !root.classList.contains("v72-analysis"))return;
  root.classList.add("v721-analysis");
  hideSeason(root);compactMacros(root);technical(root);compactForm(root);compactH2H(root);lineups(root);
 }
 if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",enhance);else enhance();
 try{new MutationObserver(()=>requestAnimationFrame(enhance)).observe(document.documentElement,{subtree:true,childList:true})}catch(e){}
})();

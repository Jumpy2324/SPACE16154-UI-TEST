...
function v7726FilterLeagueLogo(f){
  ...
  // Use exactly the same SportMonks competition metadata used by league filters.
  try{
    const c=v60CompFromName(V60_META_CACHE,name);
    if(c?.image_path){
      return `<span class="v60-league-logo small"><img class="v7727-card-league-logo" src="${realEsc(c.image_path)}" alt="${realEsc(name)}" loading="lazy" referrerpolicy="no-referrer"></span>`;
    }
  }catch{}

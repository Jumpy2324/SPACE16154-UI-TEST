...
function v19League(n){
 let ts=V19T[n]||[];
 document.querySelector("#main").innerHTML=`<div class="page v19-ranking v23-ranking">
   ...
   <div class="v23-rank-wrap">
     <div class="v23-rank-title">CLASSIFICA MODELLO</div>
     ${ts.map((t,i)=>{
       ...
       return `<button class="v23-rank-row" data-v19t="${t[0]}" data-v19league="${n}">
         <span class="v23-rank-pos">${i+1}</span>
         <span class="v23-rank-team">${crest(t[1],"")}<b>${t[2]}</b></span>
         <span class="v23-rank-meta"><small>PG</small><strong>${t[3]}</strong></span>
         <span class="v23-rank-form">${v19Form(vals)}</span>
         <span class="v23-rank-rating"><small>RATING</small><strong>${t[9]}</strong><i class="${tc}">${ti}</i></span>
       </button>`;
     }).join("")}
   </div>
 </div>`;
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=7.6.0-ui-test-v23",{updateViaCache:"none"}).catch(()=>{}));

...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=10.6.0-v56-v47-real",{updateViaCache:"none"}).catch(()=>{}));
...
const v31mNow=new Date(); let mo=v31mNow.getMonth(),day=v31mNow.getDate(),active=null,m13ManualDate=false;
...
function bindPicker(root){
 ...
 root.querySelectorAll("[data-pick]").forEach(b=>b.onclick=()=>{m13ManualDate=true;if(b.dataset.pick==="month")mo=+b.dataset.i;else day=+b.dataset.i+1;root.querySelector(".m13-sheetback")?.remove();render(root)});
}
...
window.addEventListener("popstate",e=>{
 ...
 if(s?.space16154v27&&s.kind==="matches-league"){
   active=s.league;
   const root=document.querySelector("[data-m13='1']");
   if(root)render(root);
...
/* ============================================================
   SPACE16154 V56 — V47 UI + SPORTMONKS REAL DATA
   Mantiene la struttura V47. Nessun dato demo viene usato per
   le fixture reali. I valori non disponibili restano “—”.
   ============================================================ */
(()=>{
const V56_BASE="https://space16154-api.gianmarco-candeletti.workers.dev";
const V56_LEAGUES=["Serie A","Premier League","La Liga","Bundesliga","Ligue 1"];
const esc=v=>String(v??"").replace(/[&<>'"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c]));
const num=v=>{const n=Number(v);return Number.isFinite(n)?n:null};
const clamp=(v,a=0,b=100)=>Math.max(a,Math.min(b,v));
const fmt=(v,d=1)=>v==null?"—":Number(v).toLocaleString("it-IT",{minimumFractionDigits:d,maximumFractionDigits:d});
const code=t=>String(t?.short_code||t?.code||t?.name||"?").replace(/[^A-Za-z0-9]/g,"").slice(0,3).toUpperCase()||"?";
const imageCrest=t=>t?.image_path?`<span class="v56-real-crest"><img src="${esc(t.image_path)}" alt="${esc(t.name||'')}" loading="lazy" referrerpolicy="no-referrer"></span>`:crest(code(t),"");
const loc=(f,x)=>(Array.isArray(f?.participants)?f.participants:[]).find(p=>String(p?.meta?.location||"").toLowerCase()===x)||null;
const normFixture=f=>{const home=loc(f,"home")||{},away=loc(f,"away")||{};const raw=String(f?.starting_at||"");return {...f,home,away,time:raw.slice(11,16)||"--:--"}};
const currentDate=()=>dashDateMeta().id;
const leagueOk=f=>V56_LEAGUES.includes(f?.league?.name||"");
let v56Cache={date:"",fixtures:[]};
async function api(path){const r=await fetch(V56_BASE+path,{headers:{Accept:"application/json"},cache:"no-store"});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b?.sportmonks?.message||b?.message||b?.error||`HTTP ${r.status}`);return b}
async function fixturesFor(date){if(v56Cache.date===date)return v56Cache.fixtures;const b=await api(`/fixtures?date=${encodeURIComponent(date)}`);const arr=(Array.isArray(b?.data)?b.data:[]).map(normFixture).filter(leagueOk);v56Cache={date,fixtures:arr};return arr}

function scoreFor(f,teamId){const ss=Array.isArray(f?.scores)?f.scores:[];let pool=ss.filter(s=>String(s?.participant_id)===String(teamId)&&String(s?.description||"").toUpperCase()==="CURRENT");if(!pool.length)pool=ss.filter(s=>String(s?.participant_id)===String(teamId));const vals=pool.map(s=>num(s?.score?.goals??s?.score?.value??s?.score)).filter(v=>v!=null);return vals.length?vals.at(-1):null}
function opp(f,id){return (Array.isArray(f?.participants)?f.participants:[]).find(p=>String(p?.id)!==String(id))||null}
function history(pack,side){const b=pack?.[side];return b?.ok===true&&Array.isArray(b?.data)?b.data:[]}
function recent(fixtures,teamId){return fixtures.slice().sort((a,b)=>String(b?.starting_at||"").localeCompare(String(a?.starting_at||""))).slice(0,5).map(f=>{const o=opp(f,teamId),gf=scoreFor(f,teamId),ga=o?scoreFor(f,o.id):null;return {gf,ga,r:gf==null||ga==null?"—":gf>ga?"W":gf<ga?"L":"D",f}})}
function statName(s){return String(s?.type?.developer_name||s?.type?.name||s?.name||"").toUpperCase()}
const aliases={
 "Possesso":["BALL_POSSESSION","POSSESSION"],"Tiri":["SHOTS_TOTAL","TOTAL_SHOTS"],"Tiri in porta":["SHOTS_ON_TARGET","SHOTS_ON_GOAL"],
 "Corner":["CORNERS","CORNER_KICKS"],"Attacchi":["ATTACKS"],"Attacchi pericolosi":["DANGEROUS_ATTACKS"],"Falli":["FOULS"],
 "Tiri fuori":["SHOTS_OFF_TARGET","SHOTS_OFF_GOAL"]
};
function avgStat(fs,teamId,label){const vals=[];for(const f of fs){for(const s of (Array.isArray(f?.statistics)?f.statistics:[])){if(String(s?.participant_id)!==String(teamId))continue;const n=statName(s);if(!(aliases[label]||[]).some(a=>n===a||n.includes(a)))continue;const v=num(s?.data?.value??s?.value);if(v!=null)vals.push(v)}}return vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null}
function avgXg(fs,teamId){const vals=[];for(const f of fs){const xs=Array.isArray(f?.xGFixture)?f.xGFixture:Array.isArray(f?.xgfixture)?f.xgfixture:[];for(const x of xs)if(String(x?.participant_id)===String(teamId)){const v=num(x?.data?.value??x?.value);if(v!=null)vals.push(v)}}return vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null}
function teamMetrics(fs,team){const r=recent(fs,team.id);const gfs=r.map(x=>x.gf).filter(x=>x!=null),gas=r.map(x=>x.ga).filter(x=>x!=null);const points=r.reduce((a,x)=>a+(x.r==="W"?3:x.r==="D"?1:0),0);return {
 form:r,formPts:r.length?points/(r.length*3)*100:null,gf:gfs.length?gfs.reduce((a,b)=>a+b,0)/gfs.length:null,ga:gas.length?gas.reduce((a,b)=>a+b,0)/gas.length:null,
 xg:avgXg(fs,team.id),pos:avgStat(fs,team.id,"Possesso"),shots:avgStat(fs,team.id,"Tiri"),sot:avgStat(fs,team.id,"Tiri in porta"),corners:avgStat(fs,team.id,"Corner"),attacks:avgStat(fs,team.id,"Attacchi"),danger:avgStat(fs,team.id,"Attacchi pericolosi"),fouls:avgStat(fs,team.id,"Falli")
}}
function preliminary(m){
 const threatVals=[m.xg!=null?clamp(m.xg/2.2*100):null,m.shots!=null?clamp(m.shots/16*100):null,m.sot!=null?clamp(m.sot/7*100):null,m.danger!=null?clamp(m.danger/65*100):null].filter(v=>v!=null);
 const threat=threatVals.length?Math.round(threatVals.reduce((a,b)=>a+b,0)/threatVals.length):null;
 const defense=m.ga!=null?Math.round(clamp(100-m.ga/2.2*100)):null;
 const form=m.formPts!=null?Math.round(m.formPts):null;
 const pressing=m.danger!=null?Math.round(clamp(m.danger/65*100)):null;
 return {threat,defense,form,opponent:null,pressing,availability:null};
}
function standing(pack,teamId){const d=pack?.standings?.ok===true?pack.standings.data:null;const rows=Array.isArray(d)?d:Array.isArray(d?.data)?d.data:[];const row=rows.find(x=>String(x?.participant_id??x?.participant?.id)===String(teamId));return row?.position??row?.rank??null}
function formDotsReal(r){return `<div class="form-dots">${r.length?r.map(x=>`<i class="${x.r==='W'?'win':x.r==='L'?'loss':x.r==='D'?'draw':''}">${x.r==='W'?'V':x.r==='L'?'P':x.r==='D'?'N':'—'}</i>`).join(""):`<span class="v56-muted">—</span>`}</div>`}
function technicalRows(hm,am){return [["xG",hm.xg,am.xg],["Gol fatti",hm.gf,am.gf],["Gol subiti",hm.ga,am.ga],["Possesso %",hm.pos,am.pos],["Tiri",hm.shots,am.shots],["Tiri in porta",hm.sot,am.sot],["Corner",hm.corners,am.corners],["Attacchi pericolosi",hm.danger,am.danger],["Attacchi",hm.attacks,am.attacks],["Falli",hm.fouls,am.fouls]]}
function metricVal(label,v){if(v==null)return "—";return label==="Possesso %"?fmt(v,0)+"%":fmt(v,1)}
function h2hRows(pack){const a=history(pack,"h2h").slice().sort((x,y)=>String(y?.starting_at||"").localeCompare(String(x?.starting_at||""))).slice(0,5);if(!a.length)return `<p class="v56-empty">Nessun H2H disponibile.</p>`;return a.map(f=>{const h=loc(f,"home")||{},w=loc(f,"away")||{};return `<div class="compare-row"><span>${esc(String(f?.starting_at||"").slice(0,10))}</span><b>${esc(h.name||'Casa')} ${scoreFor(f,h.id)??'—'}–${scoreFor(f,w.id)??'—'} ${esc(w.name||'Trasferta')}</b><b></b></div>`}).join("")}
function liveRows(detail){const a=Array.isArray(detail?.statistics)?detail.statistics:[];if(!a.length)return `<p class="v56-empty">La gara non è ancora iniziata. Le statistiche live compariranno qui automaticamente.</p>`;return a.slice(0,24).map(s=>`<div class="compare-row"><span>${esc(s?.type?.name||s?.type?.developer_name||'Statistica')}</span><b>${esc(s?.data?.value??s?.value??'—')}</b><b></b></div>`).join("")}

function v56DashHero(f){const h=f.home,a=f.away;return `<section class="panel"><div class="section-head"><h2>☆ &nbsp; MATCH IN EVIDENZA</h2><span>${esc(f.league?.name||'')}</span></div>
 <div class="match-hero"><div class="club">${imageCrest(h)}<strong>${esc(h.name||'Casa')}</strong></div><div class="match-time"><b>${esc(f.time)}</b><span>${esc(String(f.starting_at||'').slice(0,10))}</span><em>VS</em></div><div class="club">${imageCrest(a)}<strong>${esc(a.name||'Trasferta')}</strong></div></div>
 <div class="prediction-grid"><div class="pred-card"><small>1X2</small><span>MODELLO</span><strong>—</strong><em>calibrazione</em></div><div class="pred-card"><small>GOL / NO GOL</small><span>MODELLO</span><strong>—</strong><em>calibrazione</em></div><div class="pred-card"><small>OVER / UNDER 2.5</small><span>MODELLO</span><strong>—</strong><em>calibrazione</em></div></div>
 <button class="primary-btn" data-match="${esc(f.id)}">VAI ALL'ANALISI COMPLETA <span>›</span></button></section>`}

const OLD_DASH=dashboard;
dashboard=async function(){
 const meta=dashDateMeta();document.querySelector("#main").innerHTML=`<div class="page"><div class="eyebrow">SPORTMONKS · DATI REALI</div><h1>DASHBOARD</h1><p class="subtitle">${meta.label}</p><section class="panel"><div class="v56-loading">Caricamento partite reali…</div></section></div>`;bind();
 try{const fs=await fixturesFor(meta.id);const f=fs[0];document.querySelector("#main").innerHTML=`<div class="page"><div class="eyebrow">SPORTMONKS · DATI REALI</div><h1>DASHBOARD</h1><p class="subtitle">${meta.label}</p>${f?v56DashHero(f):`<section class="panel"><div class="v56-empty">Nessuna partita dei 5 campionati disponibile per questa data.</div></section>`}<section class="panel"><div class="section-head"><h2>PROSSIME PARTITE</h2><button class="text-btn" data-nav="matches">VEDI TUTTE ›</button></div>${fs.slice(1,5).map(x=>`<button class="fixture" data-match="${x.id}"><div class="fixture-time"><b>${esc(x.time)}</b><small>${esc(x.league?.name||'')}</small></div><div class="fixture-team">${imageCrest(x.home)}<span>${esc(x.home?.name||'')}</span></div><b class="vs">VS</b><div class="fixture-team away"><span>${esc(x.away?.name||'')}</span>${imageCrest(x.away)}</div><span class="competition">${esc(x.league?.name||'')}</span><span class="chev">›</span></button>`).join("")||`<div class="v56-empty">Nessun'altra partita disponibile.</div>`}</section><section class="panel"><div class="section-head"><h2>TOP SIGNALS</h2><span>MODELLO</span></div><div class="v56-empty">Segnali in attesa di calibrazione. Nessun valore demo viene usato.</div></section></div>`;bind();}catch(e){document.querySelector("#main").innerHTML=`<div class="page"><div class="eyebrow">SPORTMONKS</div><h1>DASHBOARD</h1><section class="panel"><div class="v56-empty">${esc(e.message)}</div></section></div>`;bind()}
};

async function renderRealMatch(id){
 document.querySelector("#main").innerHTML=`<div class="page v14-analysis-page"><div class="v14-analysis-top"><button class="v14-back" data-nav="matches">←</button></div><section class="v14-card"><div class="v56-loading">Caricamento analisi SportMonks…</div></section></div>`;bind();
 try{
  const [pack,fb]=await Promise.all([api(`/prematch?id=${encodeURIComponent(id)}`),api(`/fixture?id=${encodeURIComponent(id)}`)]),detail=fb?.data||fb,fx=pack.fixture||{},h=pack.teams?.home||{},a=pack.teams?.away||{};
  const hf=history(pack,"home_history"),af=history(pack,"away_history"),hm=teamMetrics(hf,h),am=teamMetrics(af,a),hp=preliminary(hm),ap=preliminary(am),hs=standing(pack,h.id),as=standing(pack,a.id);
  const macros=[["threat","Threat Creation"],["defense","Defense Stability"],["form","Forma"],["opponent","Opponent Strength"],["pressing","Pressing / PPDA"],["availability","Availability"]];
  document.querySelector("#main").innerHTML=`<div class="page v14-analysis-page">
   <div class="v14-analysis-top"><button class="v14-back" data-nav="matches">←</button><span class="v56-real-pill">SPORTMONKS · REAL DATA</span></div>
   <header class="v14-match-head"><h1>${esc(h.name)} — ${esc(a.name)}</h1><p>${esc(fx.league?.name||detail?.league?.name||'')} · ${esc(String(fx.starting_at||'').slice(0,10))} · ${esc(String(fx.starting_at||'').slice(11,16))}</p><div class="v14-versus"><div>${imageCrest(h)}<b>${esc(h.name)}</b><small>${hs?`Classifica ${hs}°`:'Classifica —'}</small></div><div class="v14-vs-center"><strong>VS</strong></div><div>${imageCrest(a)}<b>${esc(a.name)}</b><small>${as?`Classifica ${as}°`:'Classifica —'}</small></div></div></header>
   <section class="v14-card"><div class="v14-card-title">PROBABILITÀ DEL MODELLO ${info("Probabilità","La struttura è pronta, ma le probabilità restano nascoste finché il modello SPACE16154 non viene calibrato e backtestato sui dati reali.")}</div><div class="v14-markets"><div class="v14-market"><b>1X2</b><div><span><small>1</small><strong>—</strong></span><span><small>X</small><strong>—</strong></span><span><small>2</small><strong>—</strong></span></div></div><div class="v14-market"><b>GOL / NO GOL</b><div><span><small>GOL</small><strong>—</strong></span><span><small>NO GOL</small><strong>—</strong></span></div></div><div class="v14-market"><b>OVER / UNDER 2.5</b><div><span><small>OVER</small><strong>—</strong></span><span><small>UNDER</small><strong>—</strong></span></div></div></div><p class="v56-note">In calibrazione · nessuna percentuale demo.</p></section>
   <section class="v14-card v14-model"><div class="v14-card-title">ANALISI DEL MODELLO <span class="v56-prelim">PRELIMINARE</span></div><div class="v14-score-head"><div class="v14-score-team">${imageCrest(h)}<b>${esc(h.name)}</b><strong>—</strong></div><div class="v14-score-diff"><small>MODEL SCORE</small><strong>—</strong><span>calibrazione</span></div><div class="v14-score-team">${imageCrest(a)}<b>${esc(a.name)}</b><strong>—</strong></div></div>
    <div class="v14-macros">${macros.map(([k,label])=>{const hv=hp[k],av=ap[k],dv=hv!=null&&av!=null?hv-av:null,meta=V14_MACRO_META[k];return `<div class="v14-macro"><div class="v14-macro-name"><span class="v14-macro-icon">${meta.icon}</span><div><b>${meta.title} ${info(meta.title,meta.parts)}</b><small>${meta.desc}</small></div></div><div class="v14-macro-values"><div><strong>${hv??'—'}</strong><span class="v14-bar"><i style="width:${hv??0}%"></i></span></div><em class="${dv==null?'neu':dv>=0?'pos':'neg'}">${dv==null?'—':(dv>=0?'+':'')+dv}</em><div><strong>${av??'—'}</strong><span class="v14-bar"><i style="width:${av??0}%"></i></span></div></div></div>`}).join("")}</div><p class="v56-note">Le macroaree con valore sono stime preliminari ricavate dai dati reali disponibili; non sono ancora le formule definitive SPACE16154.</p></section>
   <section class="v14-card"><div class="v14-card-title">TREND MODEL</div><div class="v56-trend-grid"><div><b>${esc(h.name)}</b>${formDotsReal(hm.form)}<small>Forma recente reale</small></div><div><b>${esc(a.name)}</b>${formDotsReal(am.form)}<small>Forma recente reale</small></div></div></section>
   <div class="v14-accordions">
    <details class="v14-fold" open><summary>DATI TECNICI <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><div class="compare-head"><span>INDICATORE</span><b>${esc(code(h))}</b><b>${esc(code(a))}</b></div>${technicalRows(hm,am).map(r=>`<div class="compare-row"><span>${r[0]}</span><b>${metricVal(r[0],r[1])}</b><b>${metricVal(r[0],r[2])}</b></div>`).join("")}</div></details>
    <details class="v14-fold"><summary>FORMA RECENTE <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><div class="compare-head"><span>SQUADRA</span><b>ULTIME 5</b><b></b></div><div class="compare-row"><span>${esc(h.name)}</span><b>${formDotsReal(hm.form)}</b><b></b></div><div class="compare-row"><span>${esc(a.name)}</span><b>${formDotsReal(am.form)}</b><b></b></div></div></details>
    <details class="v14-fold"><summary>TESTA A TESTA <span class="v18-toggle">+</span></summary><div class="v14-fold-body">${h2hRows(pack)}</div></details>
    <details class="v14-fold"><summary>PROBABILI FORMAZIONI <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><p class="v56-empty">— · verranno mostrate solo quando disponibili da SportMonks.</p></div></details>
    <details class="v14-fold"><summary>ASSENZE <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><p class="v56-empty">— · nessun dato affidabile ricevuto nel pacchetto attuale.</p></div></details>
    <details class="v14-fold"><summary>ARBITRO <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><p class="v56-empty">— · in attesa del dato ufficiale.</p></div></details>
    <details class="v14-fold"><summary>STATISTICHE FIXTURE <span class="v18-toggle">+</span></summary><div class="v14-fold-body">${liveRows(detail)}</div></details>
   </div></div>`;bind();
 }catch(e){document.querySelector("#main").innerHTML=`<div class="page v14-analysis-page"><button class="back-btn" data-nav="matches">‹ MATCHES</button><section class="v14-card"><div class="v56-empty">${esc(e.message)}</div></section></div>`;bind()}
}
const OLD_MATCH_DETAIL=matchDetail;
matchDetail=function(id){return /^\d+$/.test(String(id||""))?renderRealMatch(id):OLD_MATCH_DETAIL(id)};

/* V56 Matches: conserva esattamente l'impianto V47, sostituendo solo le fixture. */
const leagueDefs=[["serie-a","Serie A","A"],["premier","Premier League","PL"],["laliga","La Liga","LL"],["bundesliga","Bundesliga","BL"],["ligue1","Ligue 1","L1"]];
let selectedLeague=null;
function leagueByKey(k){return leagueDefs.find(x=>x[0]===k)}
function selectedMonthDate(){return currentDate()}
function matchesHomeHtml(fs){return `<section class="m13-page"><div class="m13-titlebar"><h1 class="v23-matches-title">MATCHES</h1><div class="m13-datepair"><button class="m13-datebtn"><small>MESE</small><b>${MONTHS_IT[DASH_MONTH].slice(0,3).toUpperCase()}</b></button><button class="m13-datebtn"><small>GIORNO</small><b>${DASH_DAY}</b></button></div></div><div class="v56-source">SPORTMONKS · DATI REALI · ${esc(selectedMonthDate())}</div><div class="m13-leagues">${leagueDefs.map(l=>`<button class="m13-league" data-v56league="${l[0]}"><span class="m13-league-logo">${l[2]}</span><span>${l[1]}</span><em>${fs.filter(f=>f.league?.name===l[1]).length}</em></button>`).join("")}</div></section>`}
function matchesLeagueHtml(fs,l){const arr=fs.filter(f=>f.league?.name===l[1]);return `<section class="m13-page v23-matches-league"><div class="m13-leaguehead v23-leaguehead"><button class="m13-back" data-v56back>←</button><div class="m13-leagueidentity"><span class="m13-league-logo">${l[2]}</span><h1>${l[1]}</h1></div></div><div class="v56-source">SPORTMONKS · ${esc(selectedMonthDate())}</div><div class="m13-fixtures v23-league-cards">${arr.length?arr.map(f=>`<article class="v23-match-card"><div class="v23-time">${esc(f.time)}</div><div class="v23-match-row"><div class="v23-team home">${imageCrest(f.home)}<b>${esc(f.home?.name||'')}</b></div><button class="v23-analysis" data-match="${f.id}">ANALISI</button><div class="v23-team away"><b>${esc(f.away?.name||'')}</b>${imageCrest(f.away)}</div></div></article>`).join(""):`<div class="v56-empty">Nessuna partita reale disponibile.</div>`}</div></section>`}
async function mountV56Matches(){if(!document.querySelector('.nav-item.active[data-nav="matches"]'))return;const main=document.querySelector('#main');if(!main)return;main.innerHTML=`<section class="m13-page"><div class="v56-loading">Caricamento SportMonks…</div></section>`;try{const fs=await fixturesFor(selectedMonthDate());const render=()=>{main.innerHTML=selectedLeague?matchesLeagueHtml(fs,leagueByKey(selectedLeague)):matchesHomeHtml(fs);main.querySelectorAll('[data-v56league]').forEach(b=>b.onclick=()=>{selectedLeague=b.dataset.v56league;render()});main.querySelector('[data-v56back]')?.addEventListener('click',()=>{selectedLeague=null;render()});bind()};render()}catch(e){main.innerHTML=`<section class="m13-page"><div class="v56-empty">${esc(e.message)}</div></section>`}}
const obs=new MutationObserver(()=>{if(document.querySelector('.nav-item.active[data-nav="matches"]')&&!document.querySelector('[data-v56league],.v56-source,.v56-loading'))setTimeout(mountV56Matches,0)});obs.observe(document.documentElement,{childList:true,subtree:true});

/* Evita che la prima schermata resti sui dati demo dopo il caricamento. */
setTimeout(()=>{if(document.querySelector('.nav-item.active[data-nav="dashboard"]'))dashboard()},80);
})();

...
function dashboard(){
 const m=MATCHES[0], best=topOutcome(m);
 const oneX2=[["1",m.p.one],["X",m.p.x],["2",m.p.two]];
 const gg=[["GOL",m.p.gg],["NO GOL",m.p.ng]];
 const ou=[["OVER",m.p.over],["UNDER",m.p.under]];
 const leaders=MATCHES.filter(x=>x.signals.length).slice(0,3);
 const all=MATCHES.flatMap(x=>x.signals);
 ...
 <section class="v2-featured">
  <div class="v2-panel-head"><h2>MATCH IN EVIDENZA <span>☆</span></h2><b>OGGI</b></div>
  <div class="v2-meta">${m.league} <i>•</i> 12 Maggio 2025 <i>•</i> ${m.time}</div>
  <div class="v2-match">
 ...
 <section class="v2-signals">
  ...
  <div class="v2-signal-list">${leaders.map(x=>{
    ...
    const primary=c?"HIGH CONFIDENCE":v?"VALUE BET":"FORMA", primaryClass=c?"confidence":v?"value":"form";
    return `<button class="v2-signal-row" data-match="${x.id}">
      <div class="v2-signal-team">${crest(x.h.code,x.h.cls)}</div>
      <div class="v2-signal-main"><span class="${primaryClass}">${primary}</span><b>${x.h.name} - ${x.a.name}</b><strong>Esito: ${p[0]}</strong></div>
      <div class="v2-signal-metrics">
 ...
 <section class="v2-overview">
  <div class="v2-panel-head"><h2>PANORAMICA OGGI</h2></div>
  <div class="v2-overview-grid">
   ...
   <div><i class="blue">◎</i><strong>${all.length?R(avg(MATCHES.map(x=>topOutcome(x)[1])))+"%":"—"}</strong><b>Probabilità media<br>segnali</b><small>Media della probabilità degli esiti segnalati.</small></div>
   ...
   <div><i class="teal">✦</i><strong>${confAll.length?R(avg(confAll.map(s=>s.score)))+"%":"—"}</strong><b>Confidence<br>media</b><small>Convergenza media dei segnali Confidence di oggi.</small></div>
  </div>
 </section>
 </div>`; bind();
}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=5.2.0-ui-test-v3",{updateViaCache:"none"}).catch(()=>{}));

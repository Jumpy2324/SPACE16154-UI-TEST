...
const CONFIG={
  version:"11.5.0-v65-mobile-polish",
...
async function v60StandingsPage(initialKey="la-liga"){
 ...
 try{
  ...
  const load=async key=>{
   ...
   ${merged.length?`<div class="v60-standing-foot"><b>${b.live?"CLASSIFICA LIVE":"CLASSIFICA UFFICIALE"}</b> · ${realEsc(b.season?.name||"")}<br><span>${b.live?"Include i risultati parziali delle gare in corso.":"Nessuna gara in corso: classifica ufficiale corrente."}</span></div>`:`<div class="empty-note v62-standing-empty">Classifica non ricevuta da SportMonks.</div>`}`;
  };
...
async function realMatches(){
 ...
 try{
  ...
  const home=()=>{root.innerHTML=`<section class="m13-page"><div class="m13-titlebar"><h1 class="v23-matches-title">MATCHES</h1></div><div class="m13-leagues">${leagues.map(l=>`<button class="m13-league v63-match-league" data-rl="${l.key}"><span class="v63-match-league-center">${v60LeagueLogo(l)}<span>${realEsc(l.name)}</span></span></button>`).join("")}</div></section>`;root.querySelectorAll("[data-rl]").forEach(b=>b.onclick=()=>view(leagues.find(l=>l.key===b.dataset.rl)))};
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=11.5.0-v65-mobile-polish",{updateViaCache:"none"}).catch(()=>{}));

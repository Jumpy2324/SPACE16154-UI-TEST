...
async function v19AnalysisHome(){v27History({kind:"analysis-home"});document.querySelector("#main").innerHTML=`<div class="page v19-home"><header><h1>ANALYSIS</h1><p>INTELLIGENCE CENTER</p></header><div class="empty-note">Caricamento campionati…</div></div>`;bind();const meta=await v60CompetitionMeta();const comps=V60_COMPETITIONS.map(c=>({...c,...((meta?.competitions||[]).find(x=>x.key===c.key)||{})}));document.querySelector("#main").innerHTML=`<div class="page v19-home"><header><h1>ANALYSIS</h1><p>INTELLIGENCE CENTER</p></header><div class="v19-leagues">${comps.map(l=>`<button data-v60-analysis="${l.key}">${v60LeagueLogo(l)}<b>${realEsc(l.name)}</b><small>Classifica + squadre</small></button>`).join("")}</div></div>`;bind();document.querySelectorAll("[data-v60-analysis]").forEach(b=>b.onclick=()=>v60StandingsPage(b.dataset.v60Analysis))}
...
async function v60StandingsPage(initialKey="la-liga"){
 ...
 try{
  ...
  const load=async key=>{
   ...
   root.innerHTML=`<div class="page v60-standings-page"><div class="eyebrow">SPORTMONKS LIVE DATA</div><div class="v60-stand-title"><h1>CLASSIFICA</h1>${v60LeagueLogo(c)}</div>
   ...
   <div class="v60-standing-foot">${b.live?"CLASSIFICA LIVE":"STAGIONE CORRENTE"} · ${realEsc(b.season?.name||"")}</div>`;
  };
...
async function realDashboard(){
 ...
 try{
  ...
  <section class="v6-hero-combined"><div class="v2-panel-head"><h2>MATCH IN EVIDENZA <span>☆</span></h2><b>${meta.short.toUpperCase()}</b></div><div class="v2-meta">${v60LeagueLogo(comp,true)} ${realEsc(m.leagueName)} <i>•</i> ${meta.short} <i>•</i> ${realEsc(m.time)}</div><div class="v2-match"><div class="v2-club">${realCrest(m.h)}<strong>${realEsc(m.h.name||"").toUpperCase()}</strong></div><div class="v2-center"><b>VS</b><button data-real-match="${m.id}">⌁ &nbsp; Analisi completa</button></div><div class="v2-club">${realCrest(m.a)}<strong>${realEsc(m.a.name||"").toUpperCase()}</strong></div></div><div class="v6-prob-divider"></div><div class="v6-prob-title">ESITO PIÙ PROBABILE ${conf!=null?`<span class="v60-dash-sample">Confidence ${conf}%</span>`:""}</div><div class="v2-1x2"><div><span>1</span><strong>${pred?pred.one+"%":"—"}</strong><i></i></div><div><span>X</span><strong>${pred?pred.x+"%":"—"}</strong><i></i></div><div><span>2</span><strong>${pred?pred.two+"%":"—"}</strong><i></i></div></div><div class="v2-other-markets"><div class="v2-submarket"><b>GOL / NO GOL</b><div><span><small>GOL</small><strong>${pred?pred.gg+"%":"—"}</strong></span><span><small>NO GOL</small><strong>${pred?pred.ng+"%":"—"}</strong></span></div></div><div class="v2-submarket"><b>OVER / UNDER 2.5</b><div><span><small>OVER</small><strong>${pred?pred.over+"%":"—"}</strong></span><span><small>UNDER</small><strong>${pred?pred.under+"%":"—"}</strong></span></div></div></div></section>
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=11.0.0-v60-complete",{updateViaCache:"none"}).catch(()=>{}));

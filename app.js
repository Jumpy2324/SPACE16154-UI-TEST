// SPACE16154 V77.9 — FT cleanup + positive model advantage + cache refresh
...
const CONFIG={
  version:"12.2.1-v72-compact-analysis",
...
function signalCard(s,m){
 return `<article class="signal ${s.kind}"><div class="signal-type">${signalLabel(s)} ${info(s.type,s.reason)}</div><div class="signal-main">${s.pick}</div><div class="signal-match">${m.h.name} vs ${m.a.name}</div><div class="signal-bottom"><span>${s.type==="Value"?"EDGE":s.type==="Forma"?"FORMA":"CONFIDENCE"}</span><strong>${s.type==="Value"?"+"+s.score+" p.p.":s.type==="Forma"?"+"+s.score:s.score+"%"}</strong></div></article>`;
}
 ...
 return `<button class="dash-signal-match" data-match="${m.id}">
   ...
   <div class="dash-signal-lines">
     ${m.signals.map(s=>`<div class="dash-signal-line ${s.kind}">
       ...
       <b>${signalLabel(s)}</b>
       <strong>${s.type==="Value"?"+"+s.score+" p.p.":s.type==="Forma"?"+"+s.score:s.score+"%"}</strong>
     </div>`).join("")}
   </div>
 </button>`;
}
...
function v712MoveDate(delta){
  const d=new Date(Date.UTC(DASH_YEAR,DASH_MONTH,DASH_DAY,12,0,0));
  d.setUTCDate(d.getUTCDate()+delta);
  DASH_YEAR=d.getUTCFullYear(); DASH_MONTH=d.getUTCMonth(); DASH_DAY=d.getUTCDate();
  DASH_DATE_MANUAL=true;
}
...
function dashboard(){
 ...
 document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   ...
   <section class="v5-upcoming">
    ...
    <div class="v5-upcoming-list v7721-dashboard-upcoming">
...
const V14_MACRO_META={
 ...
 opponent:{icon:"🎯",title:"Opponent Strength",desc:"Livello degli avversari affrontati recentemente.",parts:"posizione SportMonks degli avversari nelle ultime 5 · classifica reale"},
 pressing:{icon:"🏃",title:"Pressing / PPDA",desc:"Intensità della pressione recente.",parts:"Football Pressure Index SportMonks medio sulle ultime 5; nessun PPDA viene inventato se il dato non esiste"},
 availability:{icon:"♟",title:"Availability",desc:"Disponibilità della rosa in base alle assenze correnti.",parts:"SportMonks sidelined · infortuni/squalifiche attive · indice disponibilità 0–100"}
};
function v14Band(v){
...
function matchDetail(id){
 ...
 document.querySelector("#main").innerHTML=`<div class="page v14-analysis-page">
  ...
  <section class="v14-card v14-model">
    ...
    <div class="v14-macros">${MACROS.map(([k])=>{
       ...
       return `<div class="v14-macro">
         <div class="v14-macro-name"><span class="v14-macro-icon">${meta.icon}</span><div><b>${meta.title} <button class="info" data-info-title="${realEsc(meta.title)}" data-info="${encodeURIComponent(meta.parts)}">i</button></b><small>${meta.desc}</small></div></div>
  ...
  <div class="v14-accordions">
    ...
    <details class="v14-fold"><summary>
function v7714CountStarters(rows=[]){
  return (Array.isArray(rows)?rows:[]).filter(x=>{
    const starter=
      ...
      x?.is_starter===true ||
      x?.meta?.type==="starter" ||
      x?.meta?.position==="starting" ||
      x?.type?.developer_name==="STARTING";
    return !!starter;
  }).length;
}
function v7714LineupsOfficial(fx,homeRows=[],awayRows=[]){
  homeRows=Array.isArray(homeRows)?homeRows:[];
  awayRows=Array.isArray(awayRows)?awayRows:[];
  const explicit=
    fx?.lineups_confirmed===true ||
    fx?.lineup_confirmed===true ||
    fx?.confirmed===true ||
    fx?.official===true ||
    fx?.metadata?.lineups_confirmed===true ||
    fx?.metadata?.lineups_official===true;

  if(explicit)return true;

  const hs=v7714CountStarters(homeRows), as=v7714CountStarters(awayRows);
  if(hs>=11 && as>=11){
    const start=Date.parse(String(fx?.starting_at||""));
    if(Number.isFinite(start)){
      const diff=start-Date.now();
      // Treat complete XIs as official from 120 minutes before kickoff onward.
      if(diff<=120*60*1000)return true;
    }
    // If already live/ft, complete XIs are certainly official.
    const st=v67FixtureState(fx);
    if(st==="live"||st==="ft")return true;
  }
  return false;
}

PROBABILI FORMAZIONI <span class="v18-toggle">+</span></summary><div class="v14-fold-body v14-lineups">${v14TeamFormation(m.h,"h")}${v14TeamFormation(m.a,"a")}</div></details>
...
/* ============================================================
   V73 — TOP SIGNALS REALI / ZERO DEMO
   ============================================================ */
let V73_SIG={type:"Tutti",league:""};
let V73_SIGNAL_CACHE={date:"",items:[],fixtures:[],meta:null};

function v73SignalMarket(pred){
  const groups=[
    ["1",pred.one],["X",pred.x],["2",pred.two],
    ["GOL",pred.gg],["NO GOL",pred.ng],
    ["OVER 2.5",pred.over],["UNDER 2.5",pred.under]
  ].filter(x=>Number.isFinite(Number(x[1])));
  if(!groups.length)return ["—",null];
  groups.sort((a,b)=>Number(b[1])-Number(a[1]));
  const top=Number(groups[0][1]);
  const tied=groups.filter(x=>Number(x[1])===top);
  if(tied.length===1)return [groups[0][0],top];
  const one=[["1",pred.one],["X",pred.x],["2",pred.two]].sort((a,b)=>Number(b[1])-Number(a[1]))[0];
  return [one[0],Number(one[1])];
}

function v73RealOddsEdge(pack,pred){
  // Value is shown only if a real bookmaker price can be located.
  // No demo/default odds are ever used.
  const candidates=[];
  const scan=x=>{
    if(!x)return;
    if(Array.isArray(x)){x.forEach(scan);return}
    if(typeof x!=="object")return;
    const label=String(x?.label||x?.name||x?.market_description||x?.description||"").toUpperCase();
    const odd=realNum(x?.odd??x?.odds??x?.price);
    if(odd && odd>1.01 && odd<100 && label)candidates.push({label,odd});
    for(const [k,v] of Object.entries(x)){
      if(["participants","statistics","events","lineups","scores"].includes(k))continue;
      if(Array.isArray(v)||(v&&typeof v==="object"))scan(v);
    }
  };
  scan(pack?.odds);
  scan(pack?.bookmakers);
  scan(pack?.markets);
  if(!candidates.length)return null;

  const refs=[
    {pick:"1",prob:pred.one,keys:["HOME","1"]},
    {pick:"X",prob:pred.x,keys:["DRAW","X"]},
    {pick:"2",prob:pred.two,keys:["AWAY","2"]},
    {pick:"GOL",prob:pred.gg,keys:["BOTH TEAMS TO SCORE","BTTS YES","YES"]},
    {pick:"NO GOL",prob:pred.ng,keys:["BTTS NO","NO"]},
    {pick:"OVER 2.5",prob:pred.over,keys:["OVER 2.5","OVER 2,5"]},
    {pick:"UNDER 2.5",prob:pred.under,keys:["UNDER 2.5","UNDER 2,5"]}
  ];
  let best=null;
  for(const r of refs){
    for(const c of candidates){
      if(!r.keys.some(k=>c.label===k||c.label.includes(k)))continue;
      const implied=100/c.odd;
      const edge=Math.round(Number(r.prob)-implied);
      if(edge>=8 && (!best||edge>best.score))best={type:"Value",kind:"value",score:edge,pick:r.pick,prob:r.prob,odd:c.odd,implied:Math.round(implied*10)/10};
    }
  }
  return best;
}

function v77MarketConfidence(pred,pick,hm,am){
  const probs={"1":pred.one,"X":pred.x,"2":pred.two,"GOL":pred.gg,"NO GOL":pred.ng,"OVER 2.5":pred.over,"UNDER 2.5":pred.under};
  const p=Number(probs[pick]);
  if(!Number.isFinite(p))return 0;
  const neutral=["1","X","2"].includes(pick)?33.3:50;
  const separation=Math.min(1,Math.abs(p-neutral)/(pick==="X"?25:35));
  const sample=Math.min(1,Math.min(hm?.sample||0,am?.sample||0)/5);
  const completeness=[hm?.gf,hm?.ga,am?.gf,am?.ga,hm?.xg,am?.xg,hm?.shots,am?.shots].filter(Number.isFinite).length/8;
  let c=Math.round(48+separation*24+sample*18+completeness*10);
  if(sample<.6)c=Math.min(c,64); else if(sample<1)c=Math.min(c,78);
  return Math.max(0,Math.min(92,c));
}
function v77SignalWhy(pick,pred,hm,am,value=null){
  const bits=[];
  const avg=(a,b)=>Number.isFinite(a)&&Number.isFinite(b)?(a+b)/2:null;
  const gf=avg(hm?.gf,am?.gf),ga=avg(hm?.ga,am?.ga),xg=avg(hm?.xg,am?.xg);
  if(pick==="UNDER 2.5"){
    if(gf!=null)bits.push(`produzione recente contenuta (${gf.toFixed(1)} gol fatti medi)`);
    if(xg!=null)bits.push(`xG medio ${xg.toFixed(2)}`);
  }else if(pick==="OVER 2.5"||pick==="GOL"){
    if(gf!=null)bits.push(`produzione offensiva recente ${gf.toFixed(1)} gol medi`);
    if(ga!=null)bits.push(`difese concedono ${ga.toFixed(1)} gol medi`);
    if(xg!=null)bits.push(`xG medio ${xg.toFixed(2)}`);
  }else if(pick==="NO GOL"){
    if(ga!=null)bits.push(`tenuta difensiva recente (${ga.toFixed(1)} gol subiti medi)`);
  }else{
    const fs=(hm?.formScore!=null&&am?.formScore!=null)?Math.round(Math.abs(hm.formScore-am.formScore)):null;
    if(fs!=null&&fs>=10)bits.push(`differenza di forma recente di ${fs} punti`);
    if(xg!=null)bits.push(`profilo xG coerente con l'esito`);
  }
  if(value?.odd)bits.push(`quota ${Number(value.odd).toFixed(2)} contro probabilità modello ${Math.round(value.prob)}%`);
  if(!bits.length)bits.push("più indicatori disponibili convergono sullo stesso mercato");
  return bits.slice(0,3).join(" · ");
}

async function v73BuildSignal(f){
  try{
    const src=await v69SafeAnalysisPack(f.id);
    const pack=src.pack||{};
    const h=pack.teams?.home||f.h||realLoc(f,"home")||{};
    const a=pack.teams?.away||f.a||realLoc(f,"away")||{};
    if(!h?.id||!a?.id)return null;

    const hfs=realHistory(pack,"home_history"), afs=realHistory(pack,"away_history");
    const hm=v58Metrics(hfs,h), am=v58Metrics(afs,a);
    const hsplit=v58Metrics(hfs,h,"home"), asplit=v58Metrics(afs,a,"away");
    const minSample=Math.min(hm.sample||0,am.sample||0);

    const pred=v59ShrinkPrediction(v58Prediction(hm,am,hsplit,asplit),hm,am);
    const formEdge=(hm.formScore!=null&&am.formScore!=null)?Math.abs(hm.formScore-am.formScore):null;
    const [pick,prob]=v73SignalMarket(pred);
    const confidence=v77MarketConfidence(pred,pick,hm,am);
    const signals=[];

    // Confidence is specific to the SAME market shown as Top Signal.
    if(minSample>=3 && confidence>=72){
      signals.push({type:"Confidence",kind:"confidence",score:confidence,pick,prob,reason:v77SignalWhy(pick,pred,hm,am)});
    }

    // Form requires at least three usable matches for both teams.
    if(minSample>=3 && formEdge!=null && formEdge>=25){
      signals.push({type:"Forma",kind:"form",score:Math.round(formEdge),pick,prob});
    }

    // Value exists only if the real SportMonks payload contains usable bookmaker odds.
    const value=v73RealOddsEdge(pack,pred);
    if(minSample>=3 && value){
      const valueConfidence=v77MarketConfidence(pred,value.pick,hm,am);
      if(valueConfidence>=72){
        value.confidence=valueConfidence;
        value.reason=v77SignalWhy(value.pick,pred,hm,am,value);
        signals.push(value);
      }
    }
    ...
    return {
      id:String(f.id),
      fixture:f,
      h,a,pred,confidence,formEdge,minSample,
      league:f.leagueName||f?.league?.name||pack?.fixture?.league?.name||"",
      time:fmtTime(f.starting_at)||f.time||"",
      signals,
      pick,prob
    };
  }catch(_){
...
function v73LeagueKey(name){
  return String(name||"").toLowerCase().replace(/\s+/g,"-");
}

function v73LeagueIcon(meta,name){
  const c=v60CompFromName(meta,name);
  if(c)return v60LeagueLogo(c,true);
  return `<span class="v60-league-logo small">${realEsc(String(name||"?").slice(0,2).toUpperCase())}</span>`;
}

function v73SignalBadges(signals){
  return signals.map(s=>`<span class="${s.kind}">${signalLabel(s)} <b>${s.type==="Value"?"+"+s.score+" p.p.":s.type==="Forma"?"+"+s.score:s.score+"%"}</b></span>`).join("");
}

function v73SignalCard(m){
  const c=m.signals.find(s=>s.type==="Confidence");
  const v=m.signals.find(s=>s.type==="Value");
  const f=m.signals.find(s=>s.type==="Forma");
  return `<button class="v41-signal-card v73-signal-card" data-v73-signal-match="${realEsc(m.id)}">
    <div class="v41-card-top">
      <span>${realEsc(m.league)}</span><b>${realEsc(m.time)}</b>
    </div>
    <div class="v41-match">
      <div class="v41-team">${realCrest(m.h)}<strong>${realEsc(m.h.name||"")}</strong></div>
      <div class="v41-pick"><small>TOP SIGNAL</small><b>${realEsc(m.pick)}</b><em>${m.prob==null?"—":Math.round(m.prob)+"%"}</em></div>
      <div class="v41-team">${realCrest(m.a)}<strong>${realEsc(m.a.name||"")}</strong></div>
    </div>
    <div class="v41-metrics">
      <div><small>CONFIDENCE</small><b class="${c?"green":""}">${c?c.score+"%":"—"}</b></div>
      <div><small>VALUE</small><b class="${v?"orange":""}">${v?"+"+v.score+" p.p.":"—"}</b></div>
      <div><small>FORMA</small><b class="${f?"blue":""}">${f?"+"+f.score:"—"}</b></div>
    </div>
    <div class="v41-reasons">
      ${v73SignalBadges(m.signals)}
      <em>ANALISI ›</em>
    </div>
    <div class="v77-signal-why"><b>PERCHÉ QUESTO SEGNALE</b><p>${realEsc((v||c||f)?.reason||v77SignalWhy(m.pick,m.pred,{},{}))}</p>${v?.odd?`<small>Quota ${Number(v.odd).toFixed(2)} · implicita ${v.implied}% · modello ${Math.round(v.prob)}% · edge +${v.score} p.p.</small>`:""}</div>
  </button>`;
}

function v73RenderSignals(){
  const list=document.querySelector("#sigList");
  const count=document.querySelector("#sigCount");
  if(!list||!count)return;
  const rows=V73_SIGNAL_CACHE.items.filter(m=>{
    const typeOk=V73_SIG.type==="Tutti"||m.signals.some(s=>s.type===V73_SIG.type);
    const leagueOk=!V73_SIG.league||m.league===V73_SIG.league;
    return typeOk&&leagueOk;
  });
  count.textContent=rows.length+" "+(rows.length===1?"partita":"partite");
  list.innerHTML=rows.length
    ?rows.map(v73SignalCard).join("")
    :`<div class="v73-empty-signals">
       <span>◎</span>
       <b>NESSUN TOP SIGNAL DISPONIBILE</b>
       <p>Il modello non ha rilevato segnali con dati sufficienti per questa giornata.</p>
     </div>`;
  document.querySelectorAll("[data-v73-signal-match]").forEach(b=>b.onclick=()=>route("match",b.dataset.v73SignalMatch));
}

function v73BindSignalFilters(){
  document.querySelectorAll("[data-v73-type]").forEach(b=>b.onclick=()=>{
    V73_SIG.type=b.dataset.v73Type;
    document.querySelectorAll("[data-v73-type]").forEach(x=>x.classList.toggle("active",x.dataset.v73Type===V73_SIG.type));
    v73RenderSignals();
  });
  document.querySelectorAll("[data-v73-league]").forEach(b=>b.onclick=()=>{
    const l=b.dataset.v73League;
    V73_SIG.league=V73_SIG.league===l?"":l;
    document.querySelectorAll("[data-v73-league]").forEach(x=>x.classList.toggle("active",x.dataset.v73League===V73_SIG.league));
    v73RenderSignals();
  });
}

async function realSignalsPage(){
  const root=document.querySelector("#main");
  const initial=dashDateMeta().id;
  root.innerHTML=`<div class="page v41-signals-page v73-signals-page">
    <div class="v64-signal-head"><h1>TOP SIGNALS</h1><div class="eyebrow">SIGNAL CENTER</div></div>
    <div class="v73-signal-loading">Analisi dei segnali reali SportMonks…</div>
  </div>`;
  bind();

  try{
    let date=initial, fixtures=await v68SafeFixtures(date);
    // On the first automatic opening we may still helpfully move to the next
    // matchday. Once the user changes the date, Top Signals must respect it.
    if(!fixtures.length && !DASH_DATE_MANUAL){
      const next=await v70NextAvailableDate(date);
      if(next){
        v70SetDashDate(next);
        date=next;
        fixtures=await v68SafeFixtures(date);
      }
    }
    const meta=await v60CompetitionMeta().catch(()=>({competitions:[]}));

    // Limit concurrent calls so the Worker is not hammered on larger matchdays.
    const items=[];
    const queue=fixtures.slice();
    const workers=Array.from({length:Math.min(4,queue.length)},async()=>{
      while(queue.length){
        const f=queue.shift();
        const s=await v73BuildSignal(f);
        if(s)items.push(s);
      }
    });
    await Promise.all(workers);
    items.sort((a,b)=>{
      const ac=a.signals.find(s=>s.type==="Confidence")?.score||0;
      const bc=b.signals.find(s=>s.type==="Confidence")?.score||0;
      return bc-ac || String(a.time).localeCompare(String(b.time));
    });
    V73_SIGNAL_CACHE={date,items,fixtures,meta};
    // Freeze every Top Signals fixture now, before kickoff. Existing snapshots are immutable.
    items.forEach(v7711SaveSignalSnapshot);
    V73_SIG={type:"Tutti",league:""};

    root.innerHTML=`<div class="page v41-signals-page v73-signals-page">
      <div class="v64-signal-head">
        <h1>TOP SIGNALS</h1>
        <div class="eyebrow">SIGNAL CENTER</div>
        <span class="v41-signal-count">${items.length} SEGNALI</span>
      </div>
      <p class="subtitle">Solo segnali generati da partite e dati reali SportMonks · ${realEsc(date.split("-").reverse().join("/"))}</p>

      <div class="v712-date-switcher v7710-signal-date">
        <button data-v7710-signal-day="-1" aria-label="Giorno precedente">‹</button>
        <div>
          <small>DATA SEGNALI</small>
          <strong>${realEsc(dashDateMeta().label)}</strong>
        </div>
        <button data-v7710-signal-day="1" aria-label="Giorno successivo">›</button>
      </div>

      <details class="v46-signal-guide">
        <summary><span>COME LEGGERE I SEGNALI</span><em>+</em></summary>
        <div class="v46-guide-grid">
          <div><b>CONFIDENCE</b><p>Compare solo quando la solidità del modello raggiunge almeno il 70% con un campione utilizzabile.</p></div>
          <div><b>VALUE</b><p>Compare solo quando sono disponibili quote reali e il vantaggio stimato è almeno di +8 punti percentuali, con Confidence specifica del mercato almeno 72%. Nessuna quota demo.</p></div>
          <div><b>FORMA</b><p>Compare solo con un campione minimo e un differenziale recente abbastanza marcato.</p></div>
        </div>
      </details>

      <div class="top-filter-grid v41-filters">
        <button class="filter active" data-v73-type="Tutti">Tutti</button>
        <button class="filter confidence" data-v73-type="Confidence">Confidence</button>
        <button class="filter value" data-v73-type="Value">Value</button>
        <button class="filter form" data-v73-type="Forma">Forma</button>
      </div>

      <div class="v73-league-filter">
        ${REAL_LEAGUES.map(l=>`<button data-v73-league="${realEsc(l)}">${v73LeagueIcon(meta,l)}<span>${realEsc(l)}</span></button>`).join("")}
      </div>
    ...
    bind();
    v73BindSignalFilters();
    v73RenderSignals();
    document.querySelectorAll("[data-v7710-signal-day]").forEach(b=>b.onclick=()=>{
      v712MoveDate(Number(b.dataset.v7710SignalDay));
      realSignalsPage();
    });
  }catch(e){
    root.innerHTML=`<div class="page v41-signals-page v73-signals-page">
      <div class="v64-signal-head"><h1>TOP SIGNALS</h1><div class="eyebrow">SIGNAL CENTER</div></div>
      <div class="v73-empty-signals"><span>!</span><b>DATI TEMPORANEAMENTE NON DISPONIBILI</b><p>${realEsc(e.message)}</p></div>
    </div>`;
    bind();
  }
}
...
function renderSig(){
 ...
 document.querySelector("#sigList").innerHTML=matches.length?matches.map(m=>{
   ...
   return `<button class="v41-signal-card" data-signal-match="${m.id}" data-signal-type="${SIG.type}">
     ...
     <div class="v41-reasons">
       ${selected.map(s=>`<span class="${s.kind}">${signalLabel(s)} <b>${s.type==="Value"?"+"+s.score+" p.p.":s.type==="Forma"?"+"+s.score:s.score+"%"}</b></span>`).join("")}
       <em>ANALISI ›</em>
     </div>
   </button>`;
 ...
 document.querySelector("#main").innerHTML=`<div class="page v43-signal-detail">
  ...
  <section class="v19-card v43-signal-metrics">
    ...
    <div class="v43-metric-grid">
      ...
      <div><small>VALUE</small><b class="orange">${v?"+"+v.score+" p.p.":"—"}</b></div>
...
function v7723CleanLeagueMark(name=""){
  const n=String(name).toLowerCase();
  let txt="L";
  if(n.includes("premier")) txt="PL";
  else if(n.includes("serie a")) txt="A";
  else if(n.includes("la liga")||n.includes("laliga")) txt="LL";
  else if(n.includes("bundesliga")) txt="BL";
  else if(n.includes("ligue 1")) txt="L1";
  const safe=realEsc(txt);
  return `<span class="v7723-clean-league-mark" aria-hidden="true">${safe}</span>`;
}


function v7726FilterLeagueLogo(f){
  const name=String(f?.leagueName||f?.league?.name||"").trim();

  try{
    const base=V60_COMPETITIONS.find(
      x=>String(x?.name||"").toLowerCase()===name.toLowerCase()
    );

    const metaComp=(V60_META_CACHE?.competitions||[]).find(
      x=>String(x?.key||"")===String(base?.key||"")
    );

    const c=base ? {...base,...(metaComp||{})} : null;

    if(c?.image_path){
      return v60LeagueLogo(c,true);
    }
  }catch{}
...
function v7721LeagueBadge(f){
  const name=f?.leagueName||f?.league?.name||"";
  return `<div class="v7721-league-badge">${v7726FilterLeagueLogo(f)}<span>${realEsc(name)}</span></div>`;
}

function v7717MatchCard(f,{league=true,action=true,compact=false}={}){
  const h=f?.h||realLoc(f,"home")||{};
  const a=f?.a||realLoc(f,"away")||{};
  const st=v67FixtureState(f);
  const hs=realScore(f,h.id), as=realScore(f,a.id);
  const time=realEsc(f?.time||fmtTime(f?.starting_at)||"");

  let centerTop="";
  let centerMain="VS";
  let centerBottom=time;
  let liveClass="";
  ...
  if(st==="ft"){
    ...
    centerTop="LIVE";
    centerMain=`${hs??0}–${as??0}`;
    centerBottom="";
    liveClass=" live";
  }

  return `<article class="v7717-match-card ${compact?"compact":""}" data-real-match="${f.id}">
    ${league?`<header class="v7721-card-league">${v7721LeagueBadge(f)}</header>`:""}

    <div class="v7717-match-body">
      <div class="v7717-team">
        <div class="v7717-crest">${realCrest(h)}</div>
        <div class="v7717-name">${realEsc(h?.name||"—")}</div>
      </div>

      <div class="v7719-center-block${liveClass}">
        ${centerTop?`<div class="v7719-center-state">${centerTop}</div>`:""}
        <div class="v7719-center-score">${centerMain}</div>
        ${centerBottom?`<div class="v7719-center-time">${centerBottom}</div>`:""}
        ${action?`<button class="v7719-inline-action" data-real-match="${f.id}">${st==="ft"?"POST-MATCH":"ANALISI"} <span>›</span></button>`:""}
      </div>

      <div class="v7717-team">
        <div class="v7717-crest">${realCrest(a)}</div>
        <div class="v7717-name">${realEsc(a?.name||"—")}</div>
      </div>
    </div>
  </article>`;
}

async function v67MatchCenter(){
  const date=dashDateMeta().id;
  document.querySelector("#main").innerHTML=`<div class="page v67-center-page">
    <header class="v67-center-head"><h1>MATCH CENTER</h1><p>SPORTMONKS REAL DATA · ${realEsc(dashDateMeta().label)}</p></header>
    <div class="v67-center-filters">
      <button class="active" data-v67-filter="all">TUTTE</button>
      <button data-v67-filter="live">LIVE</button>
      <button data-v67-filter="upcoming">PROGRAMMATE</button>
      <button data-v67-filter="ft">FINITE</button>
    </div>
    <section class="v67-center-list"><div class="empty-note">Caricamento partite…</div></section>
  </div>`; bind();
  try{
    const [dayFs,livePack]=await Promise.all([realFixtures(date),realApi("/live-fixtures").catch(()=>({data:[]})),v60CompetitionMeta().catch(()=>({competitions:[]}))]);
    const liveRaw=(Array.isArray(livePack?.data)?livePack.data:Array.isArray(livePack?.data?.data)?livePack.data.data:[]).map(realNormFixture).filter(f=>REAL_LEAGUES.includes(f.leagueName));
    const map=new Map();
    [...dayFs,...liveRaw].forEach(f=>map.set(String(f.id),f));
    const all=[...map.values()].sort((a,b)=>String(a.starting_at||"").localeCompare(String(b.starting_at||"")));
    const list=document.querySelector(".v67-center-list");
    const render=(filter="all")=>{
      const arr=filter==="all"?all:all.filter(f=>v67FixtureState(f)===filter);
      const groups=REAL_LEAGUES.map(l=>[l,arr.filter(f=>f.leagueName===l)]).filter(x=>x[1].length);
      list.innerHTML=groups.length?groups.map(([l,fs])=>`<section class="v67-center-league"><h2>${realEsc(l)}</h2>${fs.map(f=>{
        const h=f.h||realLoc(f,"home")||{},a=f.a||realLoc(f,"away")||{};
        return v7717MatchCard(f,{league:true,action:false,compact:true})}).join("")}</section>`).join(""):`<div class="empty-note">Nessuna partita per questo filtro.</div>`;
      list.querySelectorAll("[data-real-match]").forEach(b=>b.onclick=()=>route("match",b.dataset.realMatch));
    };
    render("all");
    document.querySelectorAll("[data-v67-filter]").forEach(b=>b.onclick=()=>{
      document.querySelectorAll("[data-v67-filter]").forEach(x=>x.classList.toggle("active",x===b));
      render(b.dataset.v67Filter);
    });
  }catch(e){
    document.querySelector(".v67-center-list").innerHTML=`<div class="empty-note">${realEsc(e.message)}</div>`;
  }
}
function more(){
 ...
 <button class="more-row2 v14-more-live" data-route="live"><b><i></i> Match Center</b><span>Tutte · Live · Programmate · Finite ›</span></button><button class="more-row2" data-route="standings"><b>▤ Classifiche</b><span>5 campionati · live ›</span></button>
...
/* ============================================================
   V77.11 — REAL MODEL PERFORMANCE TRACKER
   Snapshots are written only pre-match and never overwritten.
   Only FT fixtures enter the observed performance statistics.
   ============================================================ */
const V7711_LEDGER_KEY="space16154:performance:v77.11";

function v7711Ledger(){
  try{
    const x=JSON.parse(localStorage.getItem(V7711_LEDGER_KEY)||"{}");
    return x&&typeof x==="object"?x:{};
  }catch{return {}}
}
function v7711WriteLedger(x){
  try{localStorage.setItem(V7711_LEDGER_KEY,JSON.stringify(x))}catch{}
}
function v7711FixtureDate(f){
  const s=String(f?.starting_at||"");
  return /^\d{4}-\d{2}-\d{2}/.test(s)?s.slice(0,10):dashDateMeta().id;
}
function v7711BestPick(pred,group){
  const rows=group==="1X2"
    ?[["1",pred?.one],["X",pred?.x],["2",pred?.two]]
    :group==="OU"
      ?[["OVER 2.5",pred?.over],["UNDER 2.5",pred?.under]]
      :[["GOL",pred?.gg],["NO GOL",pred?.ng]];
  const valid=rows.filter(x=>Number.isFinite(Number(x[1]))).sort((a,b)=>Number(b[1])-Number(a[1]));
  return valid[0]?{pick:valid[0][0],prob:Number(valid[0][1])}:null;
}
function v7711SaveSignalSnapshot(item){
  if(!item?.id||!item?.fixture||!item?.pred)return;
  // Never create/replace a snapshot after the match has started.
  if(v67FixtureState(item.fixture)!=="upcoming")return;
  const ledger=v7711Ledger(),id=String(item.id);
  if(ledger[id])return;
  const oneXTwo=v7711BestPick(item.pred,"1X2");
  const triplet={
    "1":Number(item.pred?.one),
    "X":Number(item.pred?.x),
    "2":Number(item.pred?.two)
  };
  if(oneXTwo && Object.values(triplet).every(Number.isFinite)){
    oneXTwo.probs={...triplet};
    oneXTwo.probabilities={...triplet};
    oneXTwo.p1=triplet["1"];
    oneXTwo.px=triplet["X"];
    oneXTwo.p2=triplet["2"];
  }
  const markets={
    "1X2":oneXTwo,
    "Over/Under":v7711BestPick(item.pred,"OU"),
    "Gol/No Gol":v7711BestPick(item.pred,"GG")
  };
  ledger[id]={
    id,
    date:v7711FixtureDate(item.fixture),
    starting_at:item.fixture?.starting_at||null,
    league:item.league||"",
    home:item.h?.name||"",
    away:item.a?.name||"",
    markets,
    topSignals:(item.signals||[]).map(s=>({
      type:s.type,
      pick:s.pick,
      prob:Number(s.prob),
      confidence:Number(s.type==="Confidence"?s.score:(s.confidence??item.confidence)),
      score:Number(s.score),
      odd:s.odd??null
    })),
    primary:{pick:item.pick,prob:Number(item.prob),confidence:Number(item.confidence)},
    saved_at:Date.now()
  };
  v7711WriteLedger(ledger);
}

function v7734HasOneXTwoTriplet(row){
  const p=row?.markets?.["1X2"];
  if(!p)return false;
  const vals=[
    Number(p?.probs?.["1"] ?? p?.probabilities?.["1"] ?? p?.p1),
    Number(p?.probs?.["X"] ?? p?.probabilities?.["X"] ?? p?.px),
    Number(p?.probs?.["2"] ?? p?.probabilities?.["2"] ?? p?.p2)
  ];
  return vals.every(Number.isFinite);
}

function v7734TripletFromPred(pred){
  const t={"1":Number(pred?.one),"X":Number(pred?.x),"2":Number(pred?.two)};
  return Object.values(t).every(Number.isFinite)?t:null;
}

function v7734ApplyTripletOnly(row,pred){
  if(!row?.markets?.["1X2"])return false;
  if(v7734HasOneXTwoTriplet(row))return false;
  const t=v7734TripletFromPred(pred);
  if(!t)return false;

  // Preserve the frozen pick/probability. Add only the missing 1/X/2 vector.
  const p=row.markets["1X2"];
  p.probs={...t};
  p.probabilities={...t};
  p.p1=t["1"]; p.px=t["X"]; p.p2=t["2"];
  row.triplet_backfilled_at=Date.now();
  row.triplet_source="pre-match-backfill-v77.34";
  return true;
}

function v7711Outcome(f){
  const h=f?.h||realLoc(f,"home")||{},a=f?.a||realLoc(f,"away")||{};
  const hs=realScore(f,h.id),as=realScore(f,a.id);
  if(hs==null||as==null)return null;
  return {hs,as,result:hs>as?"1":hs<as?"2":"X",goals:hs+as};
}
function v7711PickHit(pick,o){
  if(!o||!pick)return null;
  if(["1","X","2"].includes(pick))return pick===o.result;
  if(pick==="GOL")return o.hs>0&&o.as>0;
  if(pick==="NO GOL")return !(o.hs>0&&o.as>0);
  if(pick==="OVER 2.5")return o.goals>=3;
  if(pick==="UNDER 2.5")return o.goals<=2;
  return null;
}
function v7711Stat(){return {hit:0,total:0}}
function v7711Add(stat,hit){
  if(hit===null)return;
  stat.total++; if(hit)stat.hit++;
}
function v7711Pct(s){return s.total?Math.round(s.hit/s.total*1000)/10:null}
function v7711StatText(s){
  const p=v7711Pct(s);
  return {ratio:`${s.hit}/${s.total}`,pct:p==null?"—":`${String(p).replace(".",",")}%`};
}

function v7731IsNoPick(pick,prob){
  const p=Number(prob);
  if(!Number.isFinite(p))return false;
  const binary=["GOL","NO GOL","OVER 2.5","UNDER 2.5"];
  return binary.includes(String(pick||"").toUpperCase()) && Math.abs(p-50)<0.0001;
}
function v7731BandKey(prob){
  const p=Number(prob);
  if(!Number.isFinite(p))return null;
  if(p<=50)return "≤50";
  if(p<=55)return "51–55";
  if(p<=60)return "56–60";
  if(p<=70)return "61–70";
  if(p<=80)return "71–80";
  return "81+";
}
function v7731BandMap(){
  return {
    "≤50":{total:v7711Stat(),markets:{"1X2":v7711Stat(),"Over/Under":v7711Stat(),"Gol/No Gol":v7711Stat()}},
    "51–55":{total:v7711Stat(),markets:{"1X2":v7711Stat(),"Over/Under":v7711Stat(),"Gol/No Gol":v7711Stat()}},
    "56–60":{total:v7711Stat(),markets:{"1X2":v7711Stat(),"Over/Under":v7711Stat(),"Gol/No Gol":v7711Stat()}},
    "61–70":{total:v7711Stat(),markets:{"1X2":v7711Stat(),"Over/Under":v7711Stat(),"Gol/No Gol":v7711Stat()}},
    "71–80":{total:v7711Stat(),markets:{"1X2":v7711Stat(),"Over/Under":v7711Stat(),"Gol/No Gol":v7711Stat()}},
    "81+":{total:v7711Stat(),markets:{"1X2":v7711Stat(),"Over/Under":v7711Stat(),"Gol/No Gol":v7711Stat()}}
  };
}


function v7733OneXTwoTriplet(source){
  if(!source)return null;

  // Accept the common probability containers used across SPACE16154.
  const candidates=[
    source?.probabilities,
    source?.oneXTwo,
    source?.markets?.["1X2"],
    source?.prediction?.["1X2"],
    source?.probs
  ].filter(Boolean);

  for(const c of candidates){
    if(Array.isArray(c) && c.length>=3){
      const vals=c.slice(0,3).map(Number);
      if(vals.every(Number.isFinite)) return {"1":vals[0],"X":vals[1],"2":vals[2]};
    }
    if(c && typeof c==="object"){
      const p1=Number(c["1"] ?? c.home ?? c.p1 ?? c.homeProb);
      const px=Number(c["X"] ?? c.draw ?? c.px ?? c.drawProb);
      const p2=Number(c["2"] ?? c.away ?? c.p2 ?? c.awayProb);
      if([p1,px,p2].every(Number.isFinite)) return {"1":p1,"X":px,"2":p2};
    }
  }

  // Direct fields fallback on the source itself.
  const p1=Number(source["1"] ?? source.home ?? source.p1 ?? source.homeProb);
  const px=Number(source["X"] ?? source.draw ?? source.px ?? source.drawProb);
  const p2=Number(source["2"] ?? source.away ?? source.p2 ?? source.awayProb);
  if([p1,px,p2].every(Number.isFinite)) return {"1":p1,"X":px,"2":p2};
...
function v7733EnrichSnapshotOneXTwo(snapshot,source){
  if(!snapshot||!snapshot.markets||!snapshot.markets["1X2"])return snapshot;
  const triplet=v7733OneXTwoTriplet(source)||v7733OneXTwoTriplet(snapshot);
  if(triplet){
    snapshot.markets["1X2"].probs=triplet;
    snapshot.markets["1X2"].probabilities=triplet;
    snapshot.markets["1X2"].p1=triplet["1"];
    snapshot.markets["1X2"].px=triplet["X"];
    snapshot.markets["1X2"].p2=triplet["2"];
  }
  return snapshot;
}

function v7732OneXTwoMargin(p){
  if(!p)return null;

  // Prefer a stored triplet if the snapshot already contains it.
  const raw =
    p.probs || p.probabilities || p.vector ||
    (p.oneXTwo ? p.oneXTwo : null);

  let vals=[];
  if(Array.isArray(raw)){
    vals=raw.map(Number).filter(Number.isFinite);
  }else if(raw && typeof raw==="object"){
    vals=["1","X","2","home","draw","away"].map(k=>Number(raw[k])).filter(Number.isFinite);
  }

  // Common explicit fields fallback.
  if(vals.length<3){
    const explicit=[
      p.p1,p.px,p.p2,
    ...
    if(explicit.length>=3) vals=explicit.slice(0,3);
  }

  if(vals.length<3)return null;
  const sorted=[...vals].sort((a,b)=>b-a);
  return +(sorted[0]-sorted[1]).toFixed(2);
}
function v7732OneXTwoMarginBand(m){
  const x=Number(m);
...
const V7713_AUTO_KEY="space16154:performance:v77.13:auto-state";
const V7713_DIRECT_BACKFILL_KEY="space16154:performance:v77.13:direct-backfill";

function v7713AutoState(){
  try{
    const x=JSON.parse(localStorage.getItem(V7713_AUTO_KEY)||"{}");
    return x&&typeof x==="object"?x:{};
  }catch{return {}}
}
function v7713WriteAutoState(x){
  try{localStorage.setItem(V7713_AUTO_KEY,JSON.stringify(x))}catch{}
}
function v7713LocalDate(offset=0){
  const d=new Date();
  d.setHours(12,0,0,0);
...
function v7713LegacyPredictionIds(){
  const ids=[];
  try{
    for(let i=0;i<localStorage.length;i++){
      const k=localStorage.key(i)||"";
      const m=k.match(/^space16154:prematch:v76:(\d+)$/);
      if(m)ids.push(m[1]);
    }
  }catch{}
  return [...new Set(ids)];
}
async function v7713DirectHistoricalBackfill(){
  // Re-scan is harmless: ledger rows are immutable by fixture id.
  const ledger=v7711Ledger();
  const ids=v7713LegacyPredictionIds();
  let imported=0,checked=0,usable=0;

  for(const id of ids.slice(0,40)){
    if(ledger[id])continue;
    const frozen=v68LoadPrediction(id);
    if(!frozen?.pred)continue;
    checked++;
    let f=null;
    try{f=await v68FixtureDetail(id)}catch{}
    if(!f||v67FixtureState(f)!=="ft")continue;
    usable++;
    const row=v7712SnapshotFromFrozen(f,frozen);
    if(!row)continue;
    ledger[id]=row;
    imported++;
  }
  if(imported)v7711WriteLedger(ledger);
  try{
    localStorage.setItem(V7713_DIRECT_BACKFILL_KEY,JSON.stringify({
      last_run:Date.now(),legacy_keys:ids.length,checked,usable,imported
    }));
  }catch{}
  return {legacy_keys:ids.length,checked,usable,imported};
}

async function v7713BuildTrackingItem(f){
  try{
    const src=await v69SafeAnalysisPack(f.id);
    const pack=src.pack||{};
    const h=pack.teams?.home||f.h||realLoc(f,"home")||{};
    ...
    if(minSample>=3 && confidence>=72){
      signals.push({
        type:"Confidence",kind:"confidence",score:confidence,pick,prob,
        reason:v77SignalWhy(pick,pred,hm,am)
      });
    }
    if(minSample>=3 && formEdge!=null && formEdge>=25){
      signals.push({type:"Forma",kind:"form",score:Math.round(formEdge),pick,prob});
    ...
    return {
      id:String(f.id),fixture:f,h,a,pred,confidence,formEdge,minSample,
      league:f.leagueName||f?.league?.name||pack?.fixture?.league?.name||"",
      time:fmtTime(f.starting_at)||f.time||"",signals,pick,prob
    };
  }catch{return null}
}


function v7714BusyRoute(){
  try{
    const h=(location.hash||"#dashboard").replace("#","");
    return ["matches","analysis","team","fixture","live"].some(x=>h.startsWith(x));
  }catch{return false}
}
const v7714Sleep=ms=>new Promise(r=>setTimeout(r,ms));


const V7735_TRIPLET_STATUS_KEY="space16154:performance:v77.35:future-triplets";

function v7735ReadTripletStatus(){
  try{
    const x=JSON.parse(localStorage.getItem(V7735_TRIPLET_STATUS_KEY)||"{}");
    return x&&typeof x==="object"?x:{};
  }catch{return {}}
}
function v7735WriteTripletStatus(x){
  try{localStorage.setItem(V7735_TRIPLET_STATUS_KEY,JSON.stringify(x))}catch{}
}

async function v7735EnsureFutureTriplets(){
  const status={
    running:true,
    last_run:Date.now(),
    future_seen:0,
  ...
  v7735WriteTripletStatus(status);

  try{
    const dates=[0,1,2,3].map(v7713LocalDate);
    const ledger=v7711Ledger();

    for(const date of dates){
      let fs=[];
      try{fs=await v68SafeFixtures(date)}catch{status.failed++;continue}

      for(const f of fs){
        if(!f?.id || v67FixtureState(f)!=="upcoming")continue;
        status.future_seen++;

        const id=String(f.id);
        const row=ledger[id];

        // Only enrich predictions that were already frozen before kick-off.
        if(!row)continue;
        status.frozen_future++;

        if(v7734HasOneXTwoTriplet(row)){
          status.already_ready++;
          continue;
        }

        try{
          const item=await v7713BuildTrackingItem(f);
          if(item && v7734ApplyTripletOnly(row,item.pred)){
            ledger[id]=row;
        ...
        await v7714Sleep(250);
      }
    }

    if(status.completed_now)v7711WriteLedger(ledger);

    // Verify from the saved ledger, not from in-memory assumptions.
    const verify=v7711Ledger();
    let ready=0,missing=0;
    for(const row of Object.values(verify)){
      if(!row?.starting_at)continue;
      const t=new Date(row.starting_at).getTime();
      if(!Number.isFinite(t) || t<=Date.now())continue;
      if(v7734HasOneXTwoTriplet(row))ready++;
      else missing++;
    }
    status.ready_total=ready;
    status.missing_after=missing;
  }catch{
    status.failed++;
  }
  ...
  // Also mirror it into tracker state for one consolidated diagnostics box.
  try{
    const s=v7713AutoState();
    s.triplets_backfilled=Number(s.triplets_backfilled||0)+Number(status.completed_now||0);
    s.triplets_ready=Number(status.ready_total||0);
    s.triplets_missing=Number(status.missing_after||0);
    s.triplets_last_check=status.finished_at;
    v7713WriteAutoState(s);
  }catch{}

  return status;
}

async function v7714AutomaticTracker(){
  const prev=v7713AutoState();
  if(prev.last_run && Date.now()-prev.last_run<30*60*1000)return prev;

  // Do not compete with a user opening Matches/Analysis.
  for(let i=0;i<12 && v7714BusyRoute();i++) await v7714Sleep(2500);

  const state={
    running:true,last_run:Date.now(),dates:[],fixtures_seen:0,
    upcoming_seen:0,snapshots_before:Object.keys(v7711Ledger()).length,
    snapshots_after:0,written:0,errors:0
  };
  v7713WriteAutoState(state);

  try{
    await v7713DirectHistoricalBackfill();
    ...
    for(const date of dates){
      while(v7714BusyRoute()) await v7714Sleep(1800);
      try{
        const fs=await v68SafeFixtures(date);
        state.fixtures_seen+=fs.length;
        for(const f of fs){
          if(v67FixtureState(f)==="upcoming")queue.push(f);
        }
      }catch{state.errors++}
      await v7714Sleep(500);
    }

    state.upcoming_seen=queue.length;

    // Single worker only. Existing snapshots are skipped before heavy analysis.
    while(queue.length){
      while(v7714BusyRoute()) await v7714Sleep(1800);
      const f=queue.shift();
      if(!f?.id)continue;
      ...
      const id=String(f.id);
      const currentLedger=v7711Ledger();
      const existing=currentLedger[id];

      if(existing){
        // V77.34: enrich only FUTURE frozen snapshots that predate triplet storage.
        if(!v7734HasOneXTwoTriplet(existing)){
          const item=await v7713BuildTrackingItem(f);
          if(item && v7734ApplyTripletOnly(existing,item.pred)){
            currentLedger[id]=existing;
            v7711WriteLedger(currentLedger);
            state.triplets_backfilled=(state.triplets_backfilled||0)+1;
          }
          await v7714Sleep(900);
        }
        continue;
      }

      const before=Object.keys(currentLedger).length;
      const item=await v7713BuildTrackingItem(f);
      if(item)v7711SaveSignalSnapshot(item);
      const after=Object.keys(v7711Ledger()).length;
      if(after>before)state.written+=after-before;
  ...
  state.snapshots_after=Object.keys(v7711Ledger()).length;
  v7713WriteAutoState(state);
  return state;
}

async function v7713AutomaticTracker(){
  // One scan every 30 minutes per browser/device is sufficient. It runs
  // independently from which page the user opens.
  const prev=v7713AutoState();
  if(prev.last_run && Date.now()-prev.last_run<30*60*1000)return prev;
  ...
  try{
    // Recover old frozen pre-match tests directly by fixture id first.
    await v7713DirectHistoricalBackfill();

    // Track today + the next 3 calendar days. This covers Friday/Saturday
    // without requiring Top Signals to be opened.
    const dates=[0,1,2,3].map(v7713LocalDate);
    state.dates=dates;
    const queue=[];
    for(const date of dates){
    ...
    // Low concurrency: background work must not hammer the Worker.
    const workers=Array.from({length:Math.min(2,queue.length)},async()=>{
      while(queue.length){
        const f=queue.shift();
        const before=Object.keys(v7711Ledger()).length;
        const item=await v7713BuildTrackingItem(f);
        if(item)v7711SaveSignalSnapshot(item);
        const after=Object.keys(v7711Ledger()).length;
        if(after>before)state.written+=after-before;
...
const V7712_BACKFILL_KEY="space16154:performance:v77.12:backfill";

function v7712OldFrozenPrediction(id){
  // V76+ pre-match snapshots already stored on this same device/browser.
  return v68LoadPrediction(id);
}
function v7712SnapshotFromFrozen(f,frozen){
  if(!f?.id||!frozen?.pred)return null;
  const pred=frozen.pred;
  const conf=Number(frozen.conf);
  const [pick,prob]=v73SignalMarket(pred);
  const markets={
    "1X2":v7711BestPick(pred,"1X2"),
    "Over/Under":v7711BestPick(pred,"OU"),
    "Gol/No Gol":v7711BestPick(pred,"GG")
  };
  const topSignals=[];
  // Confidence can be safely migrated because its PRE-MATCH value was saved.
  if(Number.isFinite(conf)&&conf>=72){
    topSignals.push({
      type:"Confidence",
      pick,
      prob:Number(prob),
  ...
  // Value/Forma are intentionally NOT reconstructed for old matches because
  // their exact pre-match inputs were not persisted in the legacy snapshot.
  return {
    id:String(f.id),
    date:v7711FixtureDate(f),
    starting_at:f?.starting_at||null,
    league:f?.leagueName||f?.league?.name||"",
    home:f?.h?.name||realLoc(f,"home")?.name||"",
    away:f?.a?.name||realLoc(f,"away")?.name||"",
    markets,
    topSignals,
    primary:{pick,prob:Number(prob),confidence:Number.isFinite(conf)?conf:null},
    saved_at:Number(frozen.ts)||Date.now(),
    migrated_from:"v76-prematch"
  };
}
async function v7712SafeHistoricalBackfill(){
  // V77.13 first scans the actual legacy snapshot keys by fixture id.
  await v7713DirectHistoricalBackfill();
  try{
    if(localStorage.getItem(V7712_BACKFILL_KEY)==="done")return;
  }catch{}
  const ledger=v7711Ledger();
  let imported=0,completedDays=0;
  const today=new Date();
  const dates=[];
  // Seven days is enough to recover the recent frozen tests without making
  // the Performance page issue a large historical request burst.
  for(let i=0;i<7;i++){
  ...
  for(const date of dates){
    ...
    for(const f of fixtures){
      if(v67FixtureState(f)!=="ft")continue;
      const id=String(f?.id||"");
      if(!id||ledger[id])continue;
      const frozen=v7712OldFrozenPrediction(id);
      if(!frozen?.pred)continue;
      const row=v7712SnapshotFromFrozen(f,frozen);
      if(!row)continue;
      ledger[id]=row;
  ...
  if(imported)v7711WriteLedger(ledger);
  // Mark complete only if every requested day was successfully checked.
  try{
    if(completedDays===dates.length)localStorage.setItem(V7712_BACKFILL_KEY,"done");
  }catch{}
}

async function v7711PerformanceData(){
  await v7712SafeHistoricalBackfill();
  const ledger=v7711Ledger(),entries=Object.values(ledger);
  const markets={"1X2":v7711Stat(),"Over/Under":v7711Stat(),"Gol/No Gol":v7711Stat()};
  const marketNoPick={"1X2":0,"Over/Under":0,"Gol/No Gol":0};
  const signals={"Confidence":v7711Stat(),"Value":v7711Stat(),"Forma":v7711Stat()};
  const signalNoPick={"Confidence":0,"Value":0,"Forma":0};
  const marketTotal=v7711Stat(),signalTotal=v7711Stat();
  const marketBands=v7731BandMap();
  const oneXTwoMarginBands=v7732OneXTwoBands();
  let oneXTwoMarginNoPick=0;
  const bands={"70–74":v7711Stat(),"75–79":v7711Stat(),"80–84":v7711Stat(),"85–89":v7711Stat(),"90+":v7711Stat()};
  let confSum=0,confN=0,fixturesDone=0,marketNoPickTotal=0,signalNoPickTotal=0;

  const byDate={};
  entries.forEach(e=>{if(e.date)(byDate[e.date]??=[]).push(e)});
  for(const [date,rows] of Object.entries(byDate)){
    let fixtures=[];
    try{fixtures=await v68SafeFixtures(date)}catch{continue}
    for(const e of rows){
      const f=fixtures.find(x=>String(x.id)===String(e.id));
      if(!f||v67FixtureState(f)!=="ft")continue;
      const o=v7711Outcome(f); if(!o)continue;
      fixturesDone++;

      for(const [name,p] of Object.entries(e.markets||{})){
        if(!markets[name]||!p)continue;

        // 1X2: use the gap between first and second probability.
        // 0–2 points = NO PICK because the model has no meaningful edge.
        if(name==="1X2"){
          const margin=v7732OneXTwoMargin(p);
          if(Number.isFinite(margin)){
            const mb=v7732OneXTwoMarginBand(margin);
            if(margin<=2){
              marketNoPick[name]++;
              marketNoPickTotal++;
              oneXTwoMarginNoPick++;
            ...
            const hit=v7711PickHit(p.pick,o);
            v7711Add(markets[name],hit);
            v7711Add(marketTotal,hit);
            if(mb&&oneXTwoMarginBands[mb])v7711Add(oneXTwoMarginBands[mb],hit);

            const bk=v7731BandKey(p.prob);
            if(bk&&marketBands[bk]){
              v7711Add(marketBands[bk].total,hit);
              v7711Add(marketBands[bk].markets[name],hit);
            }
            continue;
          }
        }

        // Exact 50/50 on binary markets is NOT a prediction.
        if(v7731IsNoPick(p.pick,p.prob)){
          marketNoPick[name]++;
          marketNoPickTotal++;
          continue;
        }
      ...
      for(const s of (e.topSignals||[])){
        if(!signals[s.type])continue;

        if(v7731IsNoPick(s.pick,s.prob)){
          signalNoPick[s.type]++;
          signalNoPickTotal++;
          continue;
        }

        const hit=v7711PickHit(s.pick,o);
        v7711Add(signals[s.type],hit);
        v7711Add(signalTotal,hit);

        if(s.type==="Confidence"&&Number.isFinite(Number(s.confidence))){
          const c=Number(s.confidence); confSum+=c; confN++;
          const k=c>=90?"90+":c>=85?"85–89":c>=80?"80–84":c>=75?"75–79":c>=70?"70–74":null;
          if(k)v7711Add(bands[k],hit);
        }
      }
    }
  }
  ...
  return {
    markets,marketNoPick,marketNoPickTotal,marketBands,
    oneXTwoMarginBands,oneXTwoMarginNoPick,
    signals,signalNoPick,signalNoPickTotal,
    marketTotal,signalTotal,bands,
    avgConfidence:confN?Math.round(confSum/confN):null,
    fixturesDone,tracked:entries.length
  };
}

function v7713TrackerStatusHtml(){
  const s=v7713AutoState();
  const ts=v7735ReadTripletStatus();
  let bf={};
  try{bf=JSON.parse(localStorage.getItem(V7713_DIRECT_BACKFILL_KEY)||"{}")}catch{}

  const time=s.finished_at||s.last_run;
  const when=time?new Date(time).toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit"}):"—";
  const count=Object.keys(v7711Ledger()).length;

  const ttime=ts.finished_at||ts.last_run;
  const twhen=ttime?new Date(ttime).toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit"}):"—";

  const ready=Number(ts.ready_total ?? s.triplets_ready ?? 0);
  const completed=Number(ts.completed_now ?? 0);
  const missing=Number(ts.missing_after ?? s.triplets_missing ?? 0);

  return `<div class="v7713-tracker-status">
    <div><b>TRACKER AUTOMATICO</b><strong>${s.running||ts.running?"IN CORSO":"ATTIVO"}</strong></div>
    <p>Ultimo controllo: <b>${when}</b> · snapshot congelati: <b>${count}</b> · fixture future viste: <b>${s.upcoming_seen??ts.future_seen??"—"}</b></p>
    <small>Lavora quando SPACE16154 è aperto, indipendentemente dalla pagina visualizzata. Non serve aprire o scrollare Top Signals.</small>
    ${bf.legacy_keys!=null?`<small>Backfill locale: ${bf.legacy_keys} vecchi snapshot trovati · ${bf.imported||0} importati ora.</small>`:""}
    <div class="v7735-triplet-check ${missing===0&&ready>0?"ok":""}">
      <b>SNAPSHOT 1X2 PER DOMANI</b>
      <span>Pronti: <strong>${ready}</strong> · completati ora: <strong>${completed}</strong> · mancanti: <strong>${missing}</strong></span>
      <small>Verifica ${twhen}. Il pick congelato non viene modificato: viene aggiunto solo il terzetto 1 / X / 2 necessario al margine.</small>
    </div>
  </div>`;
}

function v7711PerfRow(label,s,unit,noPick=0){
  const t=v7711StatText(s);
  return `<div class="perf-row v7711-perf-row"><b>${label}</b><span>${t.ratio} ${unit}${noPick?` · <em>${noPick} NO PICK</em>`:""}</span><strong>${t.pct}</strong></div>`;
}
function v7731BandRow(label,b){
  const t=v7711StatText(b.total);
  const sub=Object.entries(b.markets).map(([name,s])=>{
    const x=v7711StatText(s);
    return s.total?`${name==="Over/Under"?"O/U":name==="Gol/No Gol"?"G/NG":name} ${x.ratio}`:"";
  }).filter(Boolean).join(" · ");
  return `<div class="v7731-band-row">
    <div><b>${label}</b><small>${sub||"Nessun pick valutato"}</small></div>
    <span>${t.ratio} presi</span>
    <strong>${t.pct}</strong>
  </div>`;
}

function v7732MarginRow(label,s){
  const t=v7711StatText(s);
  return `<div class="v7731-band-row v7732-margin-row">
    <div><b>${label}</b><small>${label==="0–2"?"NO PICK":"Margine tra 1ª e 2ª probabilità"}</small></div>
    <span>${label==="0–2"?"Esclusi":`${t.ratio} presi`}</span>
    <strong>${label==="0–2"?"—":t.pct}</strong>
  </div>`;
}
async function performance(){
  document.querySelector("#main").innerHTML=`<div class="page"><button class="back-btn" data-nav="more">‹ ALTRO</button><div class="eyebrow">MODEL PERFORMANCE</div><h1>PERFORMANCE</h1><p class="subtitle">Risultati reali dei pronostici congelati prima del calcio d'inizio.</p><div class="v7711-loading">Controllo snapshot futuri 1X2 e risultati FT…</div></div>`;
  bind();
  await v7735EnsureFutureTriplets();
  const d=await v7711PerformanceData();
  const mt=v7711StatText(d.marketTotal),st=v7711StatText(d.signalTotal);
  document.querySelector("#main").innerHTML=`<div class="page"><button class="back-btn" data-nav="more">‹ ALTRO</button><div class="eyebrow">MODEL PERFORMANCE</div><h1>PERFORMANCE</h1><p class="subtitle">Solo pronostici pre-match salvati e partite concluse FT.</p>
   <div class="perf-kpis v7711-kpis">
    <div><strong>${mt.pct}</strong><small>Accuracy mercati</small><em>${mt.ratio} presi</em></div>
    <div><strong>${d.fixturesDone}</strong><small>Partite valutate</small><em>${d.tracked} tracciate</em></div>
    <div><strong>${d.avgConfidence==null?"—":d.avgConfidence+"%"}</strong><small>Confidence media</small><em>segnali conclusi</em></div>
   </div>
   ${v7713TrackerStatusHtml()}
   <section class="panel"><div class="section-head"><h2>PER MERCATO</h2></div>
    ${v7711PerfRow("1X2",d.markets["1X2"],"pronostici",d.marketNoPick["1X2"])}
    ${v7711PerfRow("Over/Under 2.5",d.markets["Over/Under"],"pronostici",d.marketNoPick["Over/Under"])}
    ${v7711PerfRow("Gol/No Gol",d.markets["Gol/No Gol"],"pronostici",d.marketNoPick["Gol/No Gol"])}
    <div class="perf-row v7711-total"><b>TOTALE MERCATI</b><span>${mt.ratio} pronostici${d.marketNoPickTotal?` · <em>${d.marketNoPickTotal} NO PICK</em>`:""}</span><strong>${mt.pct}</strong></div>
   </section>
   <section class="panel"><div class="section-head"><h2>PER TOP SIGNAL</h2></div>
    ${v7711PerfRow("Confidence",d.signals.Confidence,"segnali",d.signalNoPick.Confidence)}
    ${v7711PerfRow("Value",d.signals.Value,"segnali",d.signalNoPick.Value)}
    ${v7711PerfRow("Forma",d.signals.Forma,"segnali",d.signalNoPick.Forma)}
    <div class="perf-row v7711-total"><b>TOTALE TOP SIGNAL</b><span>${st.ratio} segnali${d.signalNoPickTotal?` · <em>${d.signalNoPickTotal} NO PICK</em>`:""}</span><strong>${st.pct}</strong></div>
   </section>
   <section class="panel v7731-bands-panel"><div class="section-head"><h2>RESA PER FASCIA · MERCATI</h2></div>
    <p class="v7731-band-note">Il 50/50 esatto su Gol/No Gol e Over/Under è NO PICK e non entra nelle percentuali.</p>
    ${Object.entries(d.marketBands).map(([k,b])=>v7731BandRow(k,b)).join("")}
   </section>

   <section class="panel v7731-bands-panel"><div class="section-head"><h2>1X2 · MARGINE SULLA 2ª SCELTA</h2></div>
    <p class="v7731-band-note">Nell'1X2 conta la distanza tra la probabilità più alta e la seconda. Margine 0–2 punti = NO PICK.</p>
    <div class="v7731-band-row v7732-margin-row">
      <div><b>0–2</b><small>NO PICK</small></div>
      <span>${d.oneXTwoMarginNoPick} esclusi</span>
      <strong>—</strong>
    </div>
    ${["3–5","6–9","10+"].map(k=>v7732MarginRow(k,d.oneXTwoMarginBands[k])).join("")}
   </section>

   <p class="v7711-note">Programmate e LIVE non entrano nei conteggi. I 50/50 esatti dei mercati binari e gli 1X2 con margine 0–2 punti sono classificati NO PICK. Le vecchie partite vengono recuperate solo quando esiste già uno snapshot pre-match congelato sul dispositivo; i risultati si aggiornano automaticamente a FT.</p>
  </div>`;
  bind();
}
async function confidencePage(){
  document.querySelector("#main").innerHTML=`<div class="page"><button class="back-btn" data-nav="more">‹ ALTRO</button><div class="eyebrow">MODEL CONFIDENCE</div><h1>CONFIDENCE</h1><p class="subtitle">Convergenza del modello confrontata con la resa osservata reale.</p><div class="v7711-loading">Calcolo fasce reali…</div></div>`;
  bind();
  const d=await v7711PerformanceData();
  const rows=Object.entries(d.bands);
  document.querySelector("#main").innerHTML=`<div class="page"><button class="back-btn" data-nav="more">‹ ALTRO</button><div class="eyebrow">MODEL CONFIDENCE</div><h1>CONFIDENCE</h1><p class="subtitle">Misura la convergenza del quadro, non la probabilità dell'esito.</p>
   <section class="panel"><div class="confidence-box"><div><span>MEDIA REALE TRACCIATA</span><strong>${d.avgConfidence==null?"—":d.avgConfidence+"%"}</strong></div>
    <div class="big-progress"><i style="width:${d.avgConfidence||0}%"></i></div>
    <p>La calibrazione confronta la Confidence assegnata prima della gara con quanti segnali vengono poi realmente presi.</p></div></section>
   <section class="panel"><div class="section-head"><h2>RESA PER FASCIA</h2></div>
    ${rows.map(([k,s])=>v7711PerfRow(k,s,"segnali")).join("")}
   </section>
   <p class="v7711-note">Con campioni piccoli le percentuali possono oscillare molto. Useremo queste fasce per riequilibrare la Confidence dopo venerdì e sabato.</p>
  </div>`;
  bind();
}

function showInfo(t,title="INFO"){
 const macroMap={
  "Threat Creation":{
    measure:"Misura quanto una squadra riesce a creare occasioni pericolose con qualità e continuità.",
    data:["xG","Tiri","Tiri in porta","Ultimo terzo","Volume offensivo"]
  },
  "Defense Stability":{
    measure:"Misura quanto una squadra riesce a limitare pericoli, occasioni e qualità concessa agli avversari.",
    data:["xGA","Tiri concessi","Tiri in porta concessi","Qualità occasioni concesse","Solidità difensiva"]
  },
  "Forma":{
    measure:"Valuta il rendimento recente della squadra, dando più peso ai risultati ottenuti contro avversari più forti.",
    data:["Ultime 5","Risultati","Gol fatti/subiti","Forza avversari","Continuità"]
  },
  "Opponent Strength":{
    measure:"Stima il livello degli avversari affrontati di recente, così da leggere correttamente vittorie, pareggi e sconfitte.",
    data:["Rating avversari","Classifica","Forma avversari","Casa/trasferta","Livello campionato"]
  },
  "Pressing / PPDA":{
    measure:"Misura l'intensità della pressione e la capacità di recuperare il pallone in zone avanzate.",
    data:["PPDA","Recuperi alti","Pressione offensiva","Azioni difensive alte","Possesso avversario"]
  },
  "Availability":{
    measure:"Valuta quanto la disponibilità della rosa può influire sul rendimento atteso della squadra.",
    data:["Assenze","Titolari indisponibili","Giocatori chiave","Rotazioni","Profondità rosa"]
  }
 };
 const macro=macroMap[title];
 let o=document.createElement("div");
 o.className="modal-backdrop";
 if(macro){
  o.innerHTML=`<div class="info-modal v742-info-modal">
   <button class="modal-x">×</button>
   <div class="eyebrow">${realEsc(title)}</div>
   <h3>COSA MISURA</h3>
   <p>${realEsc(macro.measure)}</p>
   <div class="v742-info-sep"></div>
   <small>DATI CONSIDERATI</small>
   <div class="v742-info-chips">${macro.data.map(x=>`<span>${realEsc(x)}</span>`).join("")}</div>
  </div>`;
 }else{
  o.innerHTML=`<div class="info-modal v74-info-modal"><button class="modal-x">×</button><div class="eyebrow">${realEsc(title)}</div><h3>COSA MISURA</h3><p>${realEsc(t)}</p></div>`;
 }
...
function bind(){
 document.querySelectorAll("[data-nav]:not(.nav-item)").forEach(b=>b.onclick=()=>route(b.dataset.nav));
 ...
 document.querySelectorAll("[data-info]").forEach(b=>{b.onclick=()=>showInfo(decodeURIComponent(b.dataset.info),b.dataset.infoTitle||"INFO")});
}
...
async function v19AnalysisHome(){v27History({kind:"analysis-home"});document.querySelector("#main").innerHTML=`<div class="page v19-home"><header><h1>ANALYSIS</h1><p>INTELLIGENCE CENTER</p></header><div class="empty-note">Caricamento campionati…</div></div>`;bind();const meta=await v60CompetitionMeta();const comps=V60_COMPETITIONS.map(c=>({...c,...((meta?.competitions||[]).find(x=>x.key===c.key)||{})}));document.querySelector("#main").innerHTML=`<div class="page v19-home"><header><h1>ANALYSIS</h1><p>INTELLIGENCE CENTER</p></header><div class="v19-leagues">${comps.map(l=>`<button data-v60-analysis="${l.key}"><span class="v62-league-center">${v60LeagueLogo(l)}<b>${realEsc(l.name)}</b></span></button>`).join("")}</div></div>`;bind();document.querySelectorAll("[data-v60-analysis]").forEach(b=>b.onclick=()=>v60StandingsPage(b.dataset.v60Analysis))}
...
/* V71.3 HOTFIX — SportMonks kickoff time -> Europe/Rome */
function fmtTime(value){
  if(!value) return "";
  try{
    let raw=String(value).trim();
    // SportMonks timestamps without an explicit offset are UTC.
...
function realNormFixture(f){return {...f,h:realLoc(f,"home")||{},a:realLoc(f,"away")||{},time:fmtTime(f?.starting_at)||"--:--",leagueName:f?.league?.name||""}}
...
function realRecent(fs,id){
  // Regola globale SPACE16154: Ultime 5 sempre in ordine cronologico visivo.
  // SINISTRA = partita più vecchia, DESTRA = partita più recente.
  const latest5=fs.slice().sort((a,b)=>String(b?.starting_at||"").localeCompare(String(a?.starting_at||""))).slice(0,5);
  return latest5.reverse().map(f=>{const o=realOpponent(f,id),gf=realScore(f,id),ga=o?realScore(f,o.id):null;return {gf,ga,r:gf==null||ga==null?"—":gf>ga?"W":gf<ga?"L":"D"}});
}
...
function v63FlattenStandingRows(data){
 const out=[],seen=new Set();
 const walk=x=>{
   if(!x)return;
   if(Array.isArray(x)){x.forEach(walk);return}
   if(typeof x!=="object")return;
   const pid=x.participant_id??x.team_id??x.participant?.id;
   if(pid!=null && (x.position!=null || x.points!=null || Array.isArray(x.details))){
     const k=String(pid)+"|"+String(x.position??"");
     if(!seen.has(k)){seen.add(k);out.push(x)}
   }
   for(const [k,v] of Object.entries(x)){
     if(["participant","type","details"].includes(k))continue;
...
function v60StandingValue(row,key){
 if(key==="pts"){const p=realNum(row?.points);if(p!=null)return p}
 const aliases={
  played:["OVERALL_MATCHES","MATCHES_PLAYED","GAMES_PLAYED","MATCHES"],
  won:["OVERALL_WINS","OVERALL_WON","WINS","WON"],
  draw:["OVERALL_DRAWS","DRAWS","DRAW"],
  lost:["OVERALL_LOSSES","OVERALL_LOST","LOSSES","LOST"],
  gf:["OVERALL_GOALS_FOR","GOALS_FOR","SCORED"],
  ga:["OVERALL_GOALS_AGAINST","GOALS_AGAINST","CONCEDED"],
  pts:["TOTAL_POINTS","OVERALL_POINTS","POINTS"]
 };
 for(const x of(Array.isArray(row?.details)?row.details:[])){
  const raw=String(x?.type?.developer_name||x?.type?.code||x?.type?.name||"").toUpperCase();
  const n=raw.replace(/[\s-]+/g,"_");
  if((aliases[key]||[]).some(a=>n===a||n.includes(a))){
    const v=realNum(x?.value??x?.data?.value);
    if(v!=null)return v
  }
 }
 const direct={
  played:["played","games_played","matches_played","overall_matches"],
  won:["won","wins","overall_wins"],
  draw:["draw","drawn","draws","overall_draws"],
  lost:["lost","losses","overall_losses"],
  gf:["goals_for","goals_scored","overall_goals_for"],
  ga:["goals_against","goals_conceded","overall_goals_against"],
  pts:["points","total_points","overall_points"]
 }[key]||[];
...
async function v60StandingsPage(initialKey="la-liga"){
 ...
 try{
  ...
  const load=async key=>{
   ...
   <div class="v67-standing-banner" data-v67-standing-banner><b>AGGIORNAMENTO AUTOMATICO</b><span>Classifica ufficiale corrente.</span></div>
   ...
   const [b,livePack]=await Promise.all([realApi(`/competition?key=${encodeURIComponent(c.key)}`),realApi("/live-fixtures").catch(()=>({data:[]}))]);
   const liveRaw=Array.isArray(livePack?.data)?livePack.data:Array.isArray(livePack?.data?.data)?livePack.data.data:[];
   const isLive=liveRaw.some(f=>String(f?.league?.name||"").toLowerCase()===String(c.name||"").toLowerCase() && v67FixtureState(f)==="live");
   const teams=Array.isArray(b.teams)?b.teams:[],rows=v63FlattenStandingRows(b.rows||b.standings||[]);
   ...
   const banner=root.querySelector("[data-v67-standing-banner]");
   if(banner) banner.innerHTML=isLive?`<b><i class="v66-live-dot"></i> CLASSIFICA LIVE · ${realEsc(b.season?.name||"")}</b><span>Include i risultati parziali delle gare in corso.</span>`:`<b>AGGIORNAMENTO AUTOMATICO · ${realEsc(b.season?.name||"")}</b><span>Classifica ufficiale corrente.</span>`;
   root.querySelector(".v60-table-panel").innerHTML=`<div class="v60-standing-head v65-standing-grid"><span>SQUADRA</span><span>PG</span><span>V</span><span>P</span><span>S</span><span>GF</span><span>GS</span><span>DR</span><b>PT</b></div>
   ${merged.map((x,i)=>{const r=x.row||{},gf=v60StandingValue(r,"gf"),ga=v60StandingValue(r,"ga");return `<button class="v60-standing-row v65-standing-grid v67-team-row" data-v67-team="${realEsc(x.team?.id||"")}" data-v67-league="${realEsc(c.key)}"><div class="v65-standing-team"><span class="v65-pos">${r?.position??i+1}</span>${realCrest(x.team)}<b>${realEsc(x.team?.name||"—")}</b></div><span>${v60StandingValue(r,"played")}</span><span>${v60StandingValue(r,"won")}</span><span>${v60StandingValue(r,"draw")}</span><span>${v60StandingValue(r,"lost")}</span><span>${gf}</span><span>${ga}</span><span>${gf-ga}</span><strong>${v60StandingValue(r,"pts")}</strong></button>`}).join("")}
   ${merged.length?"":`<div class="empty-note v62-standing-empty">Classifica non ricevuta da SportMonks.</div>`}`;
   root.querySelectorAll("[data-v67-team]").forEach(el=>el.onclick=()=>route("real-team",el.dataset.v67Team+"|"+el.dataset.v67League));
  };
...
function v67FixtureState(f){
  const raw=String(f?.state?.state||f?.state?.name||f?.state?.short_name||"").toUpperCase();
  if(raw.includes("FINISH")||["FT","AET","FT_PEN"].includes(raw)) return "ft";
  if(raw.includes("LIVE")||raw.includes("INPLAY")||raw.includes("IN_PLAY")||["1ST","2ND","HT","ET","BREAK"].includes(raw)) return "live";
  return "upcoming";
}
function v67HistoryWithCurrent(detail,history,teamId){
  const arr=Array.isArray(history)?history.slice():[];
  if(!v60FixtureFinished(detail)) return arr;
  const hasTeam=(detail?.participants||[]).some(p=>String(p?.id)===String(teamId));
  if(!hasTeam) return arr;
  const id=String(detail?.id||"");
  // A fixture already present in history can still be the stale pre-match copy.
  // On FT, always replace that copy with the fresh finished fixture so score/stats
  // enter the rolling last-5 window for BOTH teams.
  const clean=id ? arr.filter(f=>String(f?.id||"")!==id) : arr;
  return [detail,...clean];
}
function v67FixtureStatusHtml(f){
  const st=v67FixtureState(f),h=f?.h||realLoc(f,"home")||{},a=f?.a||realLoc(f,"away")||{};
  const hs=realScore(f,h.id),as=realScore(f,a.id);
  if(st==="ft") return `<div class="v67-fixture-status ft"><strong>${hs??"—"}–${as??"—"}</strong><span>FT</span></div>`;
  if(st==="live"){
    const minute=f?.time?.minute??f?.minute??f?.state?.short_name??"LIVE";
    return `<div class="v67-fixture-status live"><strong>${hs??"—"}–${as??"—"}</strong><span><i></i>${realEsc(minute)}${Number.isFinite(Number(minute))?"′":""}</span></div>`;
  }
  return `<div class="v67-fixture-status upcoming"><strong>${realEsc(f?.time||fmtTime(f?.starting_at)||"—")}</strong><span>PROGRAMMATA</span></div>`;
}
...
function v68PredictionKey(id){return `space16154:prematch:v76:${id}`}
function v68SavePrediction(id,pred,conf){
  if(!id||!pred)return;
  try{localStorage.setItem(v68PredictionKey(id),JSON.stringify({pred,conf,ts:Date.now()}))}catch{}
}
function v68LoadPrediction(id){
  try{
    const x=JSON.parse(localStorage.getItem(v68PredictionKey(id))||"null");
    return x&&x.pred?x:null;
  }catch{return null}
}
async function v68SafeFixtures(date){
  try{return await realFixtures(date)}
  catch(first){
    await new Promise(r=>setTimeout(r,350));
    return await realFixtures(date);
  }
}

async function v70NextAvailableDate(fromDate){
  try{
    const r=await realApi(`/next-fixtures?date=${encodeURIComponent(fromDate)}&days=30`);
    const d=String(r?.date||r?.next_date||"");
    return /^\d{4}-\d{2}-\d{2}$/.test(d)?d:null;
  }catch{return null}
}
function v70SetDashDate(iso){
  const d=new Date(iso+"T12:00:00Z");
  if(Number.isNaN(d.getTime()))return false;
  DASH_YEAR=d.getUTCFullYear();
  DASH_MONTH=d.getUTCMonth();
  DASH_DAY=d.getUTCDate();
  return true;
}
function v70DashboardEmpty(message,autoLabel=""){
  const meta=dashDateMeta();
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   <header class="v714-dashboard-head">
    <div><h1>DASHBOARD</h1><p>FOOTBALL INTELLIGENCE</p></div>
    <div class="v8-date-pair">
     <button class="v8-date-control" data-v63-date="month"><small>MESE</small><strong>${MONTHS_IT[DASH_MONTH].slice(0,3).toUpperCase()}</strong><span>+</span></button>
     <button class="v8-date-control day" data-v63-date="day"><small>GIORNO</small><strong>${String(DASH_DAY).padStart(2,"0")}</strong><span>+</span></button>
    </div>
   </div>
   ${autoLabel?`<div class="v70-next-date-note">${realEsc(autoLabel)}</div>`:""}
   <section class="v6-hero-combined v70-empty-dashboard">
    <div class="v2-panel-head"><h2>PARTITE</h2><b>${meta.short.toUpperCase()}</b></div>
    <div class="empty-note">${realEsc(message)}</div>
   </section>
  </div>`;
  bind();
  document.querySelectorAll("[data-v63-date]").forEach(b=>{
    const open=e=>{e.preventDefault();e.stopPropagation();openDashDropdown(b.dataset.v63Date,b)};
    b.addEventListener("pointerup",open,{passive:false});
  });
}
async function v68FixtureDetail(id){
  try{const r=await realApi(`/fixture?id=${encodeURIComponent(id)}`);return r?.data||r||null}
  catch{return null}
}
function v68HeroCenter(m,detail){
  const d=detail||m;
  if(v60FixtureFinished(d)){
    const h=m.h||realLoc(d,"home")||{},a=m.a||realLoc(d,"away")||{};
    return `<div class="v68-final-score"><strong>${realScore(d,h.id)??"—"} – ${realScore(d,a.id)??"—"}</strong><span>FT</span></div>`;
  }
  if(v67FixtureState(d)==="live"){
    const h=m.h||realLoc(d,"home")||{},a=m.a||realLoc(d,"away")||{};
    return `<div class="v68-final-score live"><strong>${realScore(d,h.id)??"—"} – ${realScore(d,a.id)??"—"}</strong><span>LIVE</span></div>`;
  }
  return `<b>VS</b>`;
}
async function realDashboard(){
 const meta=dashDateMeta();
 document.querySelector("#main").innerHTML=`<div class="page v2-dashboard"><section class="v6-hero-combined"><div class="empty-note">Caricamento SportMonks…</div></section></div>`;bind();
 try{
  await v60CompetitionMeta().catch(()=>({competitions:[]}));
  let fs=await v68SafeFixtures(meta.id);
  let autoMoved=false;
  if(!fs.length && !DASH_DATE_MANUAL){
    const nextDate=await v70NextAvailableDate(meta.id);
    if(nextDate && nextDate!==meta.id && v70SetDashDate(nextDate)){
      autoMoved=true;
      fs=await v68SafeFixtures(nextDate);
    }
  }
  const activeMeta=dashDateMeta();
  const cmeta=await v60CompetitionMeta().catch(()=>({competitions:[]}));
  const m=fs[0];
  if(!m){
    v70DashboardEmpty("Nessuna partita dei 5 campionati per la data selezionata.");
    return;
  }
  const detail=await v68FixtureDetail(m.id);
  const state=v67FixtureState(detail||m);
  const predictionLocked=state==="live"||state==="ft";
  let pred=null,conf=null;
  const frozen=predictionLocked?v68LoadPrediction(m.id):null;
  if(frozen){
    pred=frozen.pred;
    conf=frozen.conf;
  }else{
    try{
      const p=await realApi(`/prematch?id=${m.id}`),h=p.teams?.home||m.h,a=p.teams?.away||m.a,
        leagueId=p?.fixture?.league?.id||p?.fixture?.league_id||m?.leagueId||m?.league_id||null,
        hHistory=v76PreMatchLeagueHistory(realHistory(p,"home_history"),p?.fixture||m,leagueId,5),
        aHistory=v76PreMatchLeagueHistory(realHistory(p,"away_history"),p?.fixture||m,leagueId,5),
        hm=v58Metrics(hHistory,h),am=v58Metrics(aHistory,a),
        hh=v58Metrics(hHistory,h,"home"),aa=v58Metrics(aHistory,a,"away");
      pred=v59ShrinkPrediction(v58Prediction(hm,am,hh,aa),hm,am);
      conf=v59Confidence(pred,hm,am);
      v68SavePrediction(m.id,pred,conf);
    }catch{
      const cached=v68LoadPrediction(m.id);
      if(cached){pred=cached.pred;conf=cached.conf}
    }
  }
  const comp=v60CompFromName(cmeta,m.leagueName);
  const stateLabel=state==="ft"?" · FT":state==="live"?" · LIVE":"";
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   <div class="dash-global-head v8-global-head v714-dashboard-head">
    <div class="v20-dashboard-label"><span class="dash-global-kicker">DASHBOARD</span></div>
    <div class="v8-date-pair">
     <button class="v8-date-control" data-v63-date="month"><small>MESE</small><strong>${MONTHS_IT[DASH_MONTH].slice(0,3).toUpperCase()}</strong><span>+</span></button>
     <button class="v8-date-control day" data-v63-date="day"><small>GIORNO</small><strong>${String(DASH_DAY).padStart(2,"0")}</strong><span>+</span></button>
   ...
   ${autoMoved?`<div class="v70-next-date-note">PROSSIME PARTITE · ${activeMeta.short.toUpperCase()}</div>`:""}

   <section class="v716-dash-card">
    <div class="v716-dash-card-head">
      <h2>MATCH IN EVIDENZA <span>☆</span></h2>
      <b>${activeMeta.short.toUpperCase()}</b>
    </div>

    <div class="v716-dash-meta">
      ${v60LeagueLogo(comp,true)}
      <span>${realEsc(m.leagueName)}</span>
      <i>•</i><span>${activeMeta.short}</span>
      <i>•</i><span>${realEsc(m.time)}${stateLabel}</span>
    </div>

    <div class="v716-dash-versus">
      <div class="v716-dash-team">
        ${realCrest(m.h)}
        <strong>${realEsc(m.h.name||"").toUpperCase()}</strong>
      </div>
      <div class="v716-dash-center">
        ${v68HeroCenter(m,detail)}
        <button data-real-match="${m.id}">${state==="ft"?"POST-MATCH":"ANALISI"}</button>
      </div>
      <div class="v716-dash-team">
        ${realCrest(m.a)}
        <strong>${realEsc(m.a.name||"").toUpperCase()}</strong>
      </div>
    </div>

    <div class="v716-dash-prob-head">
      <strong>${state==="ft"?"PROBABILITÀ PRE-MATCH · BLOCCATE":"ESITO PIÙ PROBABILE"}</strong>
      ${conf!=null?`<span>Confidence ${conf}%</span>`:""}
    </div>

    ${pred?(()=>{const b1=v61UniqueBest([pred.one,pred.x,pred.two]),bg=v61UniqueBest([pred.gg,pred.ng]),bo=v61UniqueBest([pred.over,pred.under]);return `
      <div class="v716-market-row">
        <div class="v716-market">
          <b>1X2</b>
          <div class="v716-market-values three">
            <span class="${pred.one===b1?"v61-best":""}"><small>1</small><strong>${pred.one}%</strong></span>
            <span class="${pred.x===b1?"v61-best":""}"><small>X</small><strong>${pred.x}%</strong></span>
            <span class="${pred.two===b1?"v61-best":""}"><small>2</small><strong>${pred.two}%</strong></span>
          </div>
        </div>
        <div class="v716-market">
          <b>GOL / NO GOL</b>
          <div class="v716-market-values two">
            <span class="${pred.gg===bg?"v61-best":""}"><small>GOL</small><strong>${pred.gg}%</strong></span>
            <span class="${pred.ng===bg?"v61-best":""}"><small>NO GOL</small><strong>${pred.ng}%</strong></span>
          </div>
        </div>
        <div class="v716-market">
          <b>OVER / UNDER 2.5</b>
          <div class="v716-market-values two">
            <span class="${pred.over===bo?"v61-best":""}"><small>OVER</small><strong>${pred.over}%</strong></span>
            <span class="${pred.under===bo?"v61-best":""}"><small>UNDER</small><strong>${pred.under}%</strong></span>
          </div>
        </div>
      </div>`})():`
      <div class="v716-dash-empty">${state==="ft"?"Probabilità pre-match non salvate per questa gara.":"Campione insufficiente per una previsione reale."}</div>`}
   </section>

   ${fs.slice(1,4).length ? `<section class="v5-upcoming">
    <div class="v2-panel-head"><h2>PROSSIME PARTITE</h2><button data-nav="matches">Vedi tutte ›</button></div>
    <div class="v5-upcoming-list v7721-dashboard-upcoming">${fs.slice(1,4).map(x=>v7717MatchCard(x,{league:true,action:true,compact:true})).join("")}</div>
   </section>` : ""}
  </div>`;
  bind();
  document.querySelectorAll("[data-v63-date]").forEach(b=>{
  ...
  document.querySelectorAll("[data-real-match]").forEach(b=>b.onclick=()=>route("match",b.dataset.realMatch));
 }catch(e){
  v70DashboardEmpty(e?.message||"Dati temporaneamente non raggiungibili.")
 }
}


function v7524HistoryDebug(team,history){
  ...
  const line=f=>{
    const opp=realOpponent(f,team.id)||{};
    const gf=realScore(f,team.id),ga=opp?.id!=null?realScore(f,opp.id):null;
    const result=gf==null||ga==null?"—":gf>ga?"V":gf<ga?"P":"N";
    const rawState=String(f?.state?.short_name||f?.state?.state||f?.state?.name||"—");
    const comp=String(f?.league?.name||f?.league_name||f?.stage?.name||f?.round?.name||"COMP —");
    const date=String(f?.starting_at||"—").replace("T"," ").slice(0,16);
    const score=gf==null||ga==null?"—":`${gf}-${ga}`;
    return `<div class="v7524-debug-row">
      <b>${realEsc(date)}</b>
      <span>${realEsc(comp)}</span>
      <span>${realEsc(opp?.name||"Avversario —")}</span>
      <strong>${realEsc(score)} · ${realEsc(result)}</strong>
      <small>ID ${realEsc(String(f?.id??"—"))} · ${realEsc(rawState)}</small>
    </div>`;
  };

  return `<section class="v19-card v7524-history-debug">
    <div class="v19-chead"><b>DEBUG STORICO · 1.24</b><span>${rows.length} FIXTURE RICEVUTE</span></div>
    <p class="v7524-debug-note">Ordine mostrato: più recente → più vecchia. Qui vediamo direttamente ciò che arriva da /team-history prima di scegliere le Ultime 5.</p>
    <div class="v7524-debug-list">${rows.length?rows.map(line).join(""):`<div class="empty-note">Nessuna fixture ricevuta.</div>`}</div>
  </section>`;
}

async function v77RecentFinishedForTeam(teamId,leagueId){
  const today=new Date();
  const dates=[];
  for(let i=0;i<4;i++){const d=new Date(today);d.setDate(d.getDate()-i);dates.push(d.toISOString().slice(0,10));}
  const packs=await Promise.all(dates.map(d=>v68SafeFixtures(d).catch(()=>[])));
  return packs.flat().filter(f=>{
    const lid=f?.leagueId??f?.league_id??f?.league?.id;
    const teams=[f?.h?.id,f?.a?.id,...(Array.isArray(f?.participants)?f.participants.map(p=>p?.id):[])].map(String);
    return (!leagueId||String(lid)===String(leagueId))&&teams.includes(String(teamId))&&v67FixtureState(f)==="ft";
  });
}

async function v67RealTeamProfile(payload){
  const [teamId,leagueKey]=String(payload||"").split("|");
  const root=document.querySelector("#main");
  root.innerHTML=`<div class="page v67-team-page"><div class="empty-note">Caricamento profilo squadra…</div></div>`;
  try{
    const b=await realApi(`/competition?key=${encodeURIComponent(leagueKey)}`);
    const seasonId=b?.season?.id||b?.league?.current_season_id||null;
    const leagueId=b?.league?.id||null;
    const rawHistory=await v69SafeTeamHistory(teamId,seasonId);
    const recentFixtures=await v77RecentFinishedForTeam(teamId,leagueId);
    const mergedMap=new Map();
    [...(Array.isArray(rawHistory)?rawHistory:[]),...(Array.isArray(recentFixtures)?recentFixtures:[])].forEach(f=>{
      if(f?.id!=null) mergedMap.set(String(f.id),f);
    });
    const mergedHistory=[...mergedMap.values()].sort((a,b)=>String(b?.starting_at||"").localeCompare(String(a?.starting_at||"")));

    // V77.5 — Team Detail uses the SAME rolling path as Match Detail.
    // Find the latest finished fixture in the selected league, take its PRE-MATCH
    // league history, then roll the finished fixture into the 5-match window.
    // Internal history stays newest -> oldest; form rendering is oldest -> newest.
    let history=v76LeagueHistory(mergedHistory,leagueId,15);
    const latestFt=history.find(f=>v67FixtureState(f)==="ft")||null;
    if(latestFt?.id){
      try{
        const latestPack=await realApi(`/prematch?id=${encodeURIComponent(latestFt.id)}`);
        const lpHome=latestPack?.teams?.home||realLoc(latestFt,"home")||{};
        const lpAway=latestPack?.teams?.away||realLoc(latestFt,"away")||{};
        const side=String(lpHome?.id)===String(teamId)?"home_history":"away_history";
        const pre=v76PreMatchLeagueHistory(realHistory(latestPack,side),latestPack?.fixture||latestFt,leagueId,5);
        if(pre.length){
          const rolled=v67HistoryWithCurrent(latestFt,pre,teamId);
          history=[...rolled,...history.filter(f=>!rolled.some(r=>String(r?.id)===String(f?.id)))];
        }
      }catch{}
    }
    const teams=Array.isArray(b.teams)?b.teams:[],team=teams.find(t=>String(t.id)===String(teamId))||{};
    const rows=v63FlattenStandingRows(b.rows||b.standings||[]);
    const row=rows.find(r=>String(r?.participant_id??r?.participant?.id)===String(teamId))||{};
    const recentTeam=history.slice(0,5);
    const metrics=v58Metrics(recentTeam,team),macro=v58Macro(metrics),trend=v58PerfTrend(recentTeam,teamId);
    const sample=metrics.sample||0;
    const rating=sample>=3?v58ModelScore(macro):null;
    const meta=await v60CompetitionMeta(),comp=V60_COMPETITIONS.map(x=>({...x,...((meta?.competitions||[]).find(c=>c.key===leagueKey)||{})})).find(x=>x.key===leagueKey)||{name:leagueKey};
    const macroList=[["threat","Threat Creation"],["defense","Defense Stability"],["form","Forma"],["opponent","Opponent Strength"],["pressing","Pressing / PPDA"],["availability","Availability"]];
    const gf=v60StandingValue(row,"gf"),ga=v60StandingValue(row,"ga");
    root.innerHTML=`<div class="page v67-team-page">
      <button class="back-btn" data-v67-team-back>‹ CLASSIFICA</button>
      <header class="v67-team-head v742-team-head">${realCrest(team)}<div class="v742-team-id"><small>${realEsc(comp.name||"")}</small><h1>${realEsc(team.name||"Squadra")}</h1><span>${row?.position?`${row.position}° in classifica`:"Classifica —"}</span><div class="v742-team-form"><b>${v76Last5Label(comp.name)}</b>${realFormDots(metrics.form)}<em>${sample}/5</em></div></div></header>


      <section class="v19-card v67-rating-card v68-rating-card"><div class="v19-chead"><b>MODEL RATING</b><span class="${sample<3?"v68-prelim-tag":""}">${sample<3?"PRELIMINARE":rating+"/100"}</span></div>
        <div class="v67-rating-value">${rating??"—"}</div>
        <p>${sample<3?`Campione reale insufficiente: ${sample} gara${sample===1?"":"e"} utile. Il rating viene attivato da 3 gare.`:"Rating preliminare calcolato esclusivamente su dati reali."}</p>
      </section>

      <section class="v19-card v68-macro-card v75-team-macros"><div class="v19-chead"><b>MACROAREE</b><span>${sample<3?`PRELIMINARI · ${sample}/5`:`${sample}/5 GARE`}</span></div>
        <div class="v75-macro-list">${macroList.map(([k,title])=>{const m=V14_MACRO_META[k],v=macro[k];return `<button class="v75-macro-row v752-macro-row" data-v75-macro="${realEsc(title)}" data-v75-desc="${realEsc(m.desc)}">
  <span class="v752-macro-icon ${k==="form"?"v756-form-icon":""}">${m.icon}</span>
  <div class="v752-macro-main">
    <div class="v752-macro-head">
      <span class="v752-macro-title">${realEsc(title)}<i class="v75-info-dot">i</i></span>
      <strong class="${v==null?"neutral":(v>=80?"good":v>=60?"mid":"bad")}">${v??"—"}</strong>
    </div>
    <em class="v752-macro-track ${v==null?"neutral":(v>=80?"good":v>=60?"mid":"bad")}"><i style="width:${v??0}%"></i></em>
  </div>
</button>`}).join("")}</div>
        <p class="real-note v68-real-note v75-macro-note">${sample<3?"Macroaree preliminari: l’affidabilità aumenta con il campione reale. ":""}“—” indica dato non disponibile o campione insufficiente.</p>
      </section>

      <section class="v19-card"><div class="v19-chead"><b>TREND MODEL</b><span>ULTIME GARE</span></div>
        ${trend.length>=2?v58TrendSvg(trend):`<div class="empty-note">Trend disponibile dopo almeno 2 gare reali.</div>`}
      </section>

      ${v722TeamTech(metrics,team,sample)}
    </div>`;
    bind();
    root.querySelector("[data-v67-team-back]").onclick=()=>v60StandingsPage(leagueKey);
    root.querySelectorAll("[data-v75-macro]").forEach(el=>el.onclick=()=>showInfo(el.dataset.v75Desc||"",el.dataset.v75Macro||"INFO"));
  }catch(e){
    root.innerHTML=`<div class="page v67-team-page"><button class="back-btn" data-nav="analysis">‹ ANALYSIS</button><div class="empty-note">${realEsc(e.message)}</div></div>`;bind();
  }
}

function v76LeagueHistory(history,leagueId,limit=15){
  const rows=(Array.isArray(history)?history:[]).filter(f=>{
    if(!leagueId)return true;
    return String(f?.leagueId??f?.league_id??f?.league?.id??"")===String(leagueId);
  });
  return rows
    .slice()
    .sort((a,b)=>String(b?.starting_at||"").localeCompare(String(a?.starting_at||"")))
...
function v76PreMatchLeagueHistory(history,fixture,leagueId,limit=5){
  const kickoff=String(fixture?.starting_at||"");
  const fixtureId=String(fixture?.id??"");
  return v76LeagueHistory(history,leagueId,50)
    .filter(f=>String(f?.id??"")!==fixtureId)
    .filter(f=>!kickoff||String(f?.starting_at||"")<kickoff)
    .slice(0,Math.max(1,Number(limit)||5));
}
function v76Last5Label(leagueName){
  return `ULTIME 5 · ${String(leagueName||"CAMPIONATO").toUpperCase()}`;
}

async function v69SafeTeamHistory(teamId, seasonId=null){
  const load=async useSeason=>{
    const q=new URLSearchParams({team:String(teamId),limit:"15"});
    if(useSeason&&seasonId)q.set("season",String(seasonId));
    const r=await realApi(`/team-history?${q.toString()}`);
    if(Array.isArray(r?.data)) return r.data;
    if(Array.isArray(r?.data?.data)) return r.data.data;
    if(Array.isArray(r?.fixtures)) return r.fixtures;
    return [];
  };

  // V77 CHAMPIONSHIP FORM RULE:
  // prefer the selected competition/season. Global history is only a transport
  // fallback; the UI applies a league_id filter before selecting the last 5.
  try{
    if(seasonId){
      const seasonRows=await load(true);
      if(seasonRows.length)return seasonRows;
    }
    return await load(false);
  }catch{
    try{return await load(false)}catch{}
  }
  return [];
}

async function v69SafeAnalysisPack(id){
  // Keep the old working prematch path when available.
  try{
    const [pack,fb]=await Promise.all([
      realApi(`/prematch?id=${encodeURIComponent(id)}`),
      realApi(`/fixture?id=${encodeURIComponent(id)}`)
    ]);
    return {pack,fb,source:"prematch"};
  }catch(err){
    // Fallback only when prematch is unavailable: fixture is still enough to open the page.
    const fb=await realApi(`/fixture?id=${encodeURIComponent(id)}`);
    const detail=fb?.data||fb;
    const home=realLoc(detail,"home")||{};
    const away=realLoc(detail,"away")||{};
    const sid=detail?.season_id||detail?.season?.id||null;

    const [hh,ah,h2]=await Promise.all([
      home?.id?v69SafeTeamHistory(home.id,sid):Promise.resolve([]),
      away?.id?v69SafeTeamHistory(away.id,sid):Promise.resolve([]),
      home?.id&&away?.id
        ? realApi(`/h2h?home=${encodeURIComponent(home.id)}&away=${encodeURIComponent(away.id)}`).catch(()=>({data:[]}))
    ...
    return {
      ...
      pack:{
        ...
        fixture:detail,
        teams:{home,away},
        home_history:{ok:true,data:hh},
        away_history:{ok:true,data:ah},
        h2h:{ok:true,data:Array.isArray(h2?.data)?h2.data:[]},
        fixture_context:{ok:true,data:detail}
      }
    };
  }
}
...
async function realMatches(){
 const root=document.querySelector("#main");
 root.innerHTML=`<section class="m13-page v712-match-center"><div class="empty-note">Caricamento SportMonks…</div></section>`;
 try{
  const date=dashDateMeta().id;
  const [fs,meta]=await Promise.all([
    v68SafeFixtures(date),
    v60CompetitionMeta().catch(()=>({competitions:[]}))
  ]);
  const leagues=V60_COMPETITIONS.map(c=>({...c,...((meta?.competitions||[]).find(x=>x.key===c.key)||{})}));
  let statusFilter="all", leagueFilter="all";

  const stateKey=f=>{
    const s=v67FixtureState(f);
    return s==="ft"?"finished":s==="live"?"live":"upcoming";
  };
  const leagueKeyOf=f=>{
    const hit=leagues.find(l=>l.name===f.leagueName);
    return hit?.key||"";
  };

  const render=()=>{
    const visible=fs.filter(f=>{
      const okStatus=statusFilter==="all" || stateKey(f)===statusFilter;
      const okLeague=leagueFilter==="all" || leagueKeyOf(f)===leagueFilter;
      return okStatus&&okLeague;
    });

    root.innerHTML=`<section class="m13-page v712-match-center">
      <header class="v712-page-head">
        <h1>MATCHES</h1>
        <p>MATCH CENTER</p>
      </header>

      <div class="v712-date-switcher">
        <button data-v712-day="-1" aria-label="Giorno precedente">‹</button>
        <div>
          <small>DATA</small>
          <strong>${realEsc(dashDateMeta().label)}</strong>
        </div>
        <button data-v712-day="1" aria-label="Giorno successivo">›</button>
      </div>

      <div class="v712-status-filters">
        ${[
          ["all","TUTTE"],["live","LIVE"],["upcoming","PROSSIME"],["finished","FINITE"]
        ].map(([k,t])=>`<button class="${statusFilter===k?"active":""}" data-v712-status="${k}">${t}</button>`).join("")}
      </div>

      <div class="v74-match-leagues">
        ${leagues.map(l=>`<button type="button" class="${leagueFilter===l.key?"active":""}" data-v716-league="${l.key}">${v60LeagueLogo(l,true)}<span>${realEsc(l.name)}</span></button>`).join("")}
      </div>

      <div class="v712-fixture-list">
        ${visible.length?visible.map(f=>{
          const lk=leagueKeyOf(f), l=leagues.find(x=>x.key===lk), st=stateKey(f);
          return v7717MatchCard(f,{league:true,action:true,compact:false});
        }).join(""):`<div class="v712-empty">Nessuna partita per i filtri selezionati.</div>`}
      </div>
    </section>`;

    root.querySelectorAll("[data-v712-status]").forEach(b=>b.onclick=()=>{statusFilter=b.dataset.v712Status;render()});
    const sheet=root.querySelector("#v716SheetBackdrop");
    const openSheet=()=>{if(sheet){sheet.hidden=false;requestAnimationFrame(()=>sheet.classList.add("open"));document.body.classList.add("v716-sheet-open")}};
    const closeSheet=()=>{if(sheet){sheet.classList.remove("open");document.body.classList.remove("v716-sheet-open");setTimeout(()=>{if(sheet&&!sheet.classList.contains("open"))sheet.hidden=true},160)}};
    const leagueTrigger=root.querySelector("#v716LeagueTrigger");
    if(leagueTrigger) leagueTrigger.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();openSheet()};
    const sheetClose=root.querySelector("#v716SheetClose");
    if(sheetClose) sheetClose.onclick=closeSheet;
    if(sheet) sheet.onclick=(ev)=>{if(ev.target===sheet)closeSheet()};
    root.querySelectorAll("[data-v716-league]").forEach(btn=>btn.onclick=(ev)=>{
      ev.preventDefault();
      ev.stopPropagation();
      const picked=btn.dataset.v716League||"all";
      leagueFilter=(leagueFilter===picked)?"all":picked;
      closeSheet();
      setTimeout(render,40);
    });
    root.querySelectorAll("[data-real-match]").forEach(b=>b.onclick=()=>route("match",b.dataset.realMatch));
    root.querySelectorAll("[data-v712-day]").forEach(b=>b.onclick=()=>{
      v712MoveDate(Number(b.dataset.v712Day));
      realMatches();
    });
  };
  render();
  ...
  root.innerHTML=`<section class="m13-page v712-match-center">
    <header class="v712-page-head"><h1>MATCHES</h1><p>MATCH CENTER</p></header>
    <div class="v68-error-card"><b>DATI TEMPORANEAMENTE NON RAGGIUNGIBILI</b><p>La schermata resta disponibile anche se SportMonks non risponde.</p><button data-v68-retry>RIPROVA</button><small>${realEsc(e.message)}</small></div>
  </section>`;
  root.querySelector("[data-v68-retry]").onclick=()=>realMatches();
 }
}

function v74TeamAbsences(ctx,t){
  const xs=Array.isArray(ctx?.sidelined)?ctx.sidelined:[];
  const rows=xs.filter(x=>String(x?.participant_id??x?.team_id)===String(t.id)).map(x=>({name:x?.player?.display_name||x?.player?.name||x?.player_name||"",reason:x?.category||x?.type?.name||"Indisponibile"})).filter(x=>x.name);
  if(!rows.length)return "";
  return `<div class="v74-inline-abs"><b>ASSENZE</b>${rows.map(x=>`<p><span>${realEsc(x.name)}</span><em>${realEsc(x.reason)}</em></p>`).join("")}</div>`;
}
...
async function realMatchDetail(id){
 ...
 try{const [pack,fb]=await Promise.all([realApi(`/prematch?id=${encodeURIComponent(id)}`),realApi(`/fixture?id=${encodeURIComponent(id)}`)]),detail=fb?.data||fb,fx=pack.fixture||{},h=pack.teams?.home||realLoc(detail,'home')||{},a=pack.teams?.away||realLoc(detail,'away')||{},hHist=v67HistoryWithCurrent(detail,realHistory(pack,'home_history'),h.id),aHist=v67HistoryWithCurrent(detail,realHistory(pack,'away_history'),a.id),hm=realMetrics(hHist,h),am=realMetrics(aHist,a),hp=realMacro(hm),ap=realMacro(am),hs=realStanding(pack,h.id),as=realStanding(pack,a.id);const macros=[["threat","Threat Creation"],["defense","Defense Stability"],["form","Forma"],["opponent","Opponent Strength"],["pressing","Pressing / PPDA"],["availability","Availability"]];const tech=[["xG",hm.xg,am.xg],["Gol fatti",hm.gf,am.gf],["Gol subiti",hm.ga,am.ga],["Possesso",hm.pos,am.pos],["Tiri",hm.shots,am.shots],["Tiri in porta",hm.sot,am.sot],["Corner",hm.corners,am.corners],["Attacchi",hm.attacks,am.attacks],["Falli",hm.fouls,am.fouls]];const h2=realHistory(pack,'h2h').slice(0,5);
 document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page"><div class="v14-analysis-top"><button class="v14-back" data-nav="matches">←</button><button class="v14-star">☆</button></div><header class="v14-match-head"><h1>${realEsc(h.name)} — ${realEsc(a.name)}</h1><p>${realEsc(fx.league?.name||detail?.league?.name||'')} · ${realEsc(String(fx.starting_at||'').slice(0,10))} · ${realEsc(fmtTime(fx.starting_at))}</p><div class="v14-versus"><div>${realCrest(h)}<b>${realEsc(h.name)}</b><small>${hs?`Classifica ${hs}°`:'Classifica —'}</small></div><div class="v14-vs-center">${v60FixtureFinished(detail)?`<strong>${realScore(detail,h.id)??"—"} – ${realScore(detail,a.id)??"—"}</strong><small>FT</small>`:`<strong>VS</strong>`}</div><div>${realCrest(a)}<b>${realEsc(a.name)}</b><small>${as?`Classifica ${as}°`:'Classifica —'}</small></div></div></header>
 ...
 <section class="v14-card v14-model"><div class="v14-card-title">ANALISI DEL MODELLO</div><div class="v14-score-head"><div class="v14-score-team">${realCrest(h)}<b>${realEsc(h.name)}</b><strong>—</strong></div><div class="v14-score-diff"><small>MODEL SCORE</small><strong>—</strong><span>calibrazione</span></div><div class="v14-score-team">${realCrest(a)}<b>${realEsc(a.name)}</b><strong>—</strong></div></div><div class="v75-macro-list v756-match-macros">${macros.map(([k])=>{const meta=V14_MACRO_META[k],hv=hp[k],av=ap[k],hc=hv==null?"neutral":hv>=80?"good":hv>=60?"mid":"bad",ac=av==null?"neutral":av>=80?"good":av>=60?"mid":"bad";return `<div class="v752-macro-row v756-match-macro-row"><span class="v752-macro-icon ${k==="form"?"v756-form-icon":""}">${meta.icon}</span><div class="v752-macro-main"><div class="v752-macro-head v756-macro-head"><span class="v752-macro-title">${realEsc(meta.title)}<button class="info v756-info" data-info-title="${realEsc(meta.title)}" data-info="${encodeURIComponent(meta.parts)}">i</button></span></div><div class="v756-match-compare"><div class="v756-side"><div class="v756-side-head"><small>${realEsc(h.name)}</small><strong class="${hc}">${hv??"—"}</strong></div><em class="v752-macro-track ${hc}"><i style="width:${hv??0}%"></i></em></div><div class="v756-side"><div class="v756-side-head"><small>${realEsc(a.name)}</small><strong class="${ac}">${av??"—"}</strong></div><em class="v752-macro-track ${ac}"><i style="width:${av??0}%"></i></em></div></div></div></div>`}).join("")}</div><p class="real-note">V77.36 · 6 macroaree collegate a dati SportMonks reali. Opponent Strength usa avversari/classifica; Pressing usa Pressure Index quando disponibile; Availability usa sidelined. Se SportMonks non fornisce il dato, resta —.</p></section>
...
/* ============================================================
   V77.36 — REAL SPORTMONKS MACRO ENRICHMENT
   No fabricated source values:
   - Opponent Strength: SportMonks opponent positions / standings
   - Pressing: SportMonks Football Pressure Index
   - Availability: SportMonks sidelined records
   ============================================================ */
function v7736StandingsRows(pack){
  const d=pack?.standings?.ok===true?pack.standings.data:null;
  return Array.isArray(d)?d:Array.isArray(d?.data)?d.data:[];
}
function v7736StandingPos(rows,teamId){
  const r=(Array.isArray(rows)?rows:[]).find(x=>
    String(x?.participant_id??x?.participant?.id??x?.team_id)===String(teamId)
  );
  const p=realNum(r?.position??r?.rank);
  return p!=null?p:null;
}
function v7736OpponentStrength(fs,teamId,pack){
  const rows=v7736StandingsRows(pack);
  const leagueSize=rows.length>=10?rows.length:20;
  const vals=[];

  for(const f of (Array.isArray(fs)?fs:[]).slice(0,5)){
    const opp=realOpponent(f,teamId);
    if(!opp)continue;

    // Prefer the position stored on the historical fixture participant.
    // Fall back to the real SportMonks standings payload.
    let pos=realNum(opp?.meta?.position??opp?.position);
    if(pos==null)pos=v7736StandingPos(rows,opp.id);
    if(pos==null||pos<1)continue;

    const max=Math.max(2,leagueSize);
    const score=100-((Math.min(pos,max)-1)/(max-1))*100;
    vals.push(realClamp(score));
  }
  return vals.length?Math.round(vals.reduce((a,b)=>a+b,0)/vals.length):null;
}
function v7736PressureArray(f){
  if(Array.isArray(f?.pressure))return f.pressure;
  if(Array.isArray(f?.pressure?.data))return f.pressure.data;
  if(Array.isArray(f?.pressures))return f.pressures;
  return [];
}
function v7736PressingScore(fs,teamId){
  const fixtureMeans=[];

  for(const f of (Array.isArray(fs)?fs:[]).slice(0,5)){
    const vals=v7736PressureArray(f)
      .filter(x=>String(x?.participant_id??x?.team_id)===String(teamId))
      .map(x=>realNum(x?.pressure??x?.value??x?.data?.value))
      .filter(v=>v!=null);

    if(vals.length){
      fixtureMeans.push(vals.reduce((a,b)=>a+b,0)/vals.length);
    }
  }

  // Pressure Index itself is already a SportMonks numeric metric.
  return fixtureMeans.length
    ? Math.round(realClamp(fixtureMeans.reduce((a,b)=>a+b,0)/fixtureMeans.length))
    : null;
}
function v7736ActiveSidelined(ctx,teamId){
  if(!Array.isArray(ctx?.sidelined))return null;
  const now=Date.now();
  const seen=new Set();

  for(const x of ctx.sidelined){
    if(String(x?.participant_id??x?.team_id)!==String(teamId))continue;
    if(x?.completed===true)continue;

    // If SportMonks explicitly gives an end date already passed, do not
    // count the record as a current absence.
    const end=x?.end_date?new Date(x.end_date).getTime():null;
    if(Number.isFinite(end)&&end<now)continue;

    const key=String(x?.player_id??x?.player?.id??x?.id??"");
    if(key)seen.add(key);
  }
  return seen.size;
}
function v7736AvailabilityScore(ctx,teamId){
  const absent=v7736ActiveSidelined(ctx,teamId);
  if(absent==null)return null;

  // Availability index: share of a standard 20-player matchday pool
  // not currently listed by SportMonks as sidelined.
  // Source count is real; only the normalization converts it to 0–100.
  return Math.round(realClamp((1-Math.min(absent,20)/20)*100));
}
function v7736EnrichMacros(base,fs,teamId,pack,ctx){
  return {
    ...base,
    opponent:v7736OpponentStrength(fs,teamId,pack),
    pressing:v7736PressingScore(fs,teamId),
    availability:v7736AvailabilityScore(ctx,teamId)
  };
}


function v7737MacroVisualClass(key,val){
  const v=Number(val);
  if(!Number.isFinite(v))return "";

  // Pressing / PPDA-style reading: lower is better.
  // Keep broad bands because this screen currently shows a pressure-derived index.
  if(key==="pressing"){
    if(v<=8)return "macro-good";
    if(v<=15)return "macro-mid";
    return "macro-bad";
  }

  // Opponent Strength: explicit calibration agreed for the current 0–100 index.
  if(key==="opponent"){
    if(v>=65)return "macro-good";
    if(v>=45)return "macro-mid";
    return "macro-bad";
  }

  // Default visual logic used by the other macroareas: higher is better.
  if(v>=70)return "macro-good";
  if(v>=55)return "macro-mid";
  return "macro-bad";
}
...
function v58Technical(hm,am){return [["xG",hm.xg,am.xg],["xGA",hm.xga,am.xga],["Gol fatti",hm.gf,am.gf],["Gol subiti",hm.ga,am.ga],["Possesso",hm.pos,am.pos],["Tiri",hm.shots,am.shots],["Tiri in porta",hm.sot,am.sot],["Corner",hm.corners,am.corners]]}
...
function v64MarketWinner(items){
  const vals=items.map(x=>Number(x[1]));
...
function v722H2HStrip(h2,h,a){
  if(!Array.isArray(h2)||!h2.length)return "";
  let hw=0,aw=0,d=0;
  const results=h2.slice(0,5).map(f=>{
    const home=realLoc(f,"home")||{}, away=realLoc(f,"away")||{};
    const hs=realScore(f,home.id), as=realScore(f,away.id);
    if(hs==null||as==null)return "";
    const hg=String(home.id)===String(h.id)?hs:(String(away.id)===String(h.id)?as:null);
    const ag=String(home.id)===String(a.id)?hs:(String(away.id)===String(a.id)?as:null);
    if(hg==null||ag==null)return "";
    let cls="draw";
    if(hg>ag){hw++;cls="homewin"} else if(ag>hg){aw++;cls="awaywin"} else d++;
    return `<span class="${cls}"><b>${hg}-${ag}</b><small>${String(f.starting_at||"").slice(0,10)}</small></span>`;
  }).filter(Boolean).join("");

  return `<div class="v723-h2h">
    <div class="v723-h2h-title"><b>ULTIMI 5 SCONTRI DIRETTI</b><small>H2H</small></div>
    <div class="v723-h2h-summary">
      <div class="v723-h2h-club">${realCrest(h)}<span>${realEsc(h.name||"")}</span></div>
      <div class="v723-h2h-record">
        <div><strong>${hw}</strong><small>VITTORIE</small></div>
        <div><strong>${d}</strong><small>PAREGGI</small></div>
        <div><strong>${aw}</strong><small>VITTORIE</small></div>
      </div>
      <div class="v723-h2h-club away">${realCrest(a)}<span>${realEsc(a.name||"")}</span></div>
    </div>
    <div class="v723-h2h-results">${results}</div>
  </div>`;
}

function v722TechCompare(tech,h,a){
  const rows=(Array.isArray(tech)?tech:[]).filter(r=>r[1]!=null || r[2]!=null);
  if(!rows.length)return "";

  return `<section class="v722-tech-card">
    <header>
      <h2>DATI TECNICI</h2>
      <span>MEDIA ULTIME 5</span>
    </header>
    <div class="v722-tech-head">
      <div>${realCrest(h)}<b>${realEsc(h.name||"")}</b></div>
      <small>CONFRONTO</small>
      <div>${realCrest(a)}<b>${realEsc(a.name||"")}</b></div>
    </div>
    <div class="v722-tech-list">
      ${rows.map(r=>`<div class="v722-tech-row">
        <strong>${v58FmtMetric(r[0],r[1])}</strong>
        <span>${realEsc(r[0])}</span>
        <strong>${v58FmtMetric(r[0],r[2])}</strong>
      </div>`).join("")}
    </div>
  </section>`;
}

function v722TeamTech(metrics,team,sample){
  const fmt=(v,d)=>d==="pct"?realFmt(v,0)+"%":realFmt(v,d);
  const hero=[
    ["xG",metrics.xg,2,"Produzione offensiva"],
    ["xGA",metrics.xga,2,"Pericolosità concessa"]
  ].filter(r=>r[1]!=null);
  const rows=[
    ["Gol fatti",metrics.gf,1],
    ["Gol subiti",metrics.ga,1],
    ["Possesso",metrics.pos,"pct"],
    ["Tiri",metrics.shots,1],
    ["Tiri in porta",metrics.sot,1],
    ["Corner",metrics.corners,1]
  ].filter(r=>r[1]!=null);
  if(!hero.length&&!rows.length)return "";

  return `<section class="v19-card v723-team-tech">
    <div class="v19-chead"><b>DATI TECNICI</b><span>ULTIME ${sample||0}/5</span></div>
    <div class="v723-team-tech-head">${realCrest(team)}<div><b>${realEsc(team.name||"")}</b><small>MEDIA ULTIME ${sample||0}/5</small></div></div>
    ${hero.length?`<div class="v723-team-tech-hero">${hero.map(r=>`<div><small>${r[0]}</small><strong>${fmt(r[1],r[2])}</strong><em>${r[3]}</em></div>`).join("")}</div>`:""}
    ${rows.length?`<div class="v723-team-tech-list v74-tech-grid">${rows.map(r=>`<div><strong>${fmt(r[1],r[2])}</strong><span>${r[0]}</span></div>`).join("")}</div>`:""}
  </section>`;
}

function v722OfficialLineups(ctx,h,a,detail=null){
  const xs=Array.isArray(ctx?.lineups)?ctx.lineups:[];

  const starters=t=>xs.filter(x=>
    String(x?.team_id??x?.participant_id)===String(t?.id) &&
    Number(x?.type_id)===11 &&
    String(x?.formation_field||"").trim()!==""
  );

  const hs=starters(h), as=starters(a);

  // Explicit SportMonks flags, if available.
  const explicit=xs.some(x=>
    x?.confirmed===true ||
    x?.is_confirmed===true ||
    x?.official===true ||
    x?.is_official===true ||
    /confirm|official/i.test(String(x?.status||x?.state||x?.type?.name||""))
  );

  if(explicit && hs.length>=7 && as.length>=7) return true;

  // Real SportMonks response observed on Betis–Real Sociedad:
  // type_id 11 + formation_field identifies the actual XI.
  // When both teams have a complete XI close to kickoff, show FORMAZIONI
  // even if no separate "confirmed" flag is supplied.
  if(hs.length>=11 && as.length>=11){
    const start=Date.parse(String(detail?.starting_at||ctx?.starting_at||""));
    if(Number.isFinite(start)){
      const diff=start-Date.now();
      if(diff<=180*60*1000) return true; // within 3 hours of kickoff
    }

    const state=v67FixtureState(detail||ctx);
    if(state==="live"||state==="ft") return true;
  }
...
function v59HasOfficialLineups(ctx,h,a){
  return v722OfficialLineups(ctx,h,a);
}

function v758LineupTitle(ctx,h,a,detail){
  if(v722OfficialLineups(ctx,h,a,detail)) return "FORMAZIONI";

  const state=v67FixtureState(detail);
  const homeStarters=v59StarterLineups(ctx,h.id).length;
  const awayStarters=v59StarterLineups(ctx,a.id).length;

  // Once live/FT, lineups received from SportMonks are official even if
  // formation_field is incomplete for a few players.
  if((state==="live"||state==="ft") && homeStarters>=7 && awayStarters>=7)
    return "FORMAZIONI";

  return "PROBABILI FORMAZIONI";
}
...
async function realMatchDetail(id){
  ...
  try{
    const v69src=await v69SafeAnalysisPack(id),pack=v69src.pack,fb=v69src.fb;
    const detail=fb?.data||fb,fx=pack.fixture||{},h=pack.teams?.home||{},a=pack.teams?.away||{};
    const analysisSeasonId=fx?.season_id||fx?.season?.id||detail?.season_id||detail?.season?.id||null;
    const analysisLeagueId=fx?.league_id||fx?.league?.id||detail?.league_id||detail?.league?.id||null;
    const analysisLeagueName=fx?.league?.name||detail?.league?.name||"Campionato";
    const [allHHistoryRaw,allAHistoryRaw]=await Promise.all([
      h?.id?v69SafeTeamHistory(h.id,analysisSeasonId):Promise.resolve([]),
      a?.id?v69SafeTeamHistory(a.id,analysisSeasonId):Promise.resolve([])
    ]);
    const allHHistory=v76LeagueHistory(allHHistoryRaw,analysisLeagueId,15),
          allAHistory=v76LeagueHistory(allAHistoryRaw,analysisLeagueId,15);
    // Prediction snapshot: always reconstruct from the prematch pack (cutoff before kickoff),
    // never from a team-history request that may already contain the finished match.
    const preHfs=v76PreMatchLeagueHistory(realHistory(pack,"home_history"),fx,analysisLeagueId,5),
          preAfs=v76PreMatchLeagueHistory(realHistory(pack,"away_history"),fx,analysisLeagueId,5);
    if(!preHfs.length)preHfs.push(...v76PreMatchLeagueHistory(allHHistory,fx,analysisLeagueId,5));
    if(!preAfs.length)preAfs.push(...v76PreMatchLeagueHistory(allAHistory,fx,analysisLeagueId,5));
    const currentFt={...fx,...detail,starting_at:detail?.starting_at||fx?.starting_at,participants:(Array.isArray(detail?.participants)&&detail.participants.length?detail.participants:fx?.participants),scores:(Array.isArray(detail?.scores)&&detail.scores.length?detail.scores:fx?.scores),statistics:(Array.isArray(detail?.statistics)&&detail.statistics.length?detail.statistics:fx?.statistics),events:(Array.isArray(detail?.events)&&detail.events.length?detail.events:fx?.events)};
    const leaguePreH=preHfs.slice(0,5),leaguePreA=preAfs.slice(0,5);
    const hfs=v67HistoryWithCurrent(currentFt,leaguePreH,h.id),afs=v67HistoryWithCurrent(currentFt,leaguePreA,a.id);
    const hmRaw=v58Metrics(hfs,h),amRaw=v58Metrics(afs,a),hsplit=v58Metrics(hfs,h,"home"),asplit=v58Metrics(afs,a,"away");
    const preHm=v58Metrics(preHfs,h),preAm=v58Metrics(preAfs,a),preHsplit=v58Metrics(preHfs,h,"home"),preAsplit=v58Metrics(preAfs,a,"away");

    const v7522RollForm=(team,opp,preMetrics,postMetrics)=>{
      const preInternal=Array.isArray(preMetrics?.form)?preMetrics.form.slice():[];
      const post=Array.isArray(postMetrics?.form)?postMetrics.form.slice():[];

      // Global form order is already: oldest -> newest.
      // Do NOT reverse it again here, otherwise the rolling window selects
      // the wrong four previous league matches.
      const preChronological=preInternal.slice();

      if(!v60FixtureFinished(detail)) return preChronological;

      const gf=realScore(detail,team.id),ga=realScore(detail,opp.id);
      if(gf==null||ga==null){
        return post.length ? post.slice() : preChronological;
      }

      const current={gf,ga,r:gf>ga?"W":gf<ga?"L":"D"};
      const previousFour=preChronological.slice(-4);
      return [...previousFour,current].slice(-5);
    };

    // V77.8 — finished-match visible FORM: use the same real league history as Team Detail.
    // Keep prediction metrics untouched; only the dots shown in the FT match card are aligned.
    const v778FtLeagueForm=(team,history)=>{
      const rows=(Array.isArray(history)?history:[]).slice();
      const hasCurrent=rows.some(f=>String(f?.id??"")===String(currentFt?.id??""));
      if(v60FixtureFinished(detail)&&!hasCurrent) rows.push(currentFt);
      return realRecent(v76LeagueHistory(rows,analysisLeagueId,5),team.id);
    };
    const hForm=v60FixtureFinished(detail)?v778FtLeagueForm(h,allHHistoryRaw):v7522RollForm(h,a,preHm,hmRaw);
    const aForm=v60FixtureFinished(detail)?v778FtLeagueForm(a,allAHistoryRaw):v7522RollForm(a,h,preAm,amRaw);
    const v7520FormScore=seq=>seq.length?Math.round(seq.reduce((n,x)=>n+(x.r==="W"?3:x.r==="D"?1:0),0)/(seq.length*3)*100):null;
    const hm={...hmRaw,form:hForm,formScore:v7520FormScore(hForm)};
    const am={...amRaw,form:aForm,formScore:v7520FormScore(aForm)};

    // V77.36: current SportMonks fixture context is needed by Availability,
    // while historical fixture data supplies Opponent Strength and Pressure.
    const ctx=v58Context(pack)||detail;
    const hp=v7736EnrichMacros(v58Macro(hm),hfs,h.id,pack,ctx);
    const ap=v7736EnrichMacros(v58Macro(am),afs,a.id,pack,ctx);
    const hscore=v59ModelScore(hp,hm),ascore=v59ModelScore(ap,am);

    const predictionLocked=v67FixtureState(detail)==="live"||v67FixtureState(detail)==="ft";
    const predHm=predictionLocked?preHm:hm,predAm=predictionLocked?preAm:am,predHsplit=predictionLocked?preHsplit:hsplit,predAsplit=predictionLocked?preAsplit:asplit;
    const rawPred=v58Prediction(predHm,predAm,predHsplit,predAsplit);
    const computedPred=v59ShrinkPrediction(rawPred,predHm,predAm),computedConfidence=v59Confidence(computedPred,predHm,predAm);
    const frozenPrediction=predictionLocked?v68LoadPrediction(id):null;
    const pred=frozenPrediction?.pred||computedPred,confidence=frozenPrediction?.conf??computedConfidence;
    if(!frozenPrediction)v68SavePrediction(id,pred,confidence);
    const standingsH=realStanding(pack,h.id),standingsA=realStanding(pack,a.id),h2=realHistory(pack,"h2h").slice(0,5);
    const ref=v58Referee(ctx);
    ...
    const rawDiff=(hscore!=null&&ascore!=null)?hscore-ascore:null,diff=rawDiff==null?null:Math.abs(rawDiff),winner=rawDiff==null?"—":rawDiff>=0?h.name:a.name;
    const sampleHm=predictionLocked?preHm:hm,sampleAm=predictionLocked?preAm:am;
    const sampleWarning=(sampleHm.sample<5||sampleAm.sample<5);
    document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page">
      ...
      <header class="v14-match-head">
        ...
        <p>${realEsc(fx.league?.name||detail?.league?.name||"")} · ${realEsc(String(fx.starting_at||"").slice(0,10))} · ${realEsc(fmtTime(fx.starting_at))}${ref?` · Arbitro: ${realEsc(ref)}`:""}</p>
        <div class="v14-versus">
      ...
      <section class="v14-card">
        <div class="v14-card-title">PROBABILITÀ PRE-MATCH ${info("Probabilità pre-match","Snapshot calcolato sulle ultime 5 del campionato prima del calcio d'inizio. Dopo l'inizio della partita queste percentuali non vengono ricalcolate con il risultato o le statistiche live/post-match.")}</div>${v69src?.source==="fallback"?`<div class="v69-safe-note">Pacchetto pre-match non disponibile: analisi caricata dai dati reali ancora disponibili.</div>`:""}
        <div class="v14-markets">${markets.map(([title,vals])=>{const nums=vals.map(v=>Number(v[1])),max=Math.max(...nums),best=nums.filter(v=>v===max).length===1?max:null;return `<div class="v14-market"><b>${title}</b><div>${vals.map(v=>`<span class="${Number(v[1])===best?"best":""}"><small>${v[0]}</small><strong>${v[1]}%</strong></span>`).join("")}</div></div>`}).join("")}</div>
        ...
        ${sampleWarning?`<div class="v59-sample-warning"><b>CAMPIONE PARZIALE</b><span>${realEsc(h.name)}: ${sampleHm.sample}/5 · ${realEsc(a.name)}: ${sampleAm.sample}/5. Le probabilità sono rese più conservative automaticamente.</span></div>`:""}
        ${v722H2HStrip(h2,h,a)}
      </section>

      <section class="v14-card v14-model">
        <div class="v14-card-title">ANALISI DEL MODELLO <small class="v59-last5">${v76Last5Label(analysisLeagueName)}</small></div>
        <div class="v14-score-head">
          ...
          <div class="v14-score-diff"><small>MODEL SCORE</small><strong>${diff==null?"—":"+"+diff}</strong><span>${diff==null?"campione non confrontabile":`Vantaggio ${realEsc(winner)}`}</span></div>
        ...
        <div class="v75-macro-list v756-match-macros">${macros.map(([k])=>{const meta=V14_MACRO_META[k],hv=hm.sample<3?null:hp[k],av=am.sample<3?null:ap[k],hc=hv==null?"neutral":hv>=80?"good":hv>=60?"mid":"bad",ac=av==null?"neutral":av>=80?"good":av>=60?"mid":"bad";return `<div class="v752-macro-row v756-match-macro-row"><span class="v752-macro-icon ${k==="form"?"v756-form-icon":""}">${meta.icon}</span><div class="v752-macro-main"><div class="v752-macro-head v756-macro-head"><span class="v752-macro-title">${realEsc(meta.title)}<button class="info v756-info" data-info-title="${realEsc(meta.title)}" data-info="${encodeURIComponent(meta.parts)}">i</button></span></div><div class="v756-match-compare"><div class="v756-side"><div class="v756-side-head"><small>${realEsc(h.name)}</small><strong class="${hc}">${hv??"—"}</strong></div><em class="v752-macro-track ${hc}"><i style="width:${hv??0}%"></i></em></div><div class="v756-side"><div class="v756-side-head"><small>${realEsc(a.name)}</small><strong class="${ac}">${av??"—"}</strong></div><em class="v752-macro-track ${ac}"><i style="width:${av??0}%"></i></em></div></div></div></div>`}).join("")}</div>
      ...
      <div class="v14-accordions">
        ${/* V77.9: Dati Tecnici hidden on FT; Post-Match is the canonical final-data block */""}${v67FixtureState(detail)==="ft"?"":v722TechCompare(tech,h,a)}

        <section class="v742-match-form">
          <div class="v742-form-title"><b>FORMA</b><small>${v76Last5Label(analysisLeagueName)}</small></div>
          
          <div class="v742-form-team"><span>${realCrest(h)}<b>${realEsc(h.name)}</b></span><div>${realFormDots(hForm)}</div><em>${hm.sample}/5</em></div>
          <div class="v742-form-team"><span>${realCrest(a)}<b>${realEsc(a.name)}</b></span><div>${realFormDots(aForm)}</div><em>${am.sample}/5</em></div>
        </section>



        <details class="v14-fold">
          <summary>${v758LineupTitle(ctx,h,a,detail)} <span class="v18-toggle">+</span></summary>
          <div class="v14-fold-body v60-formation-wrap"><div class="v60-formation-tabs"><button class="active" data-v60-formtab="home">${realEsc(h.name)}</button><button data-v60-formtab="away">${realEsc(a.name)}</button></div><div data-v60-formbody="home">${v59FormationField(ctx,h)}${v74TeamAbsences(ctx,h)}</div><div data-v60-formbody="away" hidden>${v59FormationField(ctx,a)}${v74TeamAbsences(ctx,a)}</div></div>
        </details>


        ${ref?`<details class="v14-fold">
          <summary>ARBITRO <span class="v18-toggle">+</span></summary>
          <div class="v14-fold-body"><div class="v14-ref-grid"><div><small>Arbitro</small><b>${realEsc(ref)}</b></div></div></div>
        </details>`:""}
      </div>
    </div>`;bind();
...
/* V63 — bottom navigation single-tap delegation */
if(!window.__SPACE16154_BOTTOM_NAV_V63){
  window.__SPACE16154_BOTTOM_NAV_V63=true;
  document.addEventListener("pointerup",e=>{
    const b=e.target.closest?.(".bottom-nav .nav-item[data-nav]");
    if(!b)return;
    e.preventDefault();
    e.stopPropagation();
    route(b.dataset.nav);
...
function route(n,id){
 ...
 const navState=n==="match"?"matches":n==="team"||n==="real-team"||n==="standings"?"analysis":n==="performance"||n==="confidence"||n==="live"||n==="favorites"||n==="live-match"?"more":n;
 ...
 else if(n==="signals")realSignalsPage();
 else if(n==="more")more();
 ...
 else if(n==="real-team")v67RealTeamProfile(id);
 else if(n==="performance")performance();
 else if(n==="confidence")confidencePage();
 else if(n==="live")v67MatchCenter();
...
route("dashboard");
// V77.34 — one-time future snapshot 1X2-triplet backfill, then normal automatic tracker.
(function(){
  const key="space16154:v77.34:triplet-backfill-boot";
  let done=false;
...
setTimeout(()=>v7714AutomaticTracker().catch(()=>{}),9000);
setTimeout(()=>v7735EnsureFutureTriplets().catch(()=>{}),14000);
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=12.2.1-v72-compact-analysis",{updateViaCache:"none"}).catch(()=>{}));


/* V71.9 dashboard visual-only enhancer: mirrors the already-rendered confidence text. */
(function(){
  const syncV719Confidence=()=>{
    document.querySelectorAll('.v716-dash-prob-head > span').forEach(el=>{
      const txt=(el.textContent||'').trim();
      const m=txt.match(/(\d+(?:[.,]\d+)?%)/);
      if(m) el.setAttribute('data-v719-confidence',m[1]);
    });
  };
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',syncV719Confidence);
  else syncV719Confidence();
  try{
    new MutationObserver(syncV719Confidence).observe(document.documentElement,{subtree:true,childList:true});
  }catch(e){}
})();


function v7737ApplyMacroVisuals(){
  try{
    const labels=[...document.querySelectorAll("h3,h4,b,strong,div,span")];
    const configs=[
      {label:"Pressing / PPDA",key:"pressing"},
      {label:"Opponent Strength",key:"opponent"}
    ];

    for(const cfg of configs){
      const title=labels.find(el=>String(el.textContent||"").trim()===cfg.label);
      if(!title)continue;

      // Walk upward to the nearest macro block containing two progress bars.
      let row=title;
      for(let i=0;i<6 && row;i++,row=row.parentElement){
        const bars=row.querySelectorAll?.(".macro-bar,.macro-track,.real-bar,.bar-track,[class*='bar']");
        const nums=[...row.querySelectorAll?.("strong,b,[class*='score'],[class*='value']")||[]]
          .filter(el=>/^\s*\d+(?:[.,]\d+)?\s*$/.test(String(el.textContent||"")));
        if(nums.length>=2){
          nums.slice(0,2).forEach(el=>{
            const v=Number(String(el.textContent).replace(",","."));
            const cls=v7737MacroVisualClass(cfg.key,v);
            el.classList.remove("macro-good","macro-mid","macro-bad","v7737-good","v7737-mid","v7737-bad");
            el.classList.add(cls==="macro-good"?"v7737-good":cls==="macro-mid"?"v7737-mid":"v7737-bad");
          });

          // Try to color the two fill bars according to their corresponding numeric values.
          const fills=[...row.querySelectorAll("[class*='fill'],[class*='progress']")].filter(el=>el!==row);
          fills.slice(0,2).forEach((el,idx)=>{
            const n=nums[idx];
            if(!n)return;
            const v=Number(String(n.textContent).replace(",","."));
            const cls=v7737MacroVisualClass(cfg.key,v);
            el.classList.remove("v7737-good-bg","v7737-mid-bg","v7737-bad-bg");
            el.classList.add(cls==="macro-good"?"v7737-good-bg":cls==="macro-mid"?"v7737-mid-bg":"v7737-bad-bg");
          });
          break;
        }
      }
...
setTimeout(()=>v7737ApplyMacroVisuals(),120);
document.addEventListener("click",()=>setTimeout(()=>v7737ApplyMacroVisuals(),220),true);

...
function v33RatingTrend(teamId,league){
  ...
  const baseTrend=[row[9]-6,row[9]-4,row[9]-5,row[9]-2,row[9]];
  const t={id:row[0],abbr:row[1],name:row[2],league:league||"Serie A",rating:row[9],trend:baseTrend[4]-baseTrend[0]};
  const points = [
    {d:"20 LUG",v:76,type:"match"},
    {d:"24 LUG",v:80,type:"match"},
    {d:"31 LUG",v:82,type:"match"},
    {d:"4 AGO",v:85,type:"update"},
    {d:"11 AGO",v:88,type:"match"}
  ];
  const factors = [
  ...
  <main class="v33-page v36-trend-page">
    ...
    <section class="v19-card v36-chart-card">
      ...
      <div class="v36-chart-wrap">
        ...
        <div class="v36-dates">${points.map(p=>`<span>${p.d}</span>`).join("")}</div>
      </div>
      <div class="v36-event">
        <div><small>4 AGO · PARTITA</small><b>Milan 2–0 Roma</b></div>
        <div><small>RATING</small><b>82 → 85</b><strong class="pos">+3</strong></div>
      </div>
    </section>

    <section class="v19-card v36-why">
    ...
    <section class="v19-card v36-history">
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=8.8.1-ui-test-v36.1",{updateViaCache:"none"}).catch(()=>{}));

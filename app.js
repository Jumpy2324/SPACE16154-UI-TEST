...
function v68PredictionKey(id){return `space16154:prematch:${id}`}
 ...
 try{
  ...
  try{
    const p=await realApi(`/prematch?id=${m.id}`),h=p.teams?.home||m.h,a=p.teams?.away||m.a,
      hm=v58Metrics(realHistory(p,"home_history"),h),am=v58Metrics(realHistory(p,"away_history"),a),
      hh=v58Metrics(realHistory(p,"home_history"),h,"home"),aa=v58Metrics(realHistory(p,"away_history"),a,"away");
  ...
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   ...
   <section class="v716-dash-card">
    ...
    <div class="v716-dash-prob-head">
      <strong>${state==="ft"?"PROBABILITÀ PRE-MATCH":"ESITO PIÙ PROBABILE"}</strong>
...
async function v67RealTeamProfile(payload){
  ...
  try{
    ...
    const history=await v69SafeTeamHistory(teamId,null);
    ...
    root.innerHTML=`<div class="page v67-team-page">
      ...
      <header class="v67-team-head v742-team-head">${realCrest(team)}<div class="v742-team-id"><small>${realEsc(comp.name||"")}</small><h1>${realEsc(team.name||"Squadra")}</h1><span>${row?.position?`${row.position}° in classifica`:"Classifica —"}</span><div class="v742-team-form"><b>ULTIME 5</b>${realFormDots(metrics.form)}<em>${sample}/5</em></div></div></header>

      ${v7524HistoryDebug(team,history)}
...
async function v69SafeTeamHistory(teamId, seasonId=null){
  ...
  // GLOBAL FORM RULE:
  // use the last 5 official matches across ALL competitions first.
  // Season-filtered history is only a fallback if the all-competition endpoint
  // is unavailable or empty.
  try{
    const allCompetitions=await load(false);
    if(allCompetitions.length)return allCompetitions;
    if(seasonId)return await load(true);
  }catch{
    try{
      if(seasonId)return await load(true);
    }catch{}
...
async function realMatchDetail(id){
  ...
  try{
    ...
    const [allHHistory,allAHistory]=await Promise.all([
      h?.id?v69SafeTeamHistory(h.id,null):Promise.resolve([]),
      a?.id?v69SafeTeamHistory(a.id,null):Promise.resolve([])
    ]);
    const preHfs=allHHistory.length?allHHistory:realHistory(pack,"home_history"),
          preAfs=allAHistory.length?allAHistory:realHistory(pack,"away_history");
    ...
    const hfs=v67HistoryWithCurrent(currentFt,preHfs,h.id),afs=v67HistoryWithCurrent(currentFt,preAfs,a.id);
    ...
    const rawPred=v58Prediction(predHm,predAm,predHsplit,predAsplit),pred=v59ShrinkPrediction(rawPred,predHm,predAm),confidence=v59Confidence(pred,predHm,predAm);
    ...
    document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page">
      ...
      <section class="v14-card">
        <div class="v14-card-title">PROBABILITÀ DEL MODELLO ${info("Probabilità","Stima preliminare SPACE16154. Quando una squadra ha meno di 5 gare utili, il modello riduce automaticamente l'aggressività della previsione.")}</div>${v69src?.source==="fallback"?`<div class="v69-safe-note">Pacchetto pre-match non disponibile: analisi caricata dai dati reali ancora disponibili.</div>`:""}
      ...
      <div class="v14-accordions">
        ...
        <section class="v742-match-form">
          <div class="v742-form-title"><b>FORMA</b><small>ULTIME 5</small></div>

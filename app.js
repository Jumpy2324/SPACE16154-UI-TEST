...
const CONFIG={
  version:"12.0.0-v70-history-calendar",
...
function v67FixtureStatusHtml(f){
  ...
  return `<div class="v67-fixture-status upcoming"><strong>${realEsc(f?.time||String(f?.starting_at||"").slice(11,16)||"—")}</strong><span>PROGRAMMATA</span></div>`;
}
...
async function realMatches(){
 const date=dashDateMeta().id,root=document.querySelector("#main");
 root.innerHTML=`<section class="m13-page"><div class="empty-note">Caricamento SportMonks…</div></section>`;
 try{
  ...
  const home=()=>{
   root.innerHTML=`<section class="m13-page">
    <div class="m13-titlebar v68-matches-head"><h1 class="v23-matches-title">MATCHES</h1><p>MATCH CENTER</p></div>
    <div class="m13-leagues">${leagues.map(l=>`<button class="m13-league v66-analysis-twin" data-rl="${l.key}"><span class="v62-league-center">${v60LeagueLogo(l)}<b>${realEsc(l.name)}</b></span></button>`).join("")}</div>
   </section>`;
   root.querySelectorAll("[data-rl]").forEach(b=>b.onclick=()=>view(leagues.find(l=>l.key===b.dataset.rl)));
  };
  const view=l=>{
   const arr=fs.filter(f=>f.leagueName===l.name);
   root.innerHTML=`<section class="m13-page v23-matches-league">
    <div class="m13-leaguehead v23-leaguehead"><button class="m13-back" data-rback>←</button><div class="m13-leagueidentity">${v60LeagueLogo(l)}<h1>${realEsc(l.name)}</h1></div><button class="v60-table-shortcut" data-stand="${l.key}">CLASSIFICA</button></div>
    <div class="m13-fixtures v23-league-cards">${arr.length?arr.map(f=>`<article class="v23-match-card"><div class="v23-time v67-match-state">${v67FixtureStatusHtml(f)}</div><div class="v23-match-row"><div class="v23-team home">${realCrest(f.h)}<b>${realEsc(f.h.name||"")}</b></div><button class="v23-analysis" data-real-match="${f.id}">ANALISI</button><div class="v23-team away"><b>${realEsc(f.a.name||"")}</b>${realCrest(f.a)}</div></div></article>`).join(""):`<div class="empty-note">Nessuna partita reale disponibile per questa data.</div>`}</div>
   </section>`;
   root.querySelector("[data-rback]").onclick=home;
   root.querySelector("[data-stand]").onclick=()=>v60StandingsPage(l.key);
   root.querySelectorAll("[data-real-match]").forEach(b=>b.onclick=()=>route("match",b.dataset.realMatch));
  };
  home();
 }catch(e){
  root.innerHTML=`<section class="m13-page v68-fetch-error">
   <button class="m13-back" data-nav="dashboard">← DASHBOARD</button>
   <div class="v68-error-card"><b>DATI TEMPORANEAMENTE NON RAGGIUNGIBILI</b><p>La pagina Matches non perde più l'interfaccia se una chiamata SportMonks fallisce. Riprova il caricamento.</p><button data-v68-retry>RIPROVA</button><small>${realEsc(e.message)}</small></div>
  </section>`;
  bind();
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=12.0.0-v70-history-calendar",{updateViaCache:"none"}).catch(()=>{}));

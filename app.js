...
async function v67RealTeamProfile(payload){
  ...
  try{
    ...
    root.innerHTML=`<div class="page v67-team-page">
      ...
      <section class="v19-card v68-macro-card"><div class="v19-chead"><b>MACROAREE</b><span>${sample<3?`PRELIMINARI · ${sample}/5`:`${sample}/5 GARE`}</span></div>
        <div class="v19-macros v20-macros">${macroList.map(([k,title])=>{const m=V14_MACRO_META[k],v=macro[k];return `<div class="v19-macro v20-macro"><span class="v20-free-icon">${m.icon}</span><div><b>${realEsc(title)}</b><small>${realEsc(m.desc)}</small><em class="${v==null?"neutral":v14Band(v)}"><i style="width:${v??0}%"></i></em></div><strong>${v??"—"}</strong></div>`}).join("")}</div>
        <p class="real-note v68-real-note">${sample<3?"Le macroaree sono ancora preliminari: acquistano affidabilità con l’aumento del campione reale. ":""}Nessun valore demo: “—” significa dato non disponibile o campione insufficiente.</p>
      </section>

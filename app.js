...
function v33RatingTrend(teamId,league){
  ...
  const points=trendData.points.map((p,i)=>{
    const meta=[
      {label:"20 LUG · PARTITA",title:"Milan 2–1 Atalanta"},
      {label:"24 LUG · PARTITA",title:"Milan 1–0 Torino"},
      {label:"31 LUG · PARTITA",title:"Milan 3–1 Bologna"},
      {label:"4 AGO · PARTITA",title:"Milan 2–0 Roma"},
  ...
  const min=74,max=90, W=100,H=42;
  const xs=[7,29,50,72,94];
  const y=v=>36-((v-min)/(max-min))*27;
  ...
  <main class="v33-page v36-trend-page">
    ...
    <section class="v19-card v36-chart-card v37-glass">
      ...
      <div class="v36-chart-wrap">
        <div class="v36-axis"><span>90</span><span>85</span><span>80</span><span>75</span></div>
        <svg class="v36-chart" viewBox="0 0 100 42" preserveAspectRatio="none">
          <line x1="0" y1="${y(90)}" x2="100" y2="${y(90)}" class="grid"/>
          <line x1="0" y1="${y(85)}" x2="100" y2="${y(85)}" class="grid"/>
          <line x1="0" y1="${y(80)}" x2="100" y2="${y(80)}" class="grid"/>
          <line x1="0" y1="${y(75)}" x2="100" y2="${y(75)}" class="grid"/>
          <path d="${path}" class="line"/>
          ${points.map((p,i)=>`<circle cx="${xs[i]}" cy="${y(p.v)}" r="${p.type==="update"?2.2:1.8}" class="${p.type}"/><text x="${xs[i]}" y="${y(p.v)-3.2}" text-anchor="middle">${p.v}</text>`).join("")}
        </svg>
        <div class="v36-dates">${points.map((p,i)=>`<span>${p.d}</span>`).join("")}</div>
      </div>
    ...
    <section class="v19-card v36-why v37-glass">
      ...
      <p class="v36-summary">La crescita arriva soprattutto dalla produzione offensiva, dalla solidità difensiva e dalla forma recente. La disponibilità della rosa ha invece inciso negativamente.</p>
    </section>
  ...
  const renderSelection=(idx)=>{
    ...
    document.querySelectorAll(".v36-chart circle[data-trend-index]").forEach((el,j)=>el.classList.toggle("selected",j===selected));
    const p=points[selected];
    event.innerHTML=`<div><small>${p.label}</small><b>${p.title}</b></div>
      <div class="v38-event-rating"><small>RATING</small><b>${p.prev} → ${p.v} <em class="${p.delta>=0?"pos":"neg"}">${p.delta>=0?"+":""}${p.delta}</em></b></div>`;
  };
  if(chart){
    ...
    chart.querySelectorAll("circle[data-trend-index]").forEach((el,i)=>el.addEventListener("click",(e)=>{e.stopPropagation();renderSelection(i)}));
  }
}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=9.0.0-ui-test-v38",{updateViaCache:"none"}).catch(()=>{}));

...
async function v67RealTeamProfile(payload){
  ...
  <div class="v752-macro-main">
    <div class="v752-macro-head">
      ...
      <strong class="${v7738MacroTone(k,v)}" style="color:${v7740MacroHex(k,v)} !important">${v??"—"}</strong>
    </div>
    <em class="v752-macro-track ${v7738MacroTone(k,v)}"><i style="width:${v??0}%;background:${v7740MacroHex(k,v)} !important"></i></em>
  </div>
</button>`}).join("")}</div>
...
async function realMatchDetail(id){
 ...
 <section class="v14-card v14-model"><div class="v14-card-title">ANALISI DEL MODELLO</div><div class="v14-score-head"><div class="v14-score-team">${realCrest(h)}<b>${realEsc(h.name)}</b><strong>—</strong></div><div class="v14-score-diff"><small>MODEL SCORE</small><strong>—</strong><span>calibrazione</span></div><div class="v14-score-team">${realCrest(a)}<b>${realEsc(a.name)}</b><strong>—</strong></div></div><div class="v75-macro-list v756-match-macros">${macros.map(([k])=>{const meta=V14_MACRO_META[k],hv=hp[k],av=ap[k],hc=v7738MacroTone(k,hv),ac=v7738MacroTone(k,av);return `<div class="v752-macro-row v756-match-macro-row"><span class="v752-macro-icon ${k==="form"?"v756-form-icon":""}">${meta.icon}</span><div class="v752-macro-main"><div class="v752-macro-head v756-macro-head"><span class="v752-macro-title">${realEsc(meta.title)}<button class="info v756-info" data-info-title="${realEsc(meta.title)}" data-info="${encodeURIComponent(meta.parts)}">i</button></span></div><div class="v756-match-compare"><div class="v756-side"><div class="v756-side-head"><small>${realEsc(h.name)}</small><strong class="${hc}" style="color:${v7740MacroHex(k,hv)} !important">${hv??"—"}</strong></div><em class="v752-macro-track ${hc}"><i style="width:${hv??0}%;background:${v7740MacroHex(k,hv)} !important"></i></em></div><div class="v756-side"><div class="v756-side-head"><small>${realEsc(a.name)}</small><strong class="${ac}" style="color:${v7740MacroHex(k,av)} !important">${av??"—"}</strong></div><em class="v752-macro-track ${ac}"><i style="width:${av??0}%;background:${v7740MacroHex(k,av)} !important"></i></em></div></div></div></div>`}).join("")}</div><p class="real-note">V77.36 · 6 macroaree collegate a dati SportMonks reali. Opponent Strength usa avversari/classifica; Pressing usa Pressure Index quando disponibile; Availability usa sidelined. Se SportMonks non fornisce il dato, resta —.</p></section>
...
function v7738MacroTone(key,val){
  const v=Number(val);
  if(!Number.isFinite(v))return "neutral";

  // PRESSING / PPDA: lower = better.
  // Visual thresholds only; model calculation is unchanged.
  if(key==="pressing"){
    if(v<=8)return "good";
    if(v<=15)return "mid";
    return "bad";
  ...
  // OPPONENT STRENGTH: agreed 0–100 visual scale.
  if(key==="opponent"){
    if(v>=65)return "good";
    if(v>=45)return "mid";
    return "bad";
  ...
  // Existing visual semantics for the other four macroareas.
  if(v>=80)return "good";
  if(v>=60)return "mid";
  return "bad";
}

function v7740MacroHex(key,val){
  const tone=v7738MacroTone(key,val);
  return tone==="good"?"#50c878":tone==="mid"?"#e09b39":tone==="bad"?"#d85a5a":"#8b8e94";
}
  ...
  try{
    ...
    document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page">
      ...
      <section class="v14-card v14-model">
        ...
        <div class="v75-macro-list v756-match-macros">${macros.map(([k])=>{const meta=V14_MACRO_META[k],hv=hm.sample<3?null:hp[k],av=am.sample<3?null:ap[k],hc=v7738MacroTone(k,hv),ac=v7738MacroTone(k,av);return `<div class="v752-macro-row v756-match-macro-row"><span class="v752-macro-icon ${k==="form"?"v756-form-icon":""}">${meta.icon}</span><div class="v752-macro-main"><div class="v752-macro-head v756-macro-head"><span class="v752-macro-title">${realEsc(meta.title)}<button class="info v756-info" data-info-title="${realEsc(meta.title)}" data-info="${encodeURIComponent(meta.parts)}">i</button></span></div><div class="v756-match-compare"><div class="v756-side"><div class="v756-side-head"><small>${realEsc(h.name)}</small><strong class="${hc}" style="color:${v7740MacroHex(k,hv)} !important">${hv??"—"}</strong></div><em class="v752-macro-track ${hc}"><i style="width:${hv??0}%;background:${v7740MacroHex(k,hv)} !important"></i></em></div><div class="v756-side"><div class="v756-side-head"><small>${realEsc(a.name)}</small><strong class="${ac}" style="color:${v7740MacroHex(k,av)} !important">${av??"—"}</strong></div><em class="v752-macro-track ${ac}"><i style="width:${av??0}%;background:${v7740MacroHex(k,av)} !important"></i></em></div></div></div></div>`}).join("")}</div>
...
window.SPACE16154_BUILD="V77.39-ALL-MACRO-RENDERERS-COLOR-FIX";

window.SPACE16154_BUILD="V77.40-INLINE-MACRO-COLOR-FIX";

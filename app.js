...
const CONFIG={
  version:"10.2.0-ui-test-v52-real-data-polish",
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=10.2.0-ui-test-v52-real-data-polish",{updateViaCache:"none"}).catch(()=>{}));
...
function smDetailStatRows(detail){
  const stats=Array.isArray(detail?.statistics)?detail.statistics:[];
  if(!stats.length) return `<div class="sm51-empty"><b>STATISTICHE PARTITA NON ANCORA DISPONIBILI</b><span>La fixture è pre-match. SportMonks popolerà qui i dati gara quando disponibili.</span></div>`;
  const rows=stats.slice(0,24).map(st=>{
    const label=st?.type?.name||st?.type?.developer_name||`Stat ${st?.type_id??""}`;
    const val=st?.data?.value??st?.value??"—";
    const side=st?.location||st?.participant_id||"";
    return `<div class="sm51-stat"><span>${escapeHtml(label)}</span><b>${escapeHtml(val)}</b><small>${escapeHtml(side)}</small></div>`;
  }).join("");
  return `<div class="sm51-stats">${rows}</div>`;
}
function smDetailModalHtml(state,fixture=null,error=""){
  if(state==="loading") return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button><div class="sm51-kicker">SPORTMONKS LIVE DATA</div><h2>Caricamento dati partita…</h2><div class="sm51-spinner"></div></div></div>`;
  ...
  const f=normalizeFixture(fixture||{}),h=f.home||{},a=f.away||{};
  const league=fixtureLeagueName(fixture)||"—";
  const stateName=fixture?.state?.name||"—";
  const ts=Number(fixture?.starting_at_timestamp||0), dt=ts?new Date(ts*1000):null;
  const dateLabel=dt&&!Number.isNaN(dt.getTime())?dt.toLocaleDateString("sv-SE",{timeZone:"Europe/Rome"}):isoDate();
  const leagueImg=fixture?.league?.image_path||"";
  return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button>
    <div class="sm51-kicker">SPORTMONKS LIVE DATA</div><h2>DATI PARTITA</h2>
    <div class="sm51-meta"><span class="sm52-league-pill">${leagueImg?`<img src="${escapeHtml(leagueImg)}" alt="">`:""}${escapeHtml(league)}</span><span>${escapeHtml(dateLabel)}</span><span>${escapeHtml(f.time||"--:--")}</span></div>
    ...
    <div class="sm51-state">STATO: <b>${escapeHtml(stateName)}</b></div>
    <div class="sm51-section-title">STATISTICHE FIXTURE</div>${smDetailStatRows(fixture)}
    <div class="sm51-note">Questa schermata usa esclusivamente dati reali SportMonks. Le metriche proprietarie SPACE16154 verranno calcolate nel passaggio successivo.</div>
  </div></div>`;
}
function smCloseDetail(){smDetailModal?.remove();smDetailModal=null;}
async function smOpenDetail(id){
  if(!id)return;
  smCloseDetail(); document.body.insertAdjacentHTML("beforeend",smDetailModalHtml("loading")); smDetailModal=document.querySelector(".sm51-backdrop");
  smDetailModal.querySelectorAll("[data-smclose]").forEach(x=>x.onclick=smCloseDetail);
  try{
    const res=await fetch(`${bridgeBase()}/fixture?id=${encodeURIComponent(id)}`,{headers:{Accept:"application/json"},cache:"no-store"});
    const body=await res.json().catch(()=>({}));
    if(!res.ok){const detail=body?.sportmonks?.message||body?.message||body?.error||`HTTP ${res.status}`;throw new Error(detail)}
    smCloseDetail(); document.body.insertAdjacentHTML("beforeend",smDetailModalHtml("ready",body?.data||body)); smDetailModal=document.querySelector(".sm51-backdrop");
    smDetailModal.querySelectorAll("[data-smclose]").forEach(x=>x.onclick=smCloseDetail);
  }catch(e){smCloseDetail();document.body.insertAdjacentHTML("beforeend",smDetailModalHtml("error",null,e?.message||"Errore sconosciuto"));smDetailModal=document.querySelector(".sm51-backdrop");smDetailModal.querySelectorAll("[data-smclose]").forEach(x=>x.onclick=smCloseDetail)}
}

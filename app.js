...
function v7717MatchCard(f,{league=true,action=true,compact=false}={}){
  ...
  let scoreHtml=`<div class="v7717-score-vs">VS</div><div class="v7717-score-sub">${time}</div>`;

  if(st==="ft"){
    stateText="FT";
    stateClass="ft";
    scoreHtml=`<div class="v7717-score-main">${hs??"—"}–${as??"—"}</div><div class="v7717-score-sub">FT</div>`;
  }else if(st==="live"){
    stateText="LIVE";
    stateClass="live";
    scoreHtml=`<div class="v7717-score-main">${hs??0}–${as??0}</div><div class="v7717-score-sub live">● LIVE</div>`;
  }

  return `<article class="v7717-match-card ${compact?"compact":""}" data-real-match="${f.id}">
    <header class="v7717-match-header">
      <div class="v7717-league">${league?realEsc(leagueName):""}</div>
      <div class="v7717-state ${stateClass}">${stateText}</div>
    </header>
    <div class="v7717-match-body">
      ...
      <div class="v7717-score">${scoreHtml}</div>
      <div class="v7717-team">
        <div class="v7717-crest">${realCrest(a)}</div>
        <div class="v7717-name">${realEsc(a?.name||"—")}</div>
      </div>
    </div>
    ${action?`<footer class="v7717-match-footer">${st==="ft"?"POST-MATCH":"ANALISI"} <span>›</span></footer>`:""}
  </article>`;
}

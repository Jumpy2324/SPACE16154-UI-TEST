...
function v7716UnifiedMatchCard(f,{showLeague=true,showAction=true,compact=false}={}){
  ...
  let center="";
  if(st==="ft"){
    center=`<strong>${hs??"—"}–${as??"—"}</strong><small>FT</small>`;
  }else if(st==="live"){
    center=`<strong>${hs??0}–${as??0}</strong><small class="live">● LIVE</small>`;
  }else{
    center=`<strong class="vs">VS</strong><small>${realEsc(f?.time||fmtTime(f?.starting_at)||"")}</small>`;
  }

  return `<article class="v7716-match-card ${compact?"compact":""} ${st}" data-real-match="${f.id}">
    <div class="v7716-match-top">
      <span>${showLeague?realEsc(f?.leagueName||f?.league?.name||""):""}</span>
      <b>${st==="ft"?"FT":st==="live"?"LIVE":realEsc(f?.time||fmtTime(f?.starting_at)||"")}</b>
    </div>
    <div class="v7716-match-main">
      <div class="v7716-side home">
        ${realCrest(h)}
        <strong>${realEsc(h?.name||"—")}</strong>
      </div>
      <div class="v7716-center">${center}</div>
      <div class="v7716-side away">
        ${realCrest(a)}
        <strong>${realEsc(a?.name||"—")}</strong>
      </div>
    </div>
    ${showAction?`<div class="v7716-match-action">${st==="ft"?"POST-MATCH":"ANALISI"} <span>›</span></div>`:""}
  </article>`;
}
  ...
  try{
    ...
    const render=(filter="all")=>{
      ...
      list.innerHTML=groups.length?groups.map(([l,fs])=>`<section class="v67-center-league"><h2>${realEsc(l)}</h2>${fs.map(f=>{
        const h=f.h||realLoc(f,"home")||{},a=f.a||realLoc(f,"away")||{};
        return v7716UnifiedMatchCard(f,{showLeague:false,showAction:false,compact:true})}).join("")}</section>`).join(""):`<div class="empty-note">Nessuna partita per questo filtro.</div>`;
 ...
 try{
  ...
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   ...
   ${fs.slice(1,4).length ? `<section class="v5-upcoming">
    <div class="v2-panel-head"><h2>PROSSIME PARTITE</h2><button data-nav="matches">Vedi tutte ›</button></div>
    <div class="v5-upcoming-list">${fs.slice(1,4).map(x=>v7716UnifiedMatchCard(x,{showLeague:false,showAction:true,compact:true})).join("")}</div>
   </section>` : ""}
  </div>`;
  bind();
  document.querySelectorAll("[data-v63-date]").forEach(b=>{
    ...
    root.innerHTML=`<section class="m13-page v712-match-center">
      ...
      <div class="v712-fixture-list">
        ${visible.length?visible.map(f=>{
          ...
          return v7716UnifiedMatchCard(f,{showLeague:true,showAction:true,compact:false});

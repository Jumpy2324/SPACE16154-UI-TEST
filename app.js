...
async function v67RealTeamProfile(payload){
  ...
  try{
    const [b,fs]=await Promise.all([realApi(`/competition?key=${encodeURIComponent(leagueKey)}`),realFixtures(dashDateMeta().id).catch(()=>[])]);
    ...
    const todayTeam=fs.filter(f=>(f.participants||[]).some(p=>String(p.id)===String(teamId)) && v60FixtureFinished(f));
    const metrics=v58Metrics(todayTeam,team),macro=v58Macro(metrics),trend=v58PerfTrend(todayTeam,teamId);
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=11.9.1-v69-safe",{updateViaCache:"none"}).catch(()=>{}));

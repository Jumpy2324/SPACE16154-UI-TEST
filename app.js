...
const CONFIG={
  version:"10.1.0-ui-test-v51-fixture-data",
...
function smV50Normalize(f){
  ...
  return {raw:f,id:`sm-${f.id}`,smId:f.id,league:smV50LeagueName(f),time,h:{name:h.name||"Casa",code:String(h.short_code||h.name||"H").replace(/[^A-Za-z0-9]/g,"").slice(0,3).toUpperCase(),cls:""},a:{name:a.name||"Trasferta",code:String(a.short_code||a.name||"A").replace(/[^A-Za-z0-9]/g,"").slice(0,3).toUpperCase(),cls:""},state:f?.state?.name||""};
}
...
function smV50Shield(t,side=""){return `<span class="m13-shield ${side}"><b>${String(t?.code||"?").replace(/[<>]/g,"")}</b></span>`}
...
function dashboard(){
 ...
 root.innerHTML=`<div class="page v2-dashboard">
   ...
   `<section class="v6-hero-combined">
      ...
      <div class="v2-meta">${hero.league} <i>•</i> ${meta.short} <i>•</i> ${hero.time}</div>
      <div class="v2-match"><div class="v2-club">${smV50Shield(hero.h)}<strong>${hero.h.name.toUpperCase()}</strong></div><div class="v2-center"><b>VS</b><button disabled>DATI LIVE</button></div><div class="v2-club">${smV50Shield(hero.a,"away")}<strong>${hero.a.name.toUpperCase()}</strong></div></div>
   ...
   <section class="v5-upcoming"><div class="v2-panel-head"><h2>PROSSIME PARTITE</h2><button data-nav="matches">Vedi tutte ›</button></div><div class="v5-upcoming-list">
     ${(!loading&&dayMatches.length?dayMatches.slice(hero?1:0,4).map(x=>`<div class="v23-match-card"><div class="v23-time">${x.time}</div><div class="v23-match-row"><div class="v23-team home">${smV50Shield(x.h)}<b>${x.h.name}</b></div><button class="v23-analysis" disabled>DATI</button><div class="v23-team away"><b>${x.a.name}</b>${smV50Shield(x.a,"away")}</div></div><div class="v48-fixture-meta">${x.league}</div></div>`).join(""):`<div class="empty-note">${loading?"Caricamento…":"Nessun'altra partita disponibile per questa data."}</div>`)}
   </div></section>
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=10.1.0-ui-test-v51-fixture-data",{updateViaCache:"none"}).catch(()=>{}));
...
const shield=(x,c="")=>`<span class="m13-shield ${c}"><b>${escapeHtml(x)}</b></span>`;
...
function home(){
  return `<section class="m13-page"><div class="m13-titlebar"><h1 class="v23-matches-title">MATCHES</h1>${cal()}</div>
    ${connectionBanner()}
    <div class="m13-leagues">${leagues.map(l=>`<button class="m13-league" data-league="${l[0]}">${logo(l[2])}<span>${l[1]}</span>${apiState.status==="ready"?`<em>${leagueMatches(l).length}</em>`:""}</button>`).join("")}</div>
  </section>`;
}

function fixtureCard(f){
  const h=f.home||{},a=f.away||{};
  const localId=f.id?`sm-${f.id}`:"";
  const demoId=findDemoMatchId(h.name,a.name);
  const analysisId=demoId||"";
  return `<article class="v23-match-card">
    <div class="v23-time">${escapeHtml(f.time||"--:--")}</div>
    <div class="v23-match-row">
      <div class="v23-team home">${shield(teamCode(h))}<b>${escapeHtml(h.name||"Home")}</b></div>
      ${analysisId?`<button class="v23-analysis" data-analysis="${analysisId}">ANALISI</button>`:`<button class="v23-analysis" data-smfixture="${escapeHtml(String(f.id||""))}">DATI</button>`}
      <div class="v23-team away"><b>${escapeHtml(a.name||"Away")}</b>${shield(teamCode(a),"away")}</div>
    </div>
...
function view(l){
  ...
  return `<section class="m13-page v23-matches-league">
    ...
    <div class="m13-leaguehead v23-leaguehead"><button class="m13-back" data-back>←</button><div class="m13-leagueidentity">${logo(l[2])}<h1>${l[1]}</h1></div></div>
    ${connectionBanner()}
...
function smDetailModalHtml(state,fixture=null,error=""){
  ...
  return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button>
    ...
    <div class="sm51-meta"><span>${escapeHtml(league)}</span><span>${escapeHtml(isoDate())}</span><span>${escapeHtml(f.time||"--:--")}</span></div>
    <div class="sm51-teams"><div>${shield(teamCode(h))}<b>${escapeHtml(h.name||"Casa")}</b></div><strong>VS</strong><div><b>${escapeHtml(a.name||"Trasferta")}</b>${shield(teamCode(a),"away")}</div></div>

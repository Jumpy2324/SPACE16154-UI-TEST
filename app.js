  ...
  try{
    ...
    root.innerHTML=`<div class="page v67-team-page">
      ...
      <section class="v19-card v68-macro-card v75-team-macros"><div class="v19-chead"><b>MACROAREE</b><span>${sample<3?`PRELIMINARI · ${sample}/5`:`${sample}/5 GARE`}</span></div>
        <div class="v75-macro-list">${macroList.map(([k,title])=>{const m=V14_MACRO_META[k],v=macro[k];return `<button class="v75-macro-row" data-v75-macro="${realEsc(title)}" data-v75-desc="${realEsc(m.desc)}"><span class="v20-free-icon">${m.icon}</span><div class="v751-macro-main"><div class="v751-macro-name"><b>${realEsc(title)}</b><i class="v75-info-dot">i</i></div><em class="v751-macro-track ${v==null?"neutral":(v>=80?"good":v>=60?"mid":"bad")}"><i style="width:${v??0}%"></i></em></div><strong class="${v==null?"neutral":(v>=80?"good":v>=60?"mid":"bad")}">${v??"—"}</strong></button>`}).join("")}</div>

   ...
   <div class="dash-global-head v8-global-head">
     ...
     <div class="v8-date-pair" aria-label="Selettore data">
       <button class="v8-date-control" data-open-month>
         <small>MESE</small><strong>${MONTHS_IT[DASH_MONTH].slice(0,3).toUpperCase()}</strong><span>⌄</span>
       </button>
       <button class="v8-date-control day" data-open-day>
         <small>GIORNO</small><strong>${String(DASH_DAY).padStart(2,"0")}</strong><span>⌄</span>
       </button>
     </div>
   </div>
...
function v14TeamFormation(team,side){
 const names=side==="h"?["POR","TD","DC","DC","TS","CC","CC","TQ","AD","AS","ATT"]:["POR","TD","DC","DC","TS","MED","CC","CC","AD","AS","ATT"];
 return `<div class="v14-lineup"><div class="v14-lineup-title">${team.name}</div><div class="v14-pitch">${names.map((n,i)=>`<span class="v14-player p${i+1}">${n}</span>`).join("")}</div><small>Probabile formazione demo · diventerà ufficiale automaticamente quando disponibile.</small></div>`;
}
 ...
 document.querySelector("#main").innerHTML=`<div class="page v14-analysis-page">
  ...
  <header class="v14-match-head">
    ...
    <div class="v14-versus">
      ...
      <div class="v14-vs-center">${d.state!=="upcoming"?`<strong>${d.hs} — ${d.as}</strong>${v14Status(d,m)}`:`<strong>VS</strong><span>${m.time}</span>`}</div>
  ...
  <div class="v14-accordions">
    <details class="v14-fold"><summary>DATI TECNICI <span>⌄</span></summary><div class="v14-fold-body"><div class="compare-head"><span>INDICATORE</span><b>${m.h.code}</b><b>${m.a.code}</b></div>${technical(m).map(r=>`<div class="compare-row"><span>${r[0]} ${info(r[0],TECH_INFO[r[0]])}</span><b>${r[1]}</b><b>${r[2]}</b></div>`).join("")}</div></details>
    <details class="v14-fold"><summary>ASSENZE <span>⌄</span></summary><div class="v14-fold-body"><div class="absence-grid">${m.abs.map(a=>`<div><b>${a[0]}</b>${a[1].map(p=>`<p><strong>${p[0]}</strong><span class="${p[1]==="Out"?"out":"doubt"}">${p[1]}</span><small>${p[2]}</small></p>`).join("")}</div>`).join("")}</div></div></details>
    <details class="v14-fold"><summary>PROBABILI FORMAZIONI <span>⌄</span></summary><div class="v14-fold-body v14-lineups">${v14TeamFormation(m.h,"h")}${v14TeamFormation(m.a,"a")}</div></details>
    <details class="v14-fold"><summary>ARBITRO <span>⌄</span></summary><div class="v14-fold-body">${v14Referee(m)}</div></details>
  </div>
 </div>`;
 bind();
...
function v14StatsTab(m,stats){
 return `<div class="v14-live-stats"><div class="compare-head"><span>STATISTICA</span><b>${m.h.code}</b><b>${m.a.code}</b></div>${stats.map(r=>`<div class="compare-row"><span>${r[0]}</span><b>${r[1]}</b><b>${r[2]}</b></div>`).join("")}</div>`;
}
function v14PrematchTab(m,d){
 const f=topOutcome(m);
 const comparison=d.state==="ft"?`<div class="v14-vs-result"><h3>PREVISIONE vs RISULTATO</h3><p>Esito più probabile: <b>${f[0]} · ${f[1]}%</b></p><p>Risultato reale: <b>${d.hs} — ${d.as}</b></p><span>Lo snapshot pre-match è conservato senza modifiche retroattive.</span></div>`:"";
 ...
 <div class="v14-pre-probs"><span>1 <b>${m.p.one}%</b></span><span>X <b>${m.p.x}%</b></span><span>2 <b>${m.p.two}%</b></span><span>Gol <b>${m.p.gg}%</b></span><span>No Gol <b>${m.p.ng}%</b></span><span>Over <b>${m.p.over}%</b></span><span>Under <b>${m.p.under}%</b></span></div>
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=7.0.0-ui-test-v14",{updateViaCache:"none"}).catch(()=>{}));
...
const games={
"serie-a":[["MIL","Milan","18:00","Roma","ROM"],["INT","Inter","20:45","Napoli","NAP"]],
"premier":[["ARS","Arsenal","18:30","Chelsea","CHE"],["MCI","Man City","21:00","Liverpool","LIV"]],
"laliga":[["BAR","Barcelona","20:00","Sevilla","SEV"],["RMA","Real Madrid","22:00","Valencia","VAL"]],
"bundesliga":[["BAY","Bayern","20:30","Leverkusen","LEV"],["BVB","Dortmund","21:00","Leipzig","RBL"]],
"ligue1":[["PSG","PSG","21:00","Lyon","LYO"],["MON","Monaco","19:00","Marseille","OM"]]};
...
function bind(root){
 ...
 root.querySelectorAll("[data-analysis]").forEach(b=>b.onclick=()=>{let n=[...document.querySelectorAll("button,a")].find(x=>/^Analysis$|^Analisi$/i.test((x.textContent||"").trim()));if(n)n.click()});
}

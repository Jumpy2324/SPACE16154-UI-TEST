...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=9.9.0-ui-test-v49-sportmonks-live",{updateViaCache:"none"}).catch(()=>{}));
...
const leagueMatches=(l)=>apiState.fixtures.filter(f=>l[3].some(n=>String(fixtureLeagueName(f)).toLowerCase()===n.toLowerCase()));
...
async function loadFixtures(force=false){
  ...
  try{
    ...
    if(!res.ok) throw new Error(body?.error||`Errore backend (${res.status})`);

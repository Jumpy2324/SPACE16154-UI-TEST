...
function v73RealOddsEdge(pack,pred){
  ...
  const scan=x=>{
    ...
    const odd=realNum(x?.value??x?.odd??x?.odds??x?.price);
  ...
  for(const r of refs){
    for(const c of candidates){
      ...
      if(edge>=15 && (!best||edge>best.score))best={type:"Value",kind:"value",score:edge,pick:r.pick,prob:r.prob};
    }
  }
  return best;
}
...
async function v73BuildSignal(f){
  try{
    ...
    const confidence=v59Confidence(pred,hm,am);
    ...
    // Threshold decided for the Signal Center: confidence >= 70.
    // v59Confidence already caps small samples, therefore a premature signal cannot pass.
    if(minSample>=3 && confidence>=70){
      signals.push({type:"Confidence",kind:"confidence",score:confidence,pick,prob});
    }
    ...
    if(minSample>=3 && value)signals.push(value);
...
function v73SignalBadges(signals){
  return signals.map(s=>`<span class="${s.kind}">${signalLabel(s)} <b>${s.type==="Value"?"+"+s.score+"%":s.type==="Forma"?"+"+s.score:s.score+"%"}</b></span>`).join("");
}
    ...
    root.innerHTML=`<div class="page v41-signals-page v73-signals-page">
      ...
      <details class="v46-signal-guide">
        ...
        <div class="v46-guide-grid">
          ...
          <div><b>VALUE</b><p>Compare solo quando sono disponibili quote reali e il vantaggio stimato è almeno del 15%. Nessuna quota demo.</p></div>
...
async function v67RealTeamProfile(payload){
  ...
  try{
    ...
    const history=v76LeagueHistory(rawHistory,leagueId,15);
...
async function realMatchDetail(id){
  ...
  try{
    ...
    document.querySelector('#main').innerHTML=`<div class="page v14-analysis-page">
      ...
      <p class="real-note v76-last5-note">${v76Last5Label(analysisLeagueName)} = le cinque gare più recenti disputate in questo campionato. Coppe e amichevoli non sono incluse in questa fase.</p>

      ${v60PostMatchHtml(detail,h,a)}
      <div class="v14-accordions">
        ${v722TechCompare(tech,h,a)}

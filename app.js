...
function v7717MatchCard(f,{league=true,action=true,compact=false}={}){
  ...
  const leagueName=f?.leagueName||f?.league?.name||"";
  const time=realEsc(f?.time||fmtTime(f?.starting_at)||"");
  const leagueImg=f?.league?.image_path||f?.league?.logo||f?.league_image_path||"";
  const leagueIdentity=league ? `<div class="v7717-league-center">${leagueImg?`<img src="${realEsc(leagueImg)}" alt="${realEsc(leagueName)}" loading="lazy" referrerpolicy="no-referrer">`:""}<span>${realEsc(leagueName)}</span></div>` : "";

  let stateText=time;
  let stateClass="scheduled";
  let scoreValue=`<div class="v7717-score-vs">VS</div>`;
  if(st==="ft"){
    stateText="FT"; stateClass="ft";
    scoreValue=`<div class="v7717-score-main">${hs??"—"}–${as??"—"}</div>`;
  }else if(st==="live"){
    stateText="● LIVE"; stateClass="live";
    scoreValue=`<div class="v7717-score-main">${hs??0}–${as??0}</div>`;
  }

  return `<article class="v7717-match-card ${compact?"compact":""}" data-real-match="${f.id}">
    ${leagueIdentity?`<header class="v7717-match-header">${leagueIdentity}</header>`:""}
    <div class="v7717-match-body">
      ...
      <div class="v7717-score">
        <div class="v7717-score-state ${stateClass}">${stateText}</div>
        ${scoreValue}
        ${action?`<div class="v7717-inline-action">ANALISI <span>›</span></div>`:""}
      </div>
      <div class="v7717-team">
  ...
  try{
    ...
    const render=(filter="all")=>{
      ...
      list.innerHTML=groups.length?groups.map(([l,fs])=>`<section class="v67-center-league"><h2>${realEsc(l)}</h2>${fs.map(f=>{
        ...
        return v7717MatchCard(f,{league:false,action:false,compact:true})}).join("")}</section>`).join(""):`<div class="empty-note">Nessuna partita per questo filtro.</div>`;
 ...
 try{
  ...
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   ...
   ${fs.slice(1,4).length ? `<section class="v5-upcoming">
    <div class="v2-panel-head"><h2>PROSSIME PARTITE</h2><button data-nav="matches">Vedi tutte ›</button></div>
    <div class="v5-upcoming-list">${fs.slice(1,4).map(x=>v7717MatchCard(x,{league:false,action:true,compact:true})).join("")}</div>
   </section>` : ""}
  </div>`;
  bind();
  document.querySelectorAll("[data-v63-date]").forEach(b=>{

...
const CONFIG={
  version:"10.5.0-ui-test-v55-full-real",
...
/* ===== V50 SPORTMONKS LIVE CORE ===== */
const SM_V50_BASE="https://space16154-api.gianmarco-candeletti.workers.dev";
const SM_V50_ALLOWED=["Serie A","Premier League","La Liga","Bundesliga","Ligue 1"];
const SM_V50_ALIASES={"Primera Division":"La Liga"};
const SM_V50_CACHE=new Map();
function smV50LeagueName(f){
  const raw=String(f?.league?.name||f?.league_name||"");
  return SM_V50_ALIASES[raw]||raw;
}
function smV50Participant(f,loc){
  const ps=Array.isArray(f?.participants)?f.participants:[];
  return ps.find(p=>String(p?.meta?.location||"").toLowerCase()===loc)||{};
}
function smV50Normalize(f){
  const h=smV50Participant(f,"home"),a=smV50Participant(f,"away");
  const ts=Number(f?.starting_at_timestamp||0);
  const d=ts?new Date(ts*1000):null;
  const time=d&&!Number.isNaN(d.getTime())?d.toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:"Europe/Rome"}):(String(f?.starting_at||"").slice(11,16)||"--:--");
  const normTeam=(t,fallback)=>({name:t.name||fallback,code:String(t.short_code||t.name||fallback[0]).replace(/[^A-Za-z0-9]/g,"").slice(0,3).toUpperCase(),cls:"",image:t.image_path||""});
  return {raw:f,id:`sm-${f.id}`,smId:f.id,league:smV50LeagueName(f),leagueImage:f?.league?.image_path||"",time,h:normTeam(h,"Casa"),a:normTeam(a,"Trasferta"),state:f?.state?.name||""};
}
async function smV50Fetch(date,force=false){
  if(!force&&SM_V50_CACHE.has(date)) return SM_V50_CACHE.get(date);
  let res,body={};
  try{
    res=await fetch(`${SM_V50_BASE}/fixtures?date=${encodeURIComponent(date)}`,{cache:"no-store",headers:{Accept:"application/json"}});
    body=await res.json().catch(()=>({}));
  }catch(e){throw new Error(`Connessione backend fallita: ${e?.message||e}`)}
  if(!res.ok){
    const sm=body?.sportmonks;
    const detail=sm?.message||sm?.error||body?.message||body?.error||"Errore sconosciuto";
    throw new Error(`HTTP ${res.status} · ${detail}`);
  }
  const arr=(Array.isArray(body?.data)?body.data:[]).map(smV50Normalize).filter(x=>SM_V50_ALLOWED.includes(x.league));
  SM_V50_CACHE.set(date,arr);
  return arr;
}
function smV50Iso(y,m,d){return `${y}-${String(m+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`}
function smV50Shield(t,side=""){
  const img=String(t?.image||"").replace(/["<>]/g,"");
  return img?`<span class="sm52-crest ${side}"><img src="${img}" alt="" loading="lazy" referrerpolicy="no-referrer"></span>`:`<span class="m13-shield ${side}"><b>${String(t?.code||"?").replace(/[<>]/g,"")}</b></span>`;
}
function smV50LeagueLogo(m){const img=String(m?.leagueImage||m?.raw?.league?.image_path||"").replace(/["<>]/g,"");return img?`<img class="sm52-league-inline" src="${img}" alt="" loading="lazy" referrerpolicy="no-referrer">`:""}
...
function matchesForSelectedDate(){
  const key=smV50Iso(DASH_YEAR,DASH_MONTH,DASH_DAY);
  return SM_V50_CACHE.get(key)||[];
}
...
function dashboard(){
 ...
 const loading=!SM_V50_CACHE.has(key);
 const hero=dayMatches[0];
 root.innerHTML=`<div class="page v2-dashboard">
   <div class="dash-global-head v8-global-head">
     ...
     <div class="v8-date-pair" aria-label="Selettore data">
       <button class="v8-date-control" data-open-month><small>MESE</small><strong>${MONTHS_IT[DASH_MONTH].slice(0,3).toUpperCase()}</strong><span class="v18-toggle">+</span></button>
       <button class="v8-date-control day" data-open-day><small>GIORNO</small><strong>${String(DASH_DAY).padStart(2,"0")}</strong><span class="v18-toggle">+</span></button>
     </div>
   </div>
   <div class="v48-api-state ${loading?"":"v48-api-ok"}"><b>SPORTMONKS ${loading?"":"LIVE DATA"}</b><span>${loading?"Caricamento partite reali…":`${dayMatches.length} partite dei 5 campionati · ${key}`}</span></div>
   ${loading?`<section class="v6-hero-combined"><div class="v48-empty"><b>Caricamento…</b><span>Sto leggendo SportMonks.</span></div></section>`:
   !hero?`<section class="v6-hero-combined"><div class="v2-panel-head"><h2>MATCH IN EVIDENZA <span>☆</span></h2><b>${meta.short.toUpperCase()}</b></div><div class="v48-empty"><b>Nessuna partita</b><span>Nessuna fixture dei 5 campionati prevista per questa data.</span></div></section>`:
   `<section class="v6-hero-combined">
      <div class="v2-panel-head"><h2>MATCH IN EVIDENZA <span>☆</span></h2><b>${meta.short.toUpperCase()}</b></div>
      <div class="v2-meta">${smV50LeagueLogo(hero)}<span>${hero.league}</span> <i>•</i> ${meta.short} <i>•</i> ${hero.time}</div>
      <div class="v2-match sm52-hero-match"><div class="v2-club">${smV50Shield(hero.h)}<strong>${hero.h.name}</strong></div><div class="v2-center"><b>VS</b><button data-smfixture="${hero.smId}">DATI</button></div><div class="v2-club">${smV50Shield(hero.a,"away")}<strong>${hero.a.name}</strong></div></div>
      <div class="v6-prob-divider"></div><div class="v48-empty"><b>PREVISIONE IN PREPARAZIONE</b><span>Le percentuali demo sono state rimosse. Collegheremo qui statistiche, xG, FPI e motore SPACE16154.</span></div>
    </section>`}
   <section class="v5-upcoming"><div class="v2-panel-head"><h2>PROSSIME PARTITE</h2><button data-nav="matches">Vedi tutte ›</button></div><div class="v5-upcoming-list">
     ${(!loading&&dayMatches.length?dayMatches.slice(hero?1:0,4).map(x=>`<div class="v23-match-card"><div class="v23-time">${x.time}</div><div class="v23-match-row"><div class="v23-team home">${smV50Shield(x.h)}<b>${x.h.name}</b></div><button class="v23-analysis" data-smfixture="${x.smId}">DATI</button><div class="v23-team away"><b>${x.a.name}</b>${smV50Shield(x.a,"away")}</div></div><div class="v48-fixture-meta">${x.league}</div></div>`).join(""):`<div class="empty-note">${loading?"Caricamento…":"Nessun'altra partita disponibile per questa data."}</div>`)}
   </div></section>
   <section class="v2-signals"><div class="v2-panel-head"><h2>TOP SIGNALS</h2></div><div class="v48-empty"><b>SEGNALI NON ANCORA CALCOLATI</b><span>Questa sezione non usa più dati demo. Verrà alimentata dal motore reale SPACE16154.</span></div></section>
 </div>`;
 bind();
 ...
 document.querySelectorAll("[data-smfixture]").forEach(b=>b.onclick=()=>window.SPACE16154_API?.openFixtureDetail?.(b.dataset.smfixture));
 if(loading){
   smV50Fetch(key).then(()=>{if(document.querySelector(".v2-dashboard"))dashboard()}).catch(err=>{
     if(!document.querySelector(".v2-dashboard"))return;
     document.querySelector("#main").insertAdjacentHTML("afterbegin",`<div class="v48-api-state v48-api-warn"><b>SPORTMONKS</b><span>${String(err.message).replace(/[<>]/g,"")}</span></div>`);
   });
 }
}
function matches(){
...
function v19AnalysisHome(){v27History({kind:"analysis-home"});document.querySelector("#main").innerHTML=`<div class="page v19-home"><header><h1>ANALYSIS</h1><p>INTELLIGENCE CENTER</p></header><div class="v19-leagues">${V19L.map(l=>`<button data-v19l="${l[0]}">${v19Logo(l[1])}<b>${l[0]}</b></button>`).join("")}</div></div>`;bind();document.querySelectorAll("[data-v19l]").forEach(b=>b.onclick=()=>v19League(b.dataset.v19l))}
...
/* ===== V55 FULL UI RESTORE · REAL DATA ===== */
const V55_BASE="https://space16154-api.gianmarco-candeletti.workers.dev";
const V55_LEAGUES=["Serie A","Premier League","La Liga","Bundesliga","Ligue 1"];
const v55Esc=s=>String(s??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
const v55Clamp=(v,a=0,b=100)=>Math.max(a,Math.min(b,v));
const v55Norm=(v,a,b)=>v==null?null:v55Clamp((Number(v)-a)/(b-a)*100);
const v55Inv=(v,a,b)=>v==null?null:100-v55Norm(v,a,b);
const v55Avg=a=>a.length?a.reduce((x,y)=>x+y,0)/a.length:null;
async function v55Json(path){const r=await fetch(V55_BASE+path,{cache:"no-store",headers:{Accept:"application/json"}});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j?.sportmonks?.message||j?.message||j?.error||`HTTP ${r.status}`);return j}
function v55Fixtures(x){return Array.isArray(x?.data)?x.data:Array.isArray(x)?x:[]}
function v55Parts(f){const p=Array.isArray(f?.participants)?f.participants:[];return {h:p.find(x=>x?.meta?.location==="home")||p[0]||{},a:p.find(x=>x?.meta?.location==="away")||p[1]||{}}}
function v55Goals(f,teamId){const s=Array.isArray(f?.scores)?f.scores:[];const cand=s.filter(x=>String(x?.participant_id)===String(teamId)&&["CURRENT","2ND_HALF","FT"].includes(String(x?.description||"").toUpperCase()));return cand.length?Number(cand[cand.length-1]?.score?.goals??cand[cand.length-1]?.score??0):null}
function v55Stat(f,teamId,names){const stats=Array.isArray(f?.statistics)?f.statistics:[];const keys=names.map(x=>x.toLowerCase());const st=stats.find(x=>String(x?.participant_id)===String(teamId)&&keys.includes(String(x?.type?.developer_name||x?.type?.name||"").toLowerCase()));const v=st?.data?.value??st?.value;const n=Number(v);return Number.isFinite(n)?n:null}
function v55Xg(f,teamId){const a=Array.isArray(f?.xGFixture)?f.xGFixture:Array.isArray(f?.xgfixture)?f.xgfixture:[];const x=a.find(z=>String(z?.participant_id)===String(teamId));const n=Number(x?.data?.value??x?.value??x?.xg);return Number.isFinite(n)?n:null}
function v55Profile(fixtures,teamId){
 const fs=fixtures.slice().sort((a,b)=>new Date(a.starting_at)-new Date(b.starting_at)).slice(-10), rows=[];
 for(const f of fs){const {h,a}=v55Parts(f),me=String(h.id)===String(teamId)?h:a,opp=String(h.id)===String(teamId)?a:h;const gf=v55Goals(f,teamId),ga=v55Goals(f,opp.id);rows.push({f,gf,ga,xg:v55Xg(f,teamId),poss:v55Stat(f,teamId,["ball possession","possession"]),shots:v55Stat(f,teamId,["shots total","shots"]),sot:v55Stat(f,teamId,["shots on target"]),corners:v55Stat(f,teamId,["corners"]),danger:v55Stat(f,teamId,["dangerous attacks"]),attacks:v55Stat(f,teamId,["attacks"]),opp:opp.name||"Avversario"})}
 const vals=k=>rows.map(r=>r[k]).filter(Number.isFinite), mean=k=>v55Avg(vals(k));
 const form=rows.slice(-5).map(r=>r.gf==null||r.ga==null?"—":r.gf>r.ga?"V":r.gf<r.ga?"P":"N");
 const pts=rows.slice(-5).map(r=>r.gf==null||r.ga==null?null:r.gf>r.ga?3:r.gf===r.ga?1:0).filter(Number.isFinite);
 const threatVals=[v55Norm(mean("xg"),.5,2.4),v55Norm(mean("shots"),5,20),v55Norm(mean("sot"),1,8),v55Norm(mean("danger"),15,80)].filter(Number.isFinite);
 const defenseVals=[v55Inv(mean("ga"),.4,2.4)].filter(Number.isFinite);
 const formScore=pts.length?pts.reduce((a,b)=>a+b,0)/(pts.length*3)*100:null;
 const macros={threat:threatVals.length?Math.round(v55Avg(threatVals)):null,defense:defenseVals.length?Math.round(v55Avg(defenseVals)):null,form:formScore==null?null:Math.round(formScore),opponent:null,pressing:null,availability:null};
 const avail=Object.values(macros).filter(Number.isFinite),score=avail.length?Math.round(v55Avg(avail)):null;
 const trend=rows.slice(-5).map(r=>{const p=r.gf==null||r.ga==null?50:r.gf>r.ga?85:r.gf===r.ga?55:25;const q=[v55Norm(r.xg,.4,2.5),v55Norm(r.shots,4,20),v55Norm(r.sot,1,8)].filter(Number.isFinite);return Math.round(q.length?p*.55+v55Avg(q)*.45:p)});
 return {rows,form,macros,score,trend,avg:{gf:mean("gf"),ga:mean("ga"),xg:mean("xg"),poss:mean("poss"),shots:mean("shots"),sot:mean("sot"),corners:mean("corners"),danger:mean("danger"),attacks:mean("attacks")}};
}
function v55Val(v,d=1){return Number.isFinite(v)?Number(v).toLocaleString("it-IT",{minimumFractionDigits:d,maximumFractionDigits:d}):"—"}
function v55MacroRows(p){const defs=[["threat","Threat Creation","Capacità offensiva da xG, tiri, tiri in porta e attacchi pericolosi."],["defense","Defense Stability","Solidità difensiva preliminare dai gol concessi; verrà ampliata con xGA e tiri concessi."],["form","Forma","Punti ottenuti nelle ultime gare disponibili."],["opponent","Opponent Strength","In attesa della ponderazione definitiva della forza avversari."],["pressing","Pressing / PPDA","In attesa di PPDA/pressione storica sufficiente."],["availability","Availability","In attesa di dati affidabili su assenze e disponibilità."]];return defs.map(([k,n,d])=>{const v=p.macros[k];return `<div class="v14-macro"><div class="v14-macro-name"><span class="v14-macro-icon">◆</span><div><b>${n}</b><small>${d}</small></div></div><div class="v14-macro-values"><div><strong>${Number.isFinite(v)?v:"—"}</strong><span class="v14-bar ${Number.isFinite(v)?(v>=75?"high":v>=60?"mid":"low"):"neutral"}"><i style="width:${Number.isFinite(v)?v:0}%"></i></span></div><em class="neu">REAL</em><div><strong>—</strong><span class="v14-bar neutral"><i style="width:0%"></i></span></div></div></div>`}).join("")}
function v55TrendSvg(a,b){const max=100,w=500,h=150,p=18;const points=(arr)=>arr.map((v,i)=>`${p+i*(w-2*p)/Math.max(1,arr.length-1)},${h-p-(v/max)*(h-2*p)}`).join(" ");return `<div class="v55-trend"><svg viewBox="0 0 ${w} ${h}" aria-label="Trend rating reale"><polyline class="home" points="${points(a.length?a:[50,50])}"/><polyline class="away" points="${points(b.length?b:[50,50])}"/></svg><div><span>● Casa</span><span>● Trasferta</span></div></div>`}

function v19AnalysisHome(){document.querySelector("#main").innerHTML=`<div class="page v19-home"><header><h1>ANALYSIS</h1><p>INTELLIGENCE CENTER · SPORTMONKS REAL DATA</p></header><div class="v19-leagues">${V55_LEAGUES.map((l,i)=>`<button data-v55league="${v55Esc(l)}"><span class="v19-logo">${["A","PL","LL","BL","L1"][i]}</span><b>${v55Esc(l)}</b></button>`).join("")}</div><section class="panel v55-preserve"><div class="section-head"><h2>STRUTTURA MODELLO</h2><span>V55 REAL</span></div><p class="subtitle">Ranking, profilo squadra, 6 macroaree, dati tecnici e Trend Model restano nel progetto. Ogni numero viene mostrato solo se deriva da SportMonks; altrimenti compare —.</p></section></div>`;bind();document.querySelectorAll("[data-v55league]").forEach(b=>b.onclick=()=>v19League(b.dataset.v55league))}
async function v19League(name){const main=document.querySelector("#main");main.innerHTML=`<div class="page v19-league"><button class="v19-back" data-v55back>← ANALYSIS</button><div class="eyebrow">SPORTMONKS REAL DATA</div><h1>${v55Esc(name)}</h1><section class="panel"><div class="v48-empty"><b>Caricamento Intelligence Ranking…</b></div></section></div>`;document.querySelector("[data-v55back]").onclick=()=>v19AnalysisHome();try{const date=smV50Iso(DASH_YEAR,DASH_MONTH,DASH_DAY);const ms=await smV50Fetch(date,true);const league=ms.filter(x=>x.league===name);const season=league[0]?.raw?.season_id;let standings=[];if(season){const st=await v55Json(`/standings?season=${season}`);standings=Array.isArray(st?.data)?st.data:[];}main.innerHTML=`<div class="page v19-league"><button class="v19-back" data-v55back>← ANALYSIS</button><div class="eyebrow">INTELLIGENCE RANKING · REAL DATA</div><h1>${v55Esc(name)}</h1><section class="v19-ranking"><div class="v19-rhead"><span>#</span><span>SQUADRA</span><span>PG</span><span>ULTIME 5</span><span>RATING</span></div>${standings.length?standings.slice(0,20).map((s,i)=>{const t=s.participant||{};return `<button class="v19-rrow" data-v55team="${t.id||s.participant_id}" data-v55league="${v55Esc(name)}"><span>${s.position||i+1}</span><span>${t.image_path?`<img class="v55-mini" src="${v55Esc(t.image_path)}">`:""}<b>${v55Esc(t.name||"Squadra")}</b></span><span>${s.games_played??s.played??"—"}</span><span>REAL</span><strong>›</strong></button>`}).join(""):`<div class="v48-empty"><b>Classifica non disponibile per questa data</b><span>Le partite reali della giornata restano disponibili sotto.</span></div>`}</section><section class="panel"><div class="section-head"><h2>PARTITE DELLA DATA</h2><span>${date}</span></div>${league.map(m=>`<button class="match-list" data-v55fixture="${m.smId}"><span>${m.time}</span><div>${smV50Shield(m.h)}<b>${v55Esc(m.h.name)}</b></div><em>VS</em><div><b>${v55Esc(m.a.name)}</b>${smV50Shield(m.a,"away")}</div><strong>DATI</strong><small>${v55Esc(m.league)}</small></button>`).join("")||`<div class="empty-note">Nessuna partita del campionato nella data selezionata.</div>`}</section></div>`;document.querySelector("[data-v55back]").onclick=()=>v19AnalysisHome();document.querySelectorAll("[data-v55fixture]").forEach(b=>b.onclick=()=>window.SPACE16154_API?.openFixtureDetail?.(b.dataset.v55fixture));document.querySelectorAll("[data-v55team]").forEach(b=>b.onclick=()=>v19Team(b.dataset.v55team,b.dataset.v55league));}catch(e){main.innerHTML=`<div class="page"><button class="v19-back" data-v55back>← ANALYSIS</button><h1>${v55Esc(name)}</h1><div class="v48-empty"><b>Dato SportMonks non disponibile</b><span>${v55Esc(e.message)}</span></div></div>`;document.querySelector("[data-v55back]").onclick=()=>v19AnalysisHome()}}
async function v19Team(id,l){const main=document.querySelector("#main");main.innerHTML=`<div class="page"><button class="v19-back" data-v55back>← ${v55Esc(l)}</button><div class="v48-empty"><b>Caricamento profilo reale…</b></div></div>`;document.querySelector("[data-v55back]").onclick=()=>v19League(l);try{const date=smV50Iso(DASH_YEAR,DASH_MONTH,DASH_DAY);const h=await v55Json(`/team-history?team=${encodeURIComponent(id)}&date=${date}`);const fs=v55Fixtures(h),p=v55Profile(fs,id);const team=fs.flatMap(f=>f.participants||[]).find(x=>String(x.id)===String(id))||{id,name:"Squadra",short_code:"—"};main.innerHTML=`<div class="page v19-team"><button class="v19-back" data-v55back>← ${v55Esc(l)}</button><div class="eyebrow">PROFILO SQUADRA · SPORTMONKS</div><div class="v19-teamhero">${team.image_path?`<img class="v55-teamlogo" src="${v55Esc(team.image_path)}">`:""}<div><h1>${v55Esc(team.name)}</h1><span>${v55Esc(l)}</span></div><div class="v19-rating"><small>MODEL RATING</small><strong>${p.score??"—"}</strong><span>PRELIMINARE</span></div></div><section class="panel"><div class="section-head"><h2>6 MACROAREE</h2><span>REAL DATA</span></div><div class="v14-macros">${v55MacroRows(p)}</div></section><section class="panel"><div class="section-head"><h2>DATI TECNICI</h2><span>MEDIE STORICO</span></div><div class="compare-row"><span>xG</span><b>${v55Val(p.avg.xg)}</b></div><div class="compare-row"><span>Gol fatti</span><b>${v55Val(p.avg.gf)}</b></div><div class="compare-row"><span>Gol subiti</span><b>${v55Val(p.avg.ga)}</b></div><div class="compare-row"><span>Possesso</span><b>${v55Val(p.avg.poss,0)}%</b></div><div class="compare-row"><span>Tiri</span><b>${v55Val(p.avg.shots)}</b></div><div class="compare-row"><span>Tiri in porta</span><b>${v55Val(p.avg.sot)}</b></div><div class="compare-row"><span>Corner</span><b>${v55Val(p.avg.corners)}</b></div></section><section class="panel"><div class="section-head"><h2>TREND MODEL SCORE</h2><span>ULTIME 5</span></div>${v55TrendSvg(p.trend,[])}</section></div>`;document.querySelector("[data-v55back]").onclick=()=>v19League(l);}catch(e){main.innerHTML=`<div class="page"><button class="v19-back" data-v55back>← ${v55Esc(l)}</button><div class="v48-empty"><b>Dati squadra non disponibili</b><span>${v55Esc(e.message)}</span></div></div>`;document.querySelector("[data-v55back]").onclick=()=>v19League(l)}}
function signalsPage(){document.querySelector("#main").innerHTML=`<div class="page v41-signals-page"><div class="v41-title-row"><div><div class="eyebrow">SIGNAL CENTER · REAL DATA</div><h1>TOP SIGNALS</h1></div><span class="v41-signal-count">0 SEGNALI</span></div><p class="subtitle">La struttura completa resta attiva. I segnali verranno pubblicati solo dopo il calcolo reale e la calibrazione.</p><details class="v46-signal-guide"><summary><span>COME LEGGERE I SEGNALI</span><em>+</em></summary><div class="v46-guide-grid"><div><b>ALTA CONVERGENZA</b><p>Più famiglie di indicatori sostengono lo stesso esito.</p></div><div><b>CONFIDENCE</b><p>Solidità e convergenza del quadro, separata dalla probabilità.</p></div><div><b>VALUE</b><p>Scostamento favorevole rispetto alla probabilità implicita.</p></div><div><b>FORMA</b><p>Dinamica recente elaborata dai dati reali.</p></div></div></details><div class="top-filter-grid v41-filters"><button class="filter active">Tutti</button><button class="filter confidence">Confidence</button><button class="filter value">Value</button><button class="filter form">Forma</button></div><div class="v48-empty"><b>NESSUN SEGNALE CALCOLATO</b><span>Nessun numero demo viene utilizzato.</span></div></div>`}
function performance(){document.querySelector("#main").innerHTML=`<div class="page"><button class="back-btn" data-nav="more">‹ ALTRO</button><div class="eyebrow">MODEL PERFORMANCE</div><h1>PERFORMANCE</h1><section class="panel"><div class="section-head"><h2>TREND MODEL</h2><span>BACKTEST REALE</span></div><div class="v48-empty"><b>STORICO IN COSTRUZIONE</b><span>La grafica resta predisposta; i risultati inizieranno dal primo backtest SportMonks.</span></div></section></div>`;bind()}
function confidencePage(){document.querySelector("#main").innerHTML=`<div class="page"><button class="back-btn" data-nav="more">‹ ALTRO</button><div class="eyebrow">MODEL CONFIDENCE</div><h1>CONFIDENCE</h1><section class="panel"><div class="section-head"><h2>CONFIDENCE ENGINE</h2><span>CALIBRAZIONE</span></div><div class="v48-empty"><b>—</b><span>Resta separata dalla probabilità dell'esito. Verrà popolata solo dopo calibrazione e backtesting.</span></div></section></div>`;bind()}
function more(){document.querySelector("#main").innerHTML=`<div class="page v14-more"><div class="eyebrow">SPACE16154</div><h1>ALTRO</h1><button class="more-row2" data-route="performance"><b>Performance</b><span>Backtest e trend del modello ›</span></button><button class="more-row2" data-route="confidence"><b>Model Confidence</b><span>Solidità della previsione ›</span></button><button class="more-row2" data-info="${encodeURIComponent("SportMonks → normalizzazione → indicatori proprietari → probabilità → Confidence. Nessun dato demo.")}"><b>Metodologia</b><span>Pipeline dati reale ›</span></button></div>`;bind()}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=10.5.0-ui-test-v55-full-real",{updateViaCache:"none"}).catch(()=>{}));

/* SPACE16154 V48 — MATCHES / SPORTMONKS BRIDGE */
(()=>{
const leagues=[
  ["serie-a","Serie A","A",["Serie A"]],
  ["premier","Premier League","PL",["Premier League"]],
  ["laliga","La Liga","LL",["La Liga","Primera Division"]],
  ["bundesliga","Bundesliga","BL",["Bundesliga"]],
  ["ligue1","Ligue 1","L1",["Ligue 1"]]
];

// V48: inseriremo qui l'URL del backend sicuro dopo la sua pubblicazione.
// La chiave SportMonks NON deve mai essere inserita in questo file.
const SPORTMONKS_BRIDGE_DEFAULT="https://space16154-api.gianmarco-candeletti.workers.dev";
const bridgeBase=()=>String(localStorage.getItem("space16154_api_base")||SPORTMONKS_BRIDGE_DEFAULT).replace(/\/$/,"");
const bridgeReady=()=>/^https:\/\//i.test(bridgeBase());
const months=["GEN","FEB","MAR","APR","MAG","GIU","LUG","AGO","SET","OTT","NOV","DIC"];
const now=new Date(); let yr=now.getFullYear(),mo=now.getMonth(),day=now.getDate(),active=null,m13ManualDate=false;
let apiState={key:"",status:"idle",fixtures:[],error:""};
const cache=new Map();

const isoDate=()=>`${yr}-${String(mo+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
const daysInSelectedMonth=()=>new Date(yr,mo+1,0).getDate();
...
const logo=(x,img="")=>img?`<span class="sm52-league-logo"><img src="${escapeHtml(img)}" alt="" loading="lazy" referrerpolicy="no-referrer"></span>`:`<span class="m13-league-logo">${x}</span>`;
const shield=(x,c="",img="")=>img?`<span class="sm52-crest ${c}"><img src="${escapeHtml(img)}" alt="" loading="lazy" referrerpolicy="no-referrer"></span>`:`<span class="m13-shield ${c}"><b>${escapeHtml(x)}</b></span>`;
const leagueImage=(l)=>{const f=apiState.fixtures.find(f=>l[3].some(n=>String(fixtureLeagueName(f)).trim().toLowerCase()===String(n).trim().toLowerCase())&&f?.league?.image_path);return f?.league?.image_path||""};
const escapeHtml=(v)=>String(v??"").replace(/[&<>'"]/g,ch=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[ch]));
const teamCode=(t)=>String(t?.short_code||t?.code||t?.name||"?").replace(/[^A-Za-z0-9]/g,"").slice(0,3).toUpperCase()||"?";
const fixtureLeagueName=(f)=>f?.league?.name||f?.league_name||"";
const participantByLocation=(f,location)=>{
  const ps=Array.isArray(f?.participants)?f.participants:[];
  return ps.find(p=>String(p?.meta?.location||"").toLowerCase()===location)||null;
};
const normalizeFixture=(f)=>{
  const home=participantByLocation(f,"home")||f?.home||{};
  const away=participantByLocation(f,"away")||f?.away||{};
  const ts=Number(f?.starting_at_timestamp||0);
  const d=ts?new Date(ts*1000):null;
  const time=d&&!Number.isNaN(d.getTime())?d.toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:"Europe/Rome"}):(String(f?.starting_at||"").slice(11,16)||"--:--");
  return {...f,home,away,time};
};
const leagueMatches=(l)=>apiState.fixtures.filter(f=>l[3].some(n=>String(fixtureLeagueName(f)).trim().toLowerCase()===String(n).trim().toLowerCase()));

function connectionBanner(){
  if(!bridgeReady()) return `<div class="v48-api-state v48-api-warn"><b>SPORTMONKS</b><span>Backend sicuro non ancora collegato.</span></div>`;
  if(apiState.status==="loading") return `<div class="v48-api-state"><b>SPORTMONKS</b><span>Caricamento partite reali…</span></div>`;
  if(apiState.status==="error") return `<div class="v48-api-state v48-api-warn"><b>SPORTMONKS</b><span>${escapeHtml(apiState.error||"Errore di collegamento")}</span></div>`;
  if(apiState.status==="ready") return `<div class="v48-api-state v48-api-ok"><b>SPORTMONKS LIVE DATA</b><span>${apiState.fixtures.length} partite ricevute · ${isoDate()}</span></div>`;
  return "";
}

function home(){
  return `<section class="m13-page"><div class="m13-titlebar"><h1 class="v23-matches-title">MATCHES</h1>${cal()}</div>
    ${connectionBanner()}
    <div class="m13-leagues">${leagues.map(l=>`<button class="m13-league" data-league="${l[0]}">${logo(l[2],leagueImage(l))}<span>${l[1]}</span>${apiState.status==="ready"?`<em>${leagueMatches(l).length}</em>`:""}</button>`).join("")}</div>
  </section>`;
}

function fixtureCard(f){
  const h=f.home||{},a=f.away||{};
  return `<article class="v23-match-card sm52-match-card">
    <div class="v23-time">${escapeHtml(f.time||"--:--")}</div>
    <div class="v23-match-row">
      <div class="v23-team home">${shield(teamCode(h),"",h.image_path||"")}<b>${escapeHtml(h.name||"Home")}</b></div>
      <button class="v23-analysis" data-smfixture="${escapeHtml(String(f.id||""))}">DATI</button>
      <div class="v23-team away"><b>${escapeHtml(a.name||"Away")}</b>${shield(teamCode(a),"away",a.image_path||"")}</div>
    </div>
    ${f.state?.name?`<div class="v48-fixture-meta">${escapeHtml(f.state.name)}</div>`:""}
  </article>`;
}

function view(l){
  const arr=leagueMatches(l);
  let content="";
  if(!bridgeReady()) content=`<div class="v48-empty"><b>Collegamento SportMonks da completare</b><span>La V48 è pronta a ricevere le fixtures dal backend sicuro.</span></div>`;
  else if(apiState.status==="loading") content=`<div class="v48-empty"><b>Caricamento…</b><span>Sto leggendo le fixtures reali del ${isoDate()}.</span></div>`;
  else if(apiState.status==="error") content=`<div class="v48-empty"><b>Impossibile caricare le partite</b><span>${escapeHtml(apiState.error)}</span><button class="primary-btn" data-v48-retry>RIPROVA</button></div>`;
  else if(!arr.length) content=`<div class="v48-empty"><b>Nessuna partita</b><span>Nessuna fixture ${escapeHtml(l[1])} trovata per il ${isoDate()}.</span></div>`;
  else content=arr.map(fixtureCard).join("");
  return `<section class="m13-page v23-matches-league">
    <div class="v23-cal-top">${cal()}</div>
    <div class="m13-leaguehead v23-leaguehead"><button class="m13-back" data-back>←</button><div class="m13-leagueidentity">${logo(l[2],leagueImage(l))}<h1>${l[1]}</h1></div></div>
    ${connectionBanner()}
    <div class="m13-fixtures v23-league-cards">${content}</div>
  </section>`;
}

function findDemoMatchId(homeName,awayName){
  const norm=s=>String(s||"").toLowerCase().replace(/[^a-z0-9]/g,"");
  const h=norm(homeName),a=norm(awayName);
  const m=MATCHES.find(x=>norm(x.h.name)===h&&norm(x.a.name)===a);
  return m?.id||"";
}

async function loadFixtures(force=false){
  if(!bridgeReady()) {apiState={key:isoDate(),status:"idle",fixtures:[],error:""};return;}
  const key=isoDate();
  if(!force&&cache.has(key)){apiState={key,status:"ready",fixtures:cache.get(key),error:""};return;}
  apiState={key,status:"loading",fixtures:[],error:""};
  const root=document.querySelector("[data-m13='1']"); if(root)render(root,false);
  try{
    const ctl=new AbortController(); const timer=setTimeout(()=>ctl.abort(),12000);
    const res=await fetch(`${bridgeBase()}/fixtures?date=${encodeURIComponent(key)}`,{headers:{"Accept":"application/json"},signal:ctl.signal,cache:"no-store"});
    clearTimeout(timer);
    const body=await res.json().catch(()=>({}));
    if(!res.ok){const sm=body?.sportmonks;const detail=sm?.message||sm?.error||body?.message||body?.error||"Errore sconosciuto";throw new Error(`HTTP ${res.status} · ${detail}`);}
    const fixtures=Array.isArray(body?.data)?body.data.map(normalizeFixture):[];
    cache.set(key,fixtures); apiState={key,status:"ready",fixtures,error:""};
  }catch(err){
    const msg=err?.name==="AbortError"?"Timeout nel collegamento al backend.":(err?.message||"Errore sconosciuto");
    apiState={key,status:"error",fixtures:[],error:msg};
  }
  const current=document.querySelector("[data-m13='1']"); if(current)render(current,false);
}

function picker(type){
  const vals=type==="month"?months:Array.from({length:daysInSelectedMonth()},(_,i)=>String(i+1));
  const cur=type==="month"?mo:day-1;
  return `<div class="m13-sheetback"><div class="m13-sheet"><div class="m13-sheethead"><b>${type==="month"?"Seleziona mese":"Seleziona giorno"}</b><button data-close>×</button></div><div class="m13-scroll">${vals.map((v,i)=>`<button class="${i===cur?"active":""}" data-pick="${type}" data-i="${i}">${v}</button>`).join("")}</div></div></div>`;
}

let smDetailModal=null;
const smNum=v=>{const n=Number(v);return Number.isFinite(n)?n:null};
const smSafeData=b=>b&&b.ok===true?b.data:null;
const smHistory=b=>{const d=smSafeData(b);return Array.isArray(d)?d:[]};
const smDateTs=f=>new Date(String(f?.starting_at||'').replace(' ','T')+'Z').getTime()||0;
const smTeamInFixture=(f,teamId)=>{const ps=Array.isArray(f?.participants)?f.participants:[];return ps.find(p=>String(p?.id)===String(teamId))||null};
const smOppInFixture=(f,teamId)=>{const ps=Array.isArray(f?.participants)?f.participants:[];return ps.find(p=>String(p?.id)!==String(teamId))||null};
function smCurrentGoals(f,teamId){
  const scores=Array.isArray(f?.scores)?f.scores:[];
  const exact=scores.filter(x=>String(x?.participant_id)===String(teamId)&&String(x?.description||'').toUpperCase()==='CURRENT');
  const pool=exact.length?exact:scores.filter(x=>String(x?.participant_id)===String(teamId));
  const vals=pool.map(x=>smNum(x?.score?.goals??x?.score?.value??x?.score)).filter(v=>v!==null);
  return vals.length?vals[vals.length-1]:null;
}
function smRecentForm(fixtures,teamId){
  return fixtures.slice().sort((a,b)=>smDateTs(b)-smDateTs(a)).slice(0,5).map(f=>{
    const gf=smCurrentGoals(f,teamId),opp=smOppInFixture(f,teamId),ga=opp?smCurrentGoals(f,opp.id):null;
    let r='—'; if(gf!==null&&ga!==null) r=gf>ga?'W':gf<ga?'L':'D';
    return {r,gf,ga,opp:opp?.name||'Avversario',date:String(f?.starting_at||'').slice(0,10)};
  });
}
function smStatName(st){return String(st?.type?.developer_name||st?.type?.name||st?.name||'').toUpperCase().replace(/[^A-Z0-9]+/g,'_')}
function smStatValue(st){return smNum(st?.data?.value??st?.value)}
const SM_METRICS=[
  ['xG',n=>n.includes('EXPECTED_GOALS')||n==='XG'],
  ['Possesso %',n=>n.includes('BALL_POSSESSION')||n.includes('POSSESSION')],
  ['Tiri',n=>n.includes('SHOTS_TOTAL')||n==='TOTAL_SHOTS'||n==='SHOTS'],
  ['Tiri in porta',n=>n.includes('SHOTS_ON_TARGET')||n.includes('SHOTS_ON_GOAL')],
  ['Corner',n=>n.includes('CORNERS')],
  ['Attacchi pericolosi',n=>n.includes('DANGEROUS_ATTACKS')],
  ['Attacchi',n=>n==='ATTACKS'||n.endsWith('_ATTACKS')],
  ['Falli',n=>n.includes('FOULS')]
];
function smHistoryAverages(fixtures,teamId){
  return SM_METRICS.map(([label,test])=>{
    const vals=[];
    fixtures.forEach(f=>(Array.isArray(f?.statistics)?f.statistics:[]).forEach(st=>{
      if(String(st?.participant_id)!==String(teamId))return;
      if(!test(smStatName(st)))return;
      const v=smStatValue(st);if(v!==null)vals.push(v);
    }));
    return vals.length?{label,value:vals.reduce((a,b)=>a+b,0)/vals.length,count:vals.length}:null;
  }).filter(Boolean);
}
function smXgAverage(fixtures,teamId){
  const vals=[];
  fixtures.forEach(f=>{
    const xs=Array.isArray(f?.xGFixture)?f.xGFixture:Array.isArray(f?.xgfixture)?f.xgfixture:[];
    xs.forEach(x=>{if(String(x?.participant_id)===String(teamId)){const v=smNum(x?.data?.value??x?.value);if(v!==null)vals.push(v)}})
  });
  return vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null;
}
function smStanding(pack,teamId){
  const d=smSafeData(pack?.standings); const rows=Array.isArray(d)?d:Array.isArray(d?.data)?d.data:[];
  const row=rows.find(x=>String(x?.participant_id??x?.participant?.id)===String(teamId));
  if(!row)return null;
  return row.position??row.rank??row.result??null;
}
function smFmt(v,d=1){return Number(v).toLocaleString('it-IT',{minimumFractionDigits:d,maximumFractionDigits:d})}
function smFormHtml(items){
  if(!items.length)return '<div class="sm54-form sm54-form-empty"><span>—</span><small>Dati recenti non disponibili</small></div>';
  return `<div class="sm54-form">${items.map(x=>`<span class="${x.r==='W'?'w':x.r==='L'?'l':x.r==='D'?'d':''}" title="${escapeHtml(x.opp)}">${x.r}</span>`).join('')}</div>`;
}
function smMetricMap(history,teamId){
  const avg=smHistoryAverages(history,teamId), map={};
  avg.forEach(x=>map[x.label]=x);
  const xg=smXgAverage(history,teamId);
  if(xg!==null) map['xG']={label:'xG',value:xg,count:history.length};
  const form=smRecentForm(history,teamId);
  const gf=form.filter(x=>x.gf!==null).map(x=>x.gf),ga=form.filter(x=>x.ga!==null).map(x=>x.ga);
  if(gf.length)map['Gol fatti']={label:'Gol fatti',value:gf.reduce((a,b)=>a+b,0)/gf.length,count:gf.length};
  if(ga.length)map['Gol subiti']={label:'Gol subiti',value:ga.reduce((a,b)=>a+b,0)/ga.length,count:ga.length};
  return {map,form};
}
function smMetricCell(x,label){
  if(!x)return `<div class="sm54-val missing"><b>—</b><small>nessun dato</small></div>`;
  const digits=label==='Possesso %'?0:1;
  return `<div class="sm54-val"><b>${smFmt(x.value,digits)}</b><small>${x.count||0} rilev.</small></div>`;
}
function smCompareTable(h,hh,a,ah){
  const hm=smMetricMap(hh,h?.id),am=smMetricMap(ah,a?.id);
  const rows=[
    ['Gol fatti','Gol fatti'],['Gol subiti','Gol subiti'],['xG','xG'],['Possesso','Possesso %'],
    ['Tiri','Tiri'],['Tiri in porta','Tiri in porta'],['Corner','Corner'],['Attacchi pericolosi','Attacchi pericolosi'],
    ['Attacchi','Attacchi'],['Falli','Falli']
  ];
  return `<section class="sm54-compare">
    <div class="sm54-compare-head"><div>${shield(teamCode(h),'',h?.image_path||'')}<b>${escapeHtml(h?.name||'Casa')}</b></div><span>CONFRONTO</span><div><b>${escapeHtml(a?.name||'Trasferta')}</b>${shield(teamCode(a),'away',a?.image_path||'')}</div></div>
    <div class="sm54-form-row"><div><small>ULTIME 5</small>${smFormHtml(hm.form)}</div><div><small>ULTIME 5</small>${smFormHtml(am.form)}</div></div>
    <div class="sm54-table">${rows.map(([label,key])=>`<div class="sm54-row">${smMetricCell(hm.map[key],key)}<span>${label}</span>${smMetricCell(am.map[key],key)}</div>`).join('')}</div>
  </section>`;
}
function smH2hHtml(pack){
  const arr=smHistory(pack?.h2h);
  if(!arr.length)return `<div class="sm54-empty">Nessun testa a testa disponibile.</div>`;
  return `<div class="sm54-h2h">${arr.slice().sort((a,b)=>smDateTs(b)-smDateTs(a)).slice(0,5).map(f=>{
    const ps=Array.isArray(f?.participants)?f.participants:[],h=ps.find(p=>p?.meta?.location==='home')||ps[0]||{},a=ps.find(p=>p?.meta?.location==='away')||ps[1]||{};
    const hg=smCurrentGoals(f,h.id),ag=smCurrentGoals(f,a.id);
    return `<div><span>${escapeHtml(String(f?.starting_at||'').slice(0,10))}</span><b>${escapeHtml(h.name||'Casa')}</b><strong>${hg??'—'}–${ag??'—'}</strong><b>${escapeHtml(a.name||'Trasferta')}</b></div>`;
  }).join('')}</div>`;
}
function smFixtureStatsHtml(detail){
  const stats=Array.isArray(detail?.statistics)?detail.statistics:[];
  if(!stats.length)return `<div class="sm54-live-empty"><b>PRE-MATCH</b><span>Le statistiche della gara compariranno qui quando la partita inizierà.</span></div>`;
  const rows=stats.slice(0,20).map(st=>`<div class="sm51-stat"><span>${escapeHtml(st?.type?.name||st?.type?.developer_name||'Statistica')}</span><b>${escapeHtml(st?.data?.value??st?.value??'—')}</b></div>`).join('');
  return `<div class="sm51-stats">${rows}</div>`;
}

function sm55RealProfile(history,teamId){return v55Profile(history,teamId)}
function sm55PairVal(h,a,key,d=1){return `<div class="compare-row"><span>${key}</span><b>${v55Val(h,d)}</b><b>${v55Val(a,d)}</b></div>`}
function sm55MacroCompare(hp,ap){const defs=[["threat","Threat Creation"],["defense","Defense Stability"],["form","Forma"],["opponent","Opponent Strength"],["pressing","Pressing / PPDA"],["availability","Availability"]];return defs.map(([k,n])=>{const hv=hp.macros[k],av=ap.macros[k],dv=Number.isFinite(hv)&&Number.isFinite(av)?hv-av:null;return `<div class="v14-macro"><div class="v14-macro-name"><span class="v14-macro-icon">◆</span><div><b>${n}</b><small>${Number.isFinite(hv)||Number.isFinite(av)?"Elaborazione preliminare su dati SportMonks":"Dato non ancora sufficiente"}</small></div></div><div class="v14-macro-values"><div><strong>${Number.isFinite(hv)?hv:"—"}</strong><span class="v14-bar ${Number.isFinite(hv)?(hv>=75?"high":hv>=60?"mid":"low"):"neutral"}"><i style="width:${Number.isFinite(hv)?hv:0}%"></i></span></div><em class="${dv==null?"neu":dv>=0?"pos":"neg"}">${dv==null?"—":(dv>=0?"+":"")+Math.round(dv)}</em><div><strong>${Number.isFinite(av)?av:"—"}</strong><span class="v14-bar ${Number.isFinite(av)?(av>=75?"high":av>=60?"mid":"low"):"neutral"}"><i style="width:${Number.isFinite(av)?av:0}%"></i></span></div></div></div>`}).join("")}
function sm55ProbabilityPlaceholder(){return `<section class="v14-card"><div class="v14-card-title">PROBABILITÀ DEL MODELLO <button class="info" data-info="${encodeURIComponent("Le probabilità 1X2, Gol/No Gol e Over/Under saranno mostrate solo dopo calibrazione del motore sui dati reali.")}">i</button></div><div class="v14-markets"><div class="v14-market"><b>1X2</b><div><span><small>1</small><strong>—</strong></span><span><small>X</small><strong>—</strong></span><span><small>2</small><strong>—</strong></span></div></div><div class="v14-market"><b>GOL / NO GOL</b><div><span><small>GOL</small><strong>—</strong></span><span><small>NO GOL</small><strong>—</strong></span></div></div><div class="v14-market"><b>OVER / UNDER 2.5</b><div><span><small>OVER</small><strong>—</strong></span><span><small>UNDER</small><strong>—</strong></span></div></div></div></section>`}

function smDetailModalHtml(state,pack=null,detail=null,error=''){
  if(state==='loading')return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal sm54-modal" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button><div class="sm51-kicker">SPORTMONKS · DATI REALI</div><h2>Caricamento analisi…</h2><div class="sm51-spinner"></div></div></div>`;
  if(state==='error')return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal sm54-modal" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button><div class="sm51-kicker">SPORTMONKS</div><h2>Impossibile caricare i dati</h2><p>${escapeHtml(error)}</p></div></div>`;
  const fx=pack?.fixture||{},teams=pack?.teams||{},h=teams.home||{},a=teams.away||{};
  const f=normalizeFixture(detail||{participants:[h,a],starting_at:fx.starting_at,league:fx.league});
  const league=fx?.league?.name||detail?.league?.name||'—',leagueImg=fx?.league?.image_path||detail?.league?.image_path||'';
  const dateLabel=String(fx?.starting_at||detail?.starting_at||isoDate()).slice(0,10),stateName=detail?.state?.name||'Pre-match';
  const hh=smHistory(pack?.home_history),ah=smHistory(pack?.away_history),hs=smStanding(pack,h.id),as=smStanding(pack,a.id);
  const hp=sm55RealProfile(hh,h.id),ap=sm55RealProfile(ah,a.id),modelDiff=Number.isFinite(hp.score)&&Number.isFinite(ap.score)?hp.score-ap.score:null;
  return `<div class="sm51-backdrop" data-smclose><div class="sm51-modal sm54-modal sm55-full" onclick="event.stopPropagation()"><button class="sm51-x" data-smclose>×</button>
    <div class="sm51-kicker">SPACE16154 · SPORTMONKS REAL DATA</div><h2>ANALISI PRE-MATCH</h2>
    <div class="sm51-meta sm54-meta"><span class="sm52-league-pill">${leagueImg?`<img src="${escapeHtml(leagueImg)}" alt="">`:''}${escapeHtml(league)}</span><span>${escapeHtml(dateLabel)}</span><span>${escapeHtml(f.time||String(fx?.starting_at||'').slice(11,16)||'--:--')}</span></div>
    <div class="sm54-hero"><div>${shield(teamCode(h),'',h.image_path||'')}<b>${escapeHtml(h.name||'Casa')}</b><small>${hs?`Classifica ${escapeHtml(hs)}°`:'Classifica —'}</small></div><strong>VS</strong><div>${shield(teamCode(a),'away',a.image_path||'')}<b>${escapeHtml(a.name||'Trasferta')}</b><small>${as?`Classifica ${escapeHtml(as)}°`:'Classifica —'}</small></div></div>
    <div class="sm54-state"><span>${escapeHtml(stateName)}</span><small>Storico disponibile: ${escapeHtml(pack?.history_window?.days||90)} giorni</small></div>
    ${sm55ProbabilityPlaceholder()}
    <section class="v14-card v14-model"><div class="v14-card-title">ANALISI DEL MODELLO · PRELIMINARE</div><div class="v14-score-head"><div class="v14-score-team">${shield(teamCode(h),'',h.image_path||'')}<b>${escapeHtml(h.name)}</b><strong>${hp.score??'—'}</strong></div><div class="v14-score-diff"><small>MODEL SCORE</small><strong>${modelDiff==null?'—':(modelDiff>=0?'+':'')+modelDiff}</strong><span>Dati reali disponibili</span></div><div class="v14-score-team">${shield(teamCode(a),'away',a.image_path||'')}<b>${escapeHtml(a.name)}</b><strong>${ap.score??'—'}</strong></div></div><div class="v14-macros">${sm55MacroCompare(hp,ap)}</div></section>
    <section class="v14-card"><div class="v14-card-title">TREND MODEL SCORE · DATI REALI</div>${v55TrendSvg(hp.trend,ap.trend)}<small class="sm55-method">Trend preliminare elaborato dalle ultime gare ricevute: risultato + produzione offensiva disponibile. Non è ancora il rating definitivo.</small></section>
    <details class="v14-fold" open><summary>DATI TECNICI <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><div class="compare-head"><span>INDICATORE</span><b>${escapeHtml(teamCode(h))}</b><b>${escapeHtml(teamCode(a))}</b></div>${sm55PairVal(hp.avg.xg,ap.avg.xg,'xG')}${sm55PairVal(hp.avg.gf,ap.avg.gf,'Gol fatti')}${sm55PairVal(hp.avg.ga,ap.avg.ga,'Gol subiti')}${sm55PairVal(hp.avg.poss,ap.avg.poss,'Possesso',0)}${sm55PairVal(hp.avg.shots,ap.avg.shots,'Tiri')}${sm55PairVal(hp.avg.sot,ap.avg.sot,'Tiri in porta')}${sm55PairVal(hp.avg.corners,ap.avg.corners,'Corner')}${sm55PairVal(hp.avg.danger,ap.avg.danger,'Attacchi pericolosi')}${sm55PairVal(hp.avg.attacks,ap.avg.attacks,'Attacchi')}</div></details>
    <details class="v14-fold"><summary>FORMA RECENTE <span class="v18-toggle">+</span></summary><div class="v14-fold-body">${smCompareTable(h,hh,a,ah)}</div></details>
    <details class="v14-fold"><summary>TESTA A TESTA <span class="v18-toggle">+</span></summary><div class="v14-fold-body">${smH2hHtml(pack)}</div></details>
    <details class="v14-fold"><summary>PROBABILI FORMAZIONI <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><div class="sm54-live-empty"><b>—</b><span>Verranno mostrate quando SportMonks renderà disponibili formazioni reali per la fixture.</span></div></div></details>
    <details class="v14-fold"><summary>ASSENZE <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><div class="sm54-live-empty"><b>—</b><span>Nessuna assenza viene inventata. La sezione resta pronta per il feed reale.</span></div></div></details>
    <details class="v14-fold"><summary>ARBITRO <span class="v18-toggle">+</span></summary><div class="v14-fold-body"><div class="sm54-live-empty"><b>—</b><span>Profilo arbitro mostrato solo quando disponibile da SportMonks.</span></div></div></details>
    <div class="sm51-section-title sm54-title">STATISTICHE DELLA FIXTURE</div>${smFixtureStatsHtml(detail)}
    <div class="sm54-note"><b>ZERO DATI DEMO</b><span>La struttura grafica e tecnica precedente è stata mantenuta. I valori presenti derivano da SportMonks e le macroaree indicate come preliminari verranno sostituite dalle formule definitive dopo calibrazione/backtesting.</span></div>
  </div></div>`;
}
function smCloseDetail(){smDetailModal?.remove();smDetailModal=null;}
async function smOpenDetail(id){
  if(!id)return;
  smCloseDetail();document.body.insertAdjacentHTML('beforeend',smDetailModalHtml('loading'));smDetailModal=document.querySelector('.sm51-backdrop');smDetailModal.querySelectorAll('[data-smclose]').forEach(x=>x.onclick=smCloseDetail);
  try{
    const [pr,fr]=await Promise.all([
      fetch(`${bridgeBase()}/prematch?id=${encodeURIComponent(id)}`,{headers:{Accept:'application/json'},cache:'no-store'}),
      fetch(`${bridgeBase()}/fixture?id=${encodeURIComponent(id)}`,{headers:{Accept:'application/json'},cache:'no-store'})
    ]);
    const pb=await pr.json().catch(()=>({})),fb=await fr.json().catch(()=>({}));
    if(!pr.ok){const d=pb?.sportmonks?.message||pb?.message||pb?.error||`HTTP ${pr.status}`;throw new Error(d)}
    if(!fr.ok){const d=fb?.sportmonks?.message||fb?.message||fb?.error||`HTTP ${fr.status}`;throw new Error(d)}
    smCloseDetail();document.body.insertAdjacentHTML('beforeend',smDetailModalHtml('ready',pb,fb?.data||fb));smDetailModal=document.querySelector('.sm51-backdrop');smDetailModal.querySelectorAll('[data-smclose]').forEach(x=>x.onclick=smCloseDetail);
  }catch(e){smCloseDetail();document.body.insertAdjacentHTML('beforeend',smDetailModalHtml('error',null,null,e?.message||'Errore sconosciuto'));smDetailModal=document.querySelector('.sm51-backdrop');smDetailModal.querySelectorAll('[data-smclose]').forEach(x=>x.onclick=smCloseDetail)}
}

function bind(root){
  root.querySelectorAll("[data-league]").forEach(b=>b.onclick=()=>{active=b.dataset.league;v27History({kind:"matches-league",league:active});render(root)});
  root.querySelector("[data-back]")?.addEventListener("click",()=>{if(history.state?.kind==="matches-league"&&history.length>1)history.back();else{active=null;render(root)}});
  ...
  root.querySelectorAll("[data-smfixture]").forEach(b=>b.onclick=()=>smOpenDetail(b.dataset.smfixture));
  root.querySelector("[data-v48-retry]")?.addEventListener("click",()=>loadFixtures(true));
}
function bindPicker(root){
  ...
  root.querySelectorAll("[data-pick]").forEach(b=>b.onclick=()=>{
    m13ManualDate=true;
    if(b.dataset.pick==="month") {mo=+b.dataset.i; day=Math.min(day,daysInSelectedMonth());}
    else day=+b.dataset.i+1;
    root.querySelector(".m13-sheetback")?.remove();
    cache.delete(isoDate());
    render(root); loadFixtures();
  });
}
function render(root,bindAfter=true){root.innerHTML=active?view(leagues.find(l=>l[0]===active)):home();if(bindAfter)bind(root);else bind(root)}
function mount(){
  let els=[...document.querySelectorAll("main,.page,.screen,section,#app")],root=els.find(e=>/Scegli un campionato|PARTITE DEL GIORNO/i.test(e.textContent||""));
  if(!root||root.dataset.m13)return;
  root.dataset.m13="1";render(root);loadFixtures();
}
window.SPACE16154_API={
  openFixtureDetail(id){return smOpenDetail(id)},
  setBackend(url){localStorage.setItem("space16154_api_base",String(url||"").replace(/\/$/,""));cache.clear();location.reload()},
  clearBackend(){localStorage.removeItem("space16154_api_base");cache.clear();location.reload()},
  getBackend(){return bridgeBase()}
};
window.addEventListener("popstate",e=>{
 const s=e.state;
 if(s?.space16154v27&&s.kind==="matches-league"){
   active=s.league; const root=document.querySelector("[data-m13='1']"); if(root)render(root);
 }else if(s?.space16154v27&&s.kind==="route"&&s.n==="matches"){
   active=null; setTimeout(mount,0);
 }
});

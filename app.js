 ...
 try{
  ...
  const render=()=>{
    ...
    root.innerHTML=`<section class="m13-page v712-match-center">
      ...
      <div class="v716-league-filter-wrap">
        <label>CAMPIONATO</label>
        <button type="button" class="v716-league-trigger" id="v716LeagueTrigger">
          <span class="v716-trigger-logo">${leagueFilter==="all"?"★":(()=>{const l=leagues.find(x=>x.key===leagueFilter);return l?v60LeagueLogo(l,true):"★"})()}</span>
          <strong>${leagueFilter==="all"?"Tutti i campionati":realEsc((leagues.find(x=>x.key===leagueFilter)||{}).name||"Tutti i campionati")}</strong>
          <span class="v716-trigger-arrow">⌄</span>
        </button>
      </div>

      <div class="v716-sheet-backdrop" id="v716SheetBackdrop" hidden>
        <div class="v716-sheet" role="dialog" aria-modal="true" aria-label="Seleziona campionato">
          <div class="v716-sheet-handle"></div>
          <div class="v716-sheet-head">
            <div><small>FILTRO</small><strong>Campionato</strong></div>
            <button type="button" id="v716SheetClose" aria-label="Chiudi">×</button>
          </div>
          <div class="v716-sheet-options">
            <button type="button" data-v716-league="all" class="${leagueFilter==="all"?"active":""}">
              <span class="v716-option-logo">★</span><b>Tutti i campionati</b><em>${leagueFilter==="all"?"✓":""}</em>
            </button>
            ${leagues.map(l=>`<button type="button" data-v716-league="${l.key}" class="${leagueFilter===l.key?"active":""}">
              <span class="v716-option-logo">${v60LeagueLogo(l,true)}</span><b>${realEsc(l.name)}</b><em>${leagueFilter===l.key?"✓":""}</em>
            </button>`).join("")}
          </div>
        </div>
      </div>
...
function v722TeamTech(metrics,team,sample){
  ...
  return `<section class="v19-card v723-team-tech">
    ...
    <div class="v723-team-tech-head">${realCrest(team)}<div><b>${realEsc(team.name||"")}</b><small>PERFORMANCE MEDIA</small></div></div>
    ...
    ${rows.length?`<div class="v723-team-tech-list">${rows.map(r=>`<div><span>${r[0]}</span><strong>${fmt(r[1],r[2])}</strong></div>`).join("")}</div>`:""}
  </section>`;
}

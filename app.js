...
function v7721LeagueBadge(f){
  ...
  const logo=f?.leagueLogo||f?.league?.image_path||f?.league?.logo||"";
  const img=logo ? `<img class="v7724-real-league-logo" src="${realEsc(logo)}" alt="" loading="lazy" decoding="async">` : "";
  return `<div class="v7721-league-badge v7724-league-badge">${img}<span>${realEsc(name)}</span></div>`;
}

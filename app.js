...
const CONFIG={
  version:"11.9.0-v69-history-fallback",
...
async function realDashboard(){
 ...
 try{
  const fs=await v68SafeFixtures(meta.id);
  const cmeta=await v60CompetitionMeta().catch(()=>({competitions:[]}));
  const m=fs[0];if(!m)throw new Error("Nessuna partita dei 5 campionati per la data selezionata.");
  ...
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   ...
   <section class="v6-hero-combined">
    <div class="v2-panel-head"><h2>MATCH IN EVIDENZA <span>☆</span></h2><b>${meta.short.toUpperCase()}</b></div>
    <div class="v2-meta">${v60LeagueLogo(comp,true)} ${realEsc(m.leagueName)} <i>•</i> ${meta.short} <i>•</i> ${realEsc(m.time)}${stateLabel}</div>
    <div class="v2-match">
  ...
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard"><div class="empty-note">${realEsc(e.message)}</div></div>`;bind()
 }
}


function v69LeagueKeyByName(name){
 const n=String(name||"").toLowerCase();
 return V60_COMPETITIONS.find(x=>String(x.name).toLowerCase()===n)?.key||"";
}
function v69HistoryData(pack){
 if(pack?.ok===true&&Array.isArray(pack.data))return pack.data;
 if(Array.isArray(pack?.data?.data))return pack.data.data;
 if(Array.isArray(pack?.fixtures))return pack.fixtures;
 return [];
}
async function v69TeamHistory(teamId,seasonId=null){
 const q=new URLSearchParams({team:String(teamId),limit:"5"});
 if(seasonId)q.set("season",String(seasonId));
 return await realApi(`/team-history?${q.toString()}`);
}
async function v69AnalysisSource(id){
 const fb=await realApi(`/fixture?id=${encodeURIComponent(id)}`);
 const detail=fb?.data||fb;
 try{
   const pack=await realApi(`/prematch?id=${encodeURIComponent(id)}`);
   return {pack,detail,prematch:true};
 }catch(preErr){
   const h=realLoc(detail,"home")||{},a=realLoc(detail,"away")||{};
   if(!h?.id||!a?.id)throw preErr;
   const sid=detail?.season_id||detail?.season?.id||null;
   const leagueKey=v69LeagueKeyByName(detail?.league?.name);
   const [hh,ah,h2,comp]=await Promise.all([
     v69TeamHistory(h.id,sid).catch(()=>({ok:false,data:[]})),
     v69TeamHistory(a.id,sid).catch(()=>({ok:false,data:[]})),
     realApi(`/h2h?home=${encodeURIComponent(h.id)}&away=${encodeURIComponent(a.id)}`).catch(()=>({ok:false,data:[]})),
     leagueKey?realApi(`/competition?key=${encodeURIComponent(leagueKey)}`).catch(()=>null):Promise.resolve(null)
   ]);
   return {
     detail,
     prematch:false,
     pack:{
       ...
       fixture:detail,
       teams:{home:h,away:a},
       home_history:{ok:true,data:v69HistoryData(hh)},
       away_history:{ok:true,data:v69HistoryData(ah)},
       h2h:{ok:true,data:v69HistoryData(h2)},
       standings:comp?{ok:true,data:comp.rows||comp.standings||[]}:{ok:false,data:[]},
...
async function v67RealTeamProfile(payload){
  ...
  try{
    ...
    const histPack=await v69TeamHistory(teamId,b?.season?.id||null).catch(()=>({ok:false,data:[]}));
    const recent=v69HistoryData(histPack).slice(0,5);
    const metrics=v58Metrics(recent,team),macro=v58Macro(metrics),trend=v58PerfTrend(recent,teamId);
    ...
    root.innerHTML=`<div class="page v67-team-page v69-team-page">
      ...
      <div class="v69-sample-banner ${sample>=3?"ready":"early"}">
        <b>${sample}/5 GARE REALI</b>
        <span>${sample===0?"Nessuna gara utile della stagione corrente.":sample<3?"Campione iniziale: valori ancora preliminari.":"Campione sufficiente per attivare il Model Rating."}</span>
      </div>
      ...
      <section class="v19-card v67-rating-card v68-rating-card"><div class="v19-chead"><b>MODEL RATING</b><span class="${sample<3?"v68-prelim-tag":""}">${sample<3?"PRELIMINARE":rating+"/100"}</span></div>
        ...
        <p>${sample<3?`Campione reale insufficiente: ${sample} gara${sample===1?"":"e"} utile. Il rating viene attivato da 3 gare.`:"Rating calcolato sulle ultime gare reali della stagione corrente."}</p>
      </section>
      ...
      <section class="v19-card v68-macro-card"><div class="v19-chead"><b>MACROAREE</b><span>${sample<3?`PRELIMINARI · ${sample}/5`:`${sample}/5 GARE`}</span></div>
        ...
        <p class="real-note v68-real-note">${sample<3?"Le macroaree sono preliminari finché il campione non cresce. ":""}Si usano solo gare della stagione corrente: nessun dato della stagione precedente viene mescolato.</p>
      </section>
...
async function realMatchDetail(id){
  ...
  try{
    const src=await v69AnalysisSource(id),pack=src.pack,detail=src.detail,fb={data:detail};
    const fx=pack.fixture||detail||{},h=pack.teams?.home||realLoc(detail,"home")||{},a=pack.teams?.away||realLoc(detail,"away")||{};
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=11.9.0-v69-history-fallback",{updateViaCache:"none"}).catch(()=>{}));

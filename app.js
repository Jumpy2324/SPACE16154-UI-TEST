...
function changeDashMonth(delta){
  DASH_MONTH+=delta;
  if(DASH_MONTH>11){DASH_MONTH=0;DASH_YEAR++}
  if(DASH_MONTH<0){DASH_MONTH=11;DASH_YEAR--}
  normalizeDashDay();
  route("dashboard");
}
function changeDashDay(delta){
  DASH_DAY+=delta;
  const max=daysInMonth(DASH_YEAR,DASH_MONTH);
  if(DASH_DAY>max){
    DASH_DAY=1;DASH_MONTH++;
    if(DASH_MONTH>11){DASH_MONTH=0;DASH_YEAR++}
  }
  if(DASH_DAY<1){
    DASH_MONTH--;
    if(DASH_MONTH<0){DASH_MONTH=11;DASH_YEAR--}
    DASH_DAY=daysInMonth(DASH_YEAR,DASH_MONTH);
  }
  route("dashboard");
}
function matchesForSelectedDate(){
  // Demo deterministico. SportMonks sostituirà questo mapping con fixture.date.
...
function dashboard(){
 ...
 document.querySelector("#main").innerHTML=`<div class="page v2-dashboard">
   <div class="dash-global-head v7-global-head">
     <div><span class="dash-global-kicker">DASHBOARD</span><small>${meta.label}</small></div>
     <div class="v7-date-wheel" aria-label="Selettore data">
       <div class="v7-wheel">
         <small>MESE</small>
         <button class="v7-arrow" data-month-up aria-label="Mese precedente">▲</button>
         <strong>${MONTHS_IT[DASH_MONTH].slice(0,3).toUpperCase()}</strong>
         <button class="v7-arrow" data-month-down aria-label="Mese successivo">▼</button>
       </div>
       <div class="v7-wheel day">
         <small>GIORNO</small>
         <button class="v7-arrow" data-day-up aria-label="Giorno precedente">▲</button>
         <strong>${String(DASH_DAY).padStart(2,"0")}</strong>
         <button class="v7-arrow" data-day-down aria-label="Giorno successivo">▼</button>
       </div>
     </div>
   </div>
 ...
 bind();
 document.querySelector("[data-month-up]").onclick=()=>changeDashMonth(-1);
 document.querySelector("[data-month-down]").onclick=()=>changeDashMonth(1);
 document.querySelector("[data-day-up]").onclick=()=>changeDashDay(-1);
 document.querySelector("[data-day-down]").onclick=()=>changeDashDay(1);
}
function matches(){
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=5.6.0-ui-test-v7",{updateViaCache:"none"}).catch(()=>{}));

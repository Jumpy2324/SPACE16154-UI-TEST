...
async function realDashboard(){
 ...
 try{
  ...
  document.querySelector("#main").innerHTML=`<div class="page v2-dashboard"><div class="dash-global-head v8-global-head"><div class="v20-dashboard-label"><span class="dash-global-kicker">DASHBOARD</span></div><div class="v8-date-pair"><button class="v8-date-control" data-v63-date="month"><small>MESE</small><strong>${MONTHS_IT[DASH_MONTH].slice(0,3).toUpperCase()}</strong><span>+</span></button><button class="v8-date-control day" data-v63-date="day"><small>GIORNO</small><strong>${String(DASH_DAY).padStart(2,"0")}</strong><span>+</span></button></div></div>
...
async function v67RealTeamProfile(payload){
  ...
  try{
    ...
    root.innerHTML=`<div class="page v67-team-page">
      ...
      <section class="v19-card v67-rating-card"><div class="v19-chead"><b>MODEL RATING</b><span>${rating==null?"—":rating+"/100"}</span></div>
      ...
      <section class="v19-card"><div class="v19-chead"><b>MACROAREE</b><span>${sample}/5 GARE</span></div>
        ...
        <p class="real-note">Nessun valore demo: “—” significa che SportMonks non fornisce ancora un campione sufficiente o il dato richiesto.</p>
      </section>
...
async function realMatches(){
 ...
 try{
  ...
  const home=()=>{root.innerHTML=`<section class="m13-page"><div class="m13-titlebar"><h1 class="v23-matches-title">MATCHES</h1></div><div class="m13-leagues">${leagues.map(l=>`<button class="m13-league v66-analysis-twin" data-rl="${l.key}"><span class="v62-league-center">${v60LeagueLogo(l)}<b>${realEsc(l.name)}</b></span></button>`).join("")}</div></section>`;root.querySelectorAll("[data-rl]").forEach(b=>b.onclick=()=>view(leagues.find(l=>l.key===b.dataset.rl)))};
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=11.7.0-v67-postmatch",{updateViaCache:"none"}).catch(()=>{}));

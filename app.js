...
function signalsPage(){
 ...
 document.querySelector("#main").innerHTML=`<div class="page v41-signals-page">
  ...
  <div class="v41-league-filter">
    ${["Tutti",...CONFIG.leagues].map((l,i)=>`<button class="${i===0?"active":""}" data-sig-league="${l}">${l==="Tutti"?"Tutti":l}</button>`).join("")}
  </div>
  <div class="v41-list-head"><h2>SEGNALI</h2><span id="sigCount"></span></div>
  <div id="sigList" class="v41-signal-list"></div>
 </div>`;
 SIG={type:"Tutti",league:"Tutti"};renderSig();bind();v42BindSignalFilters();
}
function v42BindSignalFilters(){
 ...
 document.querySelectorAll("[data-sig-league]").forEach(b=>b.onclick=()=>{
   SIG.league=b.dataset.sigLeague;
...
function renderSig(){
 ...
 document.querySelector("#sigList").innerHTML=matches.length?matches.map(m=>{
   ...
   return `<button class="v41-signal-card" data-match="${m.id}">
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=9.4.0-ui-test-v42",{updateViaCache:"none"}).catch(()=>{}));

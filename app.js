...
function v7721LeagueBadge(f){
  ...
  let logo="";
  try{
    // V77.20: resolve the competition directly by its real name.
    // V77.19 referenced a non-existent helper, so the catch silently removed the logo.
    const direct=f?.league?.image_path?f.league:null;
    const cached=v60CompFromName(V60_META_CACHE,name);
    const l=direct||cached;
    if(l) logo=v60LeagueLogo(l,true)||"";
  }catch{}
  return `<div class="v7721-league-badge">${logo}<span>${realEsc(name)}</span></div>`;
}

...
async function realMatchDetail(id){
  ...
  try{
    ...
    const v7522RollForm=(team,opp,preMetrics,postMetrics)=>{
      ...
      // Internal form order is newest -> oldest.
      // Normalize to oldest -> newest BEFORE rolling the 5-match window.
      const preChronological=preInternal.slice().reverse();

      if(!v60FixtureFinished(detail)) return preChronological;

      const gf=realScore(detail,team.id),ga=realScore(detail,opp.id);
      if(gf==null||ga==null){
        return post.length ? post.slice().reverse() : preChronological;
      }
